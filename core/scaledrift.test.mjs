...
//
// Named *.test.mjs so it stays out of the apps' served *.js surface. Run: node --test core/scaledrift.test.mjs
...
test("scaleDrift ramps as S_{i+1} = 1+(S0-1)*(1+v)^(i+1) — the i+1 exponent", () => {
  ...
  for (const i of [0, 1, 2]) {
    ...
    assert.ok(
      Math.abs(s.w - Math.abs(expect[i])) < 1e-6,
      `w must track |m| at i=${i}`,
    );
  }
  // i=0 giving 2.05 (not 2.0) is the whole point: exponent i would give (1.05)^0=1 → m=2.0.
  assert.ok(
    Math.abs(driftM(2.0, 0.05, 0).x - 2.0) > 0.04,
    "i=0 must NOT be the i-exponent value 2.0",
  );
});
...
test("scaleDrift ScaleVary=0 degenerates to a constant scale (the oracle anchor)", () => {
  for (const i of [0, 1, 5, 11]) {
    ...
    assert.ok(
      Math.abs(s.x - 2.0) < 1e-12,
      `ScaleVary=0 must be constant scale(2.0) at i=${i}, got ${s.x}`,
    );
  }
});
...
test("scaleDrift missing s.i defaults to 0 (belt-and-suspenders floor)", () => {
  ...
  assert.ok(
    Math.abs(s.x - 2.05) < 1e-6,
    `undefined s.i must floor to i=0 (m=2.05), got ${s.x}`,
  );
});
