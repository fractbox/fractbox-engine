...
// The leaf-resolution half of normalizeSceneObject (input precedence above),
// exported on its own because parameter INTERPOLATION needs it too: with an
// explicit shapeId the legacy primParam/primParam2 are read-only echoes, so a
// flythrough that lerps only those animates nothing — it must lerp the
// resolved block (app interp.ts lerpFormula).
export function resolveShapeLeaf(o) {
  ...
  // #627 — overflow params (sp4..sp7, the objAux lane) survive normalize.
  // Appended ONLY when present so ≤4-param objects keep their length-4 array
  // (and every consumer's existing shape assumptions) byte-for-byte.
  if (Array.isArray(sp))
    for (let j = 4; j < Math.min(sp.length, MAX_LEAF_PARAMS); j++)
      shapeParams.push(num(sp[j], 0));
  return { shapeId, shapeParams };
}

export function normalizeSceneObject(o) {
  const legacyType = Number(o.objType) & 0xf;
  const { shapeId, shapeParams } = resolveShapeLeaf(o);
  ...
  return {
    ...
    color: [
      col[0] ?? DEFAULT_COLOR[0],
      col[1] ?? DEFAULT_COLOR[1],
      col[2] ?? DEFAULT_COLOR[2],
    ],
