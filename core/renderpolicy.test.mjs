...
test("shadeLight: cheap tier zeroes shadow+AO+mirrors; settled passes the light through", () => {
  ...
  // #630 — reflBounces rides the same cheap-tier word flip as shadow/AO (a
  // drag renders the plain mirror-off variant; the settle pops mirrors back).
...
test("#496: itersForMagnification is one iteration per zoom octave, clamped", () => {
  ...
  assert.equal(
    itersForMagnification(5, 8.7),
    8,
    "the reporter's ×8.7 box ⇒ +3",
  );
...
test("#496: effectiveIters is behavior-preserving over the delegation", () => {
  ...
  assert.equal(
    effectiveIters(9, { dist: REF_DIST * 4 }),
    9,
    "zoomed OUT ⇒ base",
  );
  assert.equal(
