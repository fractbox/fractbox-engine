...
import { makeCamera, MIN_DIST, PT_MIN_DIST } from "./camera.js";
import {
  hybridLooseDE,
  df64Eligible,
  ptEligible,
...
export async function createPreview(canvas, opts = {}) {
  ...
  try {
    ...
    renderer = await createRenderer(canvas, {
      ...
      // composes rather than overlaps; real reinit/fallback stays deferred.
  ...
  function writeFrame(f, q, res, c) {
    ...
    settleFrame(renderer, f, coloring, q, res, c || cam, {
      ...
      perturbFor: (iters) =>
        ptNow() && !q.cheap && ptElig(f) && ensurePtOrbit(f, iters),
  ...
  // hold path) — this just warms ahead.
  function prewarmFor(formula) {
    if (!needsCompile(formula)) return null;
    const ff = frameFeatures(formula);
    return (
      (renderer.prewarmGeneralFor && renderer.prewarmGeneralFor(ff)) ||
      renderer.prewarmMarchFor(ff)
