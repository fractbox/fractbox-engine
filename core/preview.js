...
// TINY PLANET — the stereographic field-of-view range, in DEGREES. Exported so
// the app's slider and this module's clamp cannot disagree about the ends.
//
// The map's plane radius at the frame's vertical half-extent is
// tan(planetFov/4), so 360° is a true singularity (tan 90° = ∞) — the antipode
// is infinitely far out and nothing reaches the frame. MAX stops well short of
// it: at 330° the corner already looks 165° off-axis, which is as close to
// "the whole sphere" as is worth rendering. MIN sits just under a typical
// camera FOV (42°) so the slider's left end is genuinely a mild fisheye rather
// than a crop — at k = tan(fov/2)/2 the projection reproduces the perspective
// view's CENTRE magnification exactly, which for a 42° camera is ~43°.
export const PLANET_FOV_MIN = 40;
export const PLANET_FOV_MAX = 330;
// The default the View pane's toggle turns on with. Picked by MEASUREMENT —
// app/scripts/planet-probe.mjs sweeps the range against real WebGPU frames and
// scores the disc radius and the sky coverage at the frame border. 180 is the
// only value in the sweep that read as a planet on all three shapes tested
// (Mandelbulb disc 0.53, Menger 0.63, Mandelbox 0.72, border sky 1.000 on each
// — 150 already clipped the Mandelbox at edgeSky 0.929).
//
// It is also the one principled point on the scale: at 180 the plane radius is
// tan(45°) = 1, so the horizon — the circle of directions 90° off-axis —
// exactly inscribes the frame. Everything in front of you is inside that
// circle and everything behind you is in the corners, which is precisely the
// little-planet construction.
export const PLANET_FOV_DEFAULT = 180;
...
// ── World extents (CPU-DE measurement, pure) ────────────────────────────────
//
// Per-axis extents of the RENDERED body — the number a world-sized consumer
// needs (the MRI sweep's amplitude: a plane at ±ext[axis] is fully clear of
// the body, so one sweep period traverses all of it; the Loop Lab defect this
// fixes was a fixed 1.3 that never got past Whorl Citadel's midriff and
// barely nicked Mandelbox at all).
//
// This is the same seam as the streamlines world auto-scale (streamRescale
// below, the R3 Mandelbox lesson), but a STRONGER measurement, because the
// 8-ray inward march is provably insufficient here: on Whorl Citadel the
// centred rays slide through the spiral's gaps and read 0.68 while the
// rendered body reaches ±2.7 (probe census, PR #679 defect pass). A
// clearance guarantee needs volumetric coverage. And a fixed small threshold
// is equally wrong the other way: some DEs are wildly CONSERVATIVE far out
// (spiralVortex reads 0.2 at a true distance of ~1.3), so "d is small" far
// from the body proves nothing — but that same conservatism is what the
// render marches through, so its near-field iso-shell (the visible halo) IS
// the body a clip plane must clear.
//
// So: a coarse-to-fine voxel bbox of the near-field region {d < max(0.02,
// 0.6·h)}, h the current cell edge — each pass shrinks the box around what
// the previous one kept, the h-scaled threshold keeps thin features from
// slipping between samples while it is coarse, and the 0.02 floor stops the
// tightening at the rendered shell. ~180k DE evals worst case (≤ 80 ms on an
// M-class core; measured across the catalog) — cached per formula by the
// worldExtents() preview method, never on the pump.
//
// Returns { r, ext: [x, y, z] } (r = radial bound of the box corners), or
// null when the DE finds nothing inside ±48 (no CPU DE, or a degenerate
// formula) — callers keep their unit-scale fallback.
export function measureWorldExtents(de, { R0 = 48, n = 32, passes = 5 } = {}) {
  if (!de) return null;
  const lo = [-R0, -R0, -R0];
  const hi = [R0, R0, R0];
  try {
    for (let pass = 0; pass < passes; pass++) {
      const h = Math.max(...hi.map((v, a) => (v - lo[a]) / n));
      const tau = Math.max(0.02, 0.6 * h);
      const nlo = [1e9, 1e9, 1e9];
      const nhi = [-1e9, -1e9, -1e9];
      let occ = 0;
      for (let iz = 0; iz <= n; iz++)
        for (let iy = 0; iy <= n; iy++)
          for (let ix = 0; ix <= n; ix++) {
            const x = lo[0] + (ix * (hi[0] - lo[0])) / n;
            const y = lo[1] + (iy * (hi[1] - lo[1])) / n;
            const z = lo[2] + (iz * (hi[2] - lo[2])) / n;
            const d = de(x, y, z);
            if (d < tau) {
              occ++;
              if (x < nlo[0]) nlo[0] = x;
      ...
      let settled = true;
      for (let a = 0; a < 3; a++) {
        const nl = Math.max(lo[a], nlo[a] - h);
        const nh = Math.min(hi[a], nhi[a] + h);
        if (nh - nl < (hi[a] - lo[a]) * 0.95) settled = false;
        lo[a] = nl;
        hi[a] = nh;
      }
      // Box stopped shrinking AND the threshold is at its floor — done early.
      if (settled && tau <= 0.021) break;
    }
  } catch {
    return null; // a DE that throws mid-scan measures nothing
  }
  const ext = [0, 1, 2].map((a) => Math.max(Math.abs(lo[a]), Math.abs(hi[a])));
  return { r: Math.hypot(ext[0], ext[1], ext[2]), ext };
}

// worldExtents() cache — keyed on the formula OBJECT (the app clones on load,
// so an edited formula is a new key; matches getCpuDE's own memo posture).
const _worldExtCache = new WeakMap();
...
export async function createPreview(canvas, opts = {}) {
  ...
  // Fires SYNCHRONOUSLY after each live-canvas submit, before any await — the
  // only moment a WebGPU swap-chain canvas is readable from 2D/drawImage (one
  // task later it reads blank; same lifetime rule as createTileTarget's
  // "WHY NOT read the canvas" note in renderer.js). Consumers must read the
  // canvas inside the callback, not defer to a rAF. Never fires for offline/
  // export/capture draws (resized canvas or offscreen target — not the view).
  let onPresentCb = opts.onPresent || null;
  const firePresent = () => {
    if (!onPresentCb) return;
    try {
      onPresentCb();
    } catch (e) {
      console.error("onPresent:", e);
    }
  };
  ...
  let orthoH = 0;
  // TINY PLANET (IDEAS 2026-08-21 wave) — the stereographic projection's field
  // of view in RADIANS; 0 = off (the plain frustum). Lives here as closure
  // state for exactly the reasons #441 spells out above: the orbit model is a
  // tight {yaw,pitch,dist,fov,target} whose consumers break on a new field,
  // and one place should decide what writeGlobals, stillBlob and the
  // standalone bundles see.
  //
  // TRANSIENCE — matched to ortho in v1, with one deliberate difference:
  //   • NOT in the #c= share link. sharecodec's TAG.VIEW is fixed-width, so a
  //     field there is a codec-version change; ortho took the same call. The
  //     TRADE, stated rather than discovered later: a tiny-planet shot cannot
  //     be shared as a LINK, only as the PNG/video it renders to (which do
  //     carry it — the projection is in the render, not a post-pass).
  //   • Survives a formula/preset switch, and an ORBIT does not clear it —
  //     where ortho drops on the first orbit pixel. That rule exists because
  //     ortho is a snap-to-inspect mode and "reaching for orbit is asking for
  //     3D space back". Tiny planet is the opposite kind of thing: a LOOK you
  //     compose in, and orbiting is how you turn the planet. Dropping it on
  //     drag would make the mode unusable.
  let planetFov = 0;
  // tan(planetFov/4) — the plane radius the frame's vertical half-extent
  // reaches under the stereographic map, i.e. what every tier actually wants
  // (WGSL camRight.w, GL uPlanetK, cpu.js planetK, pixelRay). Derived in ONE
  // place so the four mirrors cannot disagree about the quarter-angle.
  const planetK = () => (planetFov > 0 ? Math.tan(planetFov / 4) : 0);
  // 360° EQUIRECTANGULAR (IDEAS "360°/VR" row, mono v1) — the third answer to
  // "how does a pixel become a ray", and the second arm on tiny planet's
  // seams. On, every frame is the FULL 360×180 lat-long panorama (image
  // centre = camFwd, so orbiting rotates it naturally); saved at a 2:1 size
  // it IS a 360 photo. No parameter at all (the full sphere is the point —
  // a boolean, not an angle), so unlike the planet there is no slider and no
  // remembered value.
  //
  // TRANSIENCE — the planet's rules verbatim, for the planet's reasons: NOT
  // in the #c= share link (a 360 shot travels as the PNG it renders to);
  // survives a formula/preset switch; an ORBIT does not clear it (orbiting is
  // how you look around the panorama). Mutually exclusive with BOTH ortho and
  // planet, enforced in the three setters below.
  let equirectOn = false;
  // π·H/W of the FULL image — the longitude scale every tier wants (WGSL
  // camUp.w; the GL tier derives its own constant, it never tiles; cpu.js
  // divides by its own aspect; pixelRay cancels it). Derived in ONE place so
  // the mirrors cannot disagree — and from the FULL dims, which is what makes
  // a tile render exact: during a tiled export writeFrame sees the TILE's
  // res, so stillBlobTiled parks the plate's dims here first.
  let tiledFullDims = null; // [W, H] while stillBlobTiled is rendering
  const equirectS = (res) => {
    if (!equirectOn) return 0;
    const d = tiledFullDims || res;
    return d && d[0] > 0 ? Math.PI * (d[1] / d[0]) : 0;
  };
  ...
  // ── Field streamlines (lab — core/streamlines.js, WebGPU tier only) ───────
  // While the overlay is on, a lightweight rAF ticker keeps the particles
  // moving between REAL frames: renderer.tickStreamlines() re-presents the
  // composed snapshot of the last frame + freshly-advected points, with no
  // march and no accumulation touched. It stands down whenever the pump owns
  // the frame (scheduled/busy — real draws carry the overlay themselves via
  // the renderer's drawTo/drawAccum hook) and while an offline export holds
  // the canvas.
  let streamOn = false;
  let streamRaf = 0;
  // World-scale probe (R3 — the Mandelbox lesson): presets do NOT share a
  // world size. Mandelbox renders at ~9-unit radius (camera dist 24) while
  // Menger/bulbs sit near 1.3, and a fixed spawn/bound world buried every
  // Mandelbox particle deep inside the body — alive but 99.8% occluded
  // (probe census). March the CPU DE inward along 8 rays for the bounding
  // radius and re-size the sim world (spawn shell, bound, shell offset,
  // swirl speed, and the shader-side wscale terms) to match. Best-effort:
  // no CPU DE (rare shapes) keeps the unit-scale defaults.
  //
  // DEEP ZOOM (the PR #656 v1 tail). The object radius alone is the wrong world
  // once the camera is INSIDE the object's bounding ball: at ×10³ the frustum is
  // a thousandth of the shape, the spawn ball still spanned the whole thing, and
  // essentially no particle landed on screen. The sim world therefore tracks the
  // SMALLER of the bounding radius and the frustum half-height at the orbit
  // target — and because particles live in p_rel (residual) space, whose ORIGIN
  // IS the orbit target (camera.js basis: camPos is eye − target), the existing
  // origin-centred spawn is already centred on what the user is looking at. Only
  // the scale was wrong, so only the scale changes — no shader edit, no new
  // uniform, and the emitted sim WGSL is byte-identical to v0.
  //
  // VIEW_K is the headroom that keeps this a DEEP-ZOOM branch only: the camera
  // term has to be well inside the object before it wins, so every whole-object
  // framing (every preset's own camera) still resolves to exactly the radius
  // v0 computed. The probe's per-preset numbers are the check on that.
  const STREAM_VIEW_K = 1.6;
  let streamRadius = 0, // cached bounding radius (the expensive half)
    streamRadiusFor = null, // …and the formula it was probed for
    streamLastScale = 0; // last pushed wscale (change guard for the cheap half)
  // The expensive half: march the CPU DE inward for the formula's bounding
  // radius. Cached per formula — streamRefocus runs per frame and must not.
  function streamProbeRadius() {
    if (streamRadiusFor === formula) return streamRadius;
    streamRadiusFor = formula;
    streamRadius = 0;
    try {
      const de = getCpuDE();
      if (de) {
        let rMax = 0;
        const k3 = 0.5773502691896258;
        const dirs = [
          [1, 0, 0],
          [-1, 0, 0],
          [0, 1, 0],
        ...
        for (const dir of dirs) {
          let t = 0;
          for (let i = 0; i < 256 && t < 63.5; i++) {
            const r = 64 - t;
            const d = de(dir[0] * r, dir[1] * r, dir[2] * r);
            if (!(d < 1e9)) break;
            if (d < Math.max(0.005, 0.002 * r)) {
              if (r > rMax) rMax = r;
              break;
            }
        ...
        if (rMax > 0.05) streamRadius = rMax;
      }
    } catch {
      /* scale probe is best-effort; the unit-scale default holds */
    }
    return streamRadius;
  }
  // The formula's own world radius, camera-free — what an OFFLINE export pins
  // the sim world to (see setOffline): a value derived from the live camera
  // would make the same flight render differently depending on where the viewer
  // happened to be standing when they pressed Export.
  function streamObjScale() {
    const r = streamProbeRadius();
    return r > 0.05 ? Math.max(0.2, Math.min(48, r / 1.3)) : 1;
  }
  // The cheap half — pure arithmetic over the cached radius + the live camera,
  // with a change guard so a settled view doesn't re-push a uniform every frame.
  // Only the world multiplier is ever written: every other knob is unit-scale
  // and the shader applies wscale itself, so an explicitly tuned value (UI,
  // probe) can never be clobbered by the auto probe.
  function streamRefocus() {
    if (!streamOn || !renderer.setStreamlines) return;
    const rObj = streamRadius > 0.05 ? streamRadius : 1.3;
    // Frustum half-height at the orbit target — the residual origin, and so the
    // centre of the spawn ball. Under ortho that half-height IS the zoom.
    const viewR =
      orthoH > 0 ? orthoH : Math.max(1e-12, cam.dist * Math.tan(cam.fov / 2));
    const want = Math.min(rObj, viewR * STREAM_VIEW_K);
    const s = Math.max(1e-4, Math.min(48, want / 1.3));
    // 2% deadband: a glide/spin nudges cam.dist every frame and the flow cannot
    // see a change that small anyway.
    if (streamLastScale && Math.abs(s - streamLastScale) <= streamLastScale * 0.02)
      return;
    streamLastScale = s;
    renderer.setStreamlines({ wscale: s });
  }
  // Kept as the name every caller already uses: probe the radius (formula
  // switch / first enable), then size the world to it and the camera.
  function streamRescale() {
    if (!streamOn || !renderer.setStreamlines) return;
    streamProbeRadius();
    streamLastScale = 0; // a new formula always re-pushes
    streamRefocus();
  }
  function streamTick() {
    streamRaf = 0;
    if (!streamOn || !hasGPU || deviceLost) return;
    if (!offline && !busy && !scheduled) {
      try {
        streamRefocus(); // follow a zoom that ended while the pump was idle
        renderer.tickStreamlines?.();
      } catch {
        /* a lab overlay must never break the live pump */
      }
    }
  ...
  function frameFeatures(f, over) {
    ...
    return frameFeaturesFor(f, coloring, {
      ...
      // IMGTEX (#631): mirror renderer.writeGlobals' latch (texture present
      // AND amount > 0 — amount defaults MUST match frameparams.js) so the
      // pump prewarms the texture variant the draw will select. A mismatch
      // only costs a sync compile, never a wrong render.
      envMap:
        !!renderer.hasImgTex?.("env") &&
        (coloring.light?.envMapAmount ?? 1) > 0,
      surfTex:
        !!renderer.hasImgTex?.("surface") &&
        (coloring.light?.surfTexAmount ?? 1) > 0,
      // TINY PLANET — mirror writeGlobals' latch so the pump prewarms (and
      // holds for) the variant the draw will select. Without this the toggle
      // would draw one frame through whatever variant happened to be warm.
      planet: planetK() > 0,
      // 360° EQUIRECT — same prewarm mirror, same reason.
      equirect: equirectOn,
    });
  }
  ...
  // `col` (optional): render THIS frame with an explicit coloring instead of
  // the live one — the thumbnail path's per-tile look. Passed through, never
  // assigned to the module `coloring`: the old temp-swap-and-restore around
  // renderThumbTile's await was a data race — two concurrent tile renders
  // (un-baked ★ Mine/◷ Today strips; the baked-404 renderLive fallback is
  // fire-and-forget per tile) interleaved so the SECOND tile's "previous"
  // snapshot was the FIRST tile's temp, and its guarded restore then parked a
  // TILE'S look as the session coloring — the owner's "same link, sometimes
  // blue sometimes green" (reproduced 2026-08-31: the Vast link's authored
  // blue colA/colB rendered green, byte-stable for the whole session; the
  // deep link's green palette rendered blue).
  function writeFrame(f, q, res, c, col) {
    ...
    settleFrame(renderer, f, col || coloring, q, res, c || cam, {
      orthoH, // #441 — 0 = perspective (the byte-identical path)
      planetK: planetK(), // TINY PLANET — 0 = off (the byte-identical path)
      equirectS: equirectS(res), // 360° EQUIRECT — 0 = off (same contract)
      morph,
      colorBlend,
  ...
  // LIVE-only heavy-band refinement (owner repro 3, "Vast Cube" — the v1.4
  // "not sharpening" follow-up). accumCap()'s hard OFF at ≥100 ms was tuned
  // when >100 ms meant a monster scene; a hinge scene at modest zoom measures
  // 100–250 ms settled on an M-series and its authored look then NEVER
  // converges even on an idle machine — permanent silhouette jaggies, the
  // exact "not sharpening" feel. The band refines those scenes anyway, but
  // under three guards that keep the "everything got sluggish" feedback (the
  // reason the budget exists) satisfied:
  //   • few samples (4 — AA converges visibly, then the GPU goes idle),
  //   • TIME-paced at ≤⅓ duty (accumGapUntil below: a 2×lastFullMs idle gap
  //     after each tick, so the compositor and a new drag always have slots —
  //     the rAF-parity pacing alone gave a ~95% duty at these frame costs),
  //   • lens-CENTERED ticks (pure AA, no DOF jitter — the same sample kind
  //     the heavy-export path bakes, so a saved still still matches the live
  //     look in kind; a 4-sample DOF would just be grain).
  //   • BANDED ticks (owner repro 4): when the settle itself needed banding
  //     (settleBands() > 1), each refine sample marches in the SAME scissored
  //     ~60 ms bands with a rAF between and resolves via
  //     drawAccum({skipMarch:true}) — pixel-identical per sample (the
  //     accumulateStill/export path has always run exactly this shape), and
  //     the compositor never sees a submit longer than one band.
  // Ceiling: the first cut of this band stopped at 400 ms because a tick was
  // ONE unbanded submit (a compositor stall at that cost). That left a hole
  // the fourth field report landed in: an iPad-class settle of the owner's
  // own scene measures ~530 ms (M4 at DSF2 measures the same) → refinement
  // permanently OFF → the live view stays a SINGLE full-res sample — the
  // "fuzzy + speckled, greenish at palette transitions" look (1-sample
  // aliasing), which the 24-sample PNG export then clears on the same device
  // (stillBlob accumulates on the live canvas — the owner's "when I was
  // exporting, it cleared up"). With banded ticks the stall objection is
  // gone, so the ceiling now sits just above SETTLE_MS_BUDGET: any settle
  // the resolution policy allows to exist is also allowed to converge.
  // On coarse-pointer devices the #476 governor still shrinks sustained
  // over-budget settles by scale; they refine at the capped scale first and
  // the idle-release (queueIdleUpgrade) then re-settles + re-converges at
  // full scale once the screen is truly idle.
  // Exports/stills deliberately keep reading accumCap() (bakeDOF, sample
  // counts) — their policy and latency are pinned and unchanged.
  // 1600: SETTLE_MS_BUDGET (1200) + prediction-error headroom — the owner's
  // iPad full-scale settles MEASURE 1.36-1.5 s (2026-08-31 field diag, both
  // sessions) on frames the budget PREDICTED under 1200; the ceiling must
  // admit what the budget lets through, or the idle-released full-scale
  // frame sits one sample forever (aliased) while its governed half refined.
  const ACCUM_HEAVY_MS = 1600;
  function liveAccumCap() {
    const c = accumCap();
    if (c > 0) return c;
    return lastFullMs < ACCUM_HEAVY_MS ? 4 : 0;
  }
  ...
  let accumGapUntil = 0; // heavy-band time pacing (see liveAccumCap) — 0 for light scenes
  // Accumulation-lifecycle diagnostics (the "not sharpening" field class):
  // seeds = settled base frames that armed refinement; draws = refine ticks
  // actually submitted. A healthy idle window seeds ONCE and draws cap−1
  // ticks; seeds growing while idle = a re-settle loop resetting the average
  // (the sharpen probe asserts on these — renderInfo below).
  let accumSeeds = 0;
  let accumDraws = 0;
  ...
  // ── Governor idle-release (#476 follow-up; owner field diag 2026-08-31) ──
  // The governor's jurisdiction is INTERACTION: it exists so sustained motion
  // never grinds a passively-cooled GPU at fatal cost. But it capped the IDLE
  // settle too, and — because only settles feed it — the cap never lifted
  // while the user sat still: the owner's iPad converged its accumulation at
  // scale 0.47 into a 2× box, a ~4× linear upscale of converged mush
  // ("render: … scale=0.47 gov=0.4707(active) accum=4/4"). Worse, the one
  // full-scale settle that DID run (1360 ms, banded, harmless) was fed to the
  // governor, which read the quality frame as a perf violation and cut the
  // scale — the loop that makes sharp impossible.
  // Split the jurisdictions: once the governed view has settled AND its
  // refinement has converged (a truly idle screen), ONE ungoverned settle
  // re-renders at the scale the ungoverned policy would pick (full, unless
  // settleScaleCap/SETTLE_MS_BUDGET cap it) — ALWAYS banded (~60 ms submits,
  // rAF between: the compositor stays alive; queueIdleUpgrade requires
  // drawMarchBand, so a tier whose fence is a synchronous gl.finish never
  // takes this path) — and accumulation then converges at the scale the user
  // actually sees. That settle is EXEMPT from observeSettleMs (quality time,
  // not a perf signal; feeding it recreates the loop above). Any interaction
  // renders governed exactly as before — the armed flag is consumed by
  // whatever frame draws next and re-arms only after the next convergence.
  let upgradeArmed = false; // next settled draw may exceed governorScale
  let upgradeTimer = 0;
  // Short: the queue points (below) only fire when the pump is ALREADY
  // quiescent — the settle itself proved a real pause (settleTimer 200 ms +
  // render), so a long extra grace here just reads as "it stops, then
  // sharpens" (owner polish round). The guard this delay buys is rhythmic
  // orbit-pause-orbit input, and a too-eager upgrade there costs at most one
  // ~60 ms band before the abandon check fires.
  const IDLE_UPGRADE_DELAY_MS = 120;
  function ungovernedScaleTarget() {
    // The scale the settle-cap block would pick with governorScale = 1:
    // min(1, settleScaleCap, budgetScale) — same fullResMs estimate as the
    // coarse-first upgrade block (perPx is scale-invariant).
    let budgetScale = 1;
    if (lastRender && lastRender.scale > 0 && lastFullMs > 0) {
      const fullResMs = lastFullMs / (lastRender.scale * lastRender.scale);
      if (fullResMs > SETTLE_MS_BUDGET)
        budgetScale = Math.sqrt(SETTLE_MS_BUDGET / fullResMs);
    }
    return Math.min(1, settleScaleCap, budgetScale);
  }
  // TRUE when the settled frame on screen is governed BELOW the ungoverned
  // target and the upgrade path CAN run — i.e. an idle re-settle at higher
  // scale will follow. The settle bookkeeping consults this too: refining AT
  // THE CAPPED SCALE first would be discarded work (the upgrade re-seeds the
  // average from scratch at the scale the user actually sees), and the owner
  // read that extra stage as "converges, then stops, then sharpens in one
  // step". Skipping it streamlines to settle → upgrade → refine ONCE, at
  // full scale. Tiers that can't upgrade (no drawMarchBand — the WebGL2
  // fallback has no accumulation at all anyway) keep today's behavior.
  function idleUpgradePossible() {
    if (!governorActive || governorScale >= 1) return false;
    if (!renderer?.drawMarchBand) return false; // banded settles only (see above)
    if (offline || deviceLost || !hasGPU) return false;
    const s =
      lastRender && lastRender.tier === "full" && !lastRender.cheap
        ? lastRender.scale
        : 0;
    return s > 0 && s < ungovernedScaleTarget() - 0.01;
  }
  function queueIdleUpgrade() {
    if (upgradeArmed || upgradeTimer) return;
    if (!idleUpgradePossible()) return; // nothing to upgrade (or already there)
    upgradeTimer = setTimeout(() => {
      upgradeTimer = 0;
      // Still truly idle? Any motion/invalidation waives this round — the
      // next convergence re-queues.
      if (
        busy ||
        needsDraw ||
        offline ||
  ...
  async function pump() {
    ...
    // Field streamlines: the sim world follows the camera at depth, and while a
    // drag/glide/cruise owns the frames the idle ticker (which normally does
    // this) has stood down — so re-size here too. Guarded + deadbanded inside;
    // with the overlay off it is one boolean.
    if (streamOn) streamRefocus();
    ...
    // the sample cap isn't reached. liveAccumCap, not accumCap: the LIVE view
    // also refines heavy-band scenes (100–400 ms) that exports leave alone.
    if (!needsDraw && accumN >= 1 && accumN < liveAccumCap()) {
      ...
      // 40–80 ms frames. Heavy-band scenes additionally pace by TIME
      // (accumGapUntil — a 2×lastFullMs idle gap after each tick): at
      // 100–400 ms per tick the rAF-parity gap alone is a rounding error.
      accumTick++;
      if (
        performance.now() < accumStartT ||
        performance.now() < accumGapUntil ||
        (accumTick & 1) === 1
      ) {
      ...
      busy = true;
      const heavyBand = accumCap() === 0; // refining via the band extension only
      ...
      // Heavy band: lens stays CENTERED — pure AA, the same sample kind the
      // heavy-export path bakes (see liveAccumCap), so live and saved stills
      // keep matching in kind.
      const [lx, ly] = heavyBand ? [0, 0] : lensSample(accumN);
      renderer.writeJitter(jx, jy, 1 / (accumN + 1), lx, ly);
      try {
        // Banded refine tick (owner repro 4 — see liveAccumCap): when the
        // settle itself needed banding, ONE unbanded accum march would be a
        // 100 ms+ compositor stall (#212 class) — the very reason the first
        // cut hard-capped the band at 400 ms and left heavier scenes
        // permanently unrefined. March this sample in the same scissored
        // bands instead (rAF between; abandon on any invalidation — the
        // half-marched HDR is garbage the moment anything changes), then
        // resolve with drawAccum({skipMarch:true}) — pixel-identical per
        // sample: the export path (accumulateStill) has always banded its
        // samples exactly like this. Light scenes keep the single-submit
        // tick unchanged.
        const tickBands = settleBands();
        let tickAborted = false;
        for (let i = 0; i < tickBands && tickBands > 1; i++) {
          if (i) await new Promise((r) => requestAnimationFrame(r));
          if (needsDraw || quality !== "full") {
            tickAborted = true; // gesture/setter mid-tick — abandon the sample
            break;
          }
          renderer.drawMarchBand(i, tickBands);
          if (!(await fencedSettle("accum-band"))) {
            tickAborted = true; // hung band fence (#460) — recovery rescheduled
            break;
          }
        }
        if (tickAborted) {
          // Abandoned sample: don't blend the partial HDR, don't count it.
          // The pending invalidation redraws a base frame that re-seeds the
          // average (weight 1), so no stale-ghost state survives.
          accumN = -1;
        } else {
          renderer.drawAccum(tickBands > 1 ? { skipMarch: true } : undefined);
          accumDraws++; // lifecycle diagnostics — refine tick submitted
          firePresent(); // refined frame presented — before the fence await
          // Fenced, but watchdog-safe (#460): a hung refine fence would
          // otherwise never reach `busy = false` below (it's outside the try,
          // past the await) and wedge the pump. On timeout, stop
          // accumulating; the recovery reschedules the base frame.
          if (await fencedSettle("accum")) {
            accumN++;
            if (heavyBand)
              accumGapUntil = performance.now() + 2 * lastFullMs; // ≤⅓ duty
          } else accumN = -1;
        }
      } catch (e) {
      ...
      if ((needsDraw || (accumN >= 1 && accumN < liveAccumCap())) && !scheduled) {
        scheduled = true;
        requestAnimationFrame(pump);
      } else if (!needsDraw && !scheduled) {
        // Refinement converged (or died) and nothing else wants a frame — the
        // screen is going truly idle: the governed view may now earn its
        // ungoverned settle (no-op unless the governor holds scale < 1).
        queueIdleUpgrade();
      }
      return;
    }
    ...
    let upgradeFrame = false; // governor idle-release — set once q is known
    ...
    try {
      ...
      // Governor idle-release (see queueIdleUpgrade): a one-shot ARMED settled
      // frame renders at the UNGOVERNED scale — quality time on a truly idle
      // screen, banded so no submit exceeds ~60 ms. Interactive/cheap frames
      // never qualify (quality/cheap guard), so motion is governed exactly as
      // before. The flag is consumed by this draw attempt (cleared in the
      // finally) whatever tier it turns out to be.
      upgradeFrame = upgradeArmed && quality === "full" && !q.cheap;
      ...
      if (governorScale < 1 && !upgradeFrame)
      ...
      if (quality === "full" && !q.cheap) {
        ...
        // Idle-release: an armed upgrade settle drops ONLY the governor term
        // (govTerm 1) — settleScaleCap and the settle budget still apply, so a
        // watchdog-degraded or genuinely over-budget scene never upgrades past
        // what the ungoverned policy would render.
        const govTerm = upgradeFrame ? 1 : governorScale;
        const over = fullMs > SETTLE_MS_BUDGET;
        if (over || settleScaleCap < 1 || govTerm < 1) {
          const floor = Math.min(SETTLE_SCALE_FLOOR, settleScaleCap, govTerm);
          const budgetScale = over ? Math.sqrt(SETTLE_MS_BUDGET / fullMs) : 1;
          q.scale = Math.max(
            floor,
            Math.min(q.scale, budgetScale, settleScaleCap, govTerm),
          );
        }
      }
      ...
      if (bands > 1) {
        ...
        for (let i = 0; i < bands; i++) {
          if (i) await new Promise((r) => requestAnimationFrame(r));
          // A gesture flips quality to "low" — abandon (all settles, as
          // always). An idle-release UPGRADE additionally abandons on ANY
          // invalidation (needsDraw — coloring change, resize, downshift):
          // it is pure quality time with nobody waiting on it, and at 1 s+
          // total it would otherwise hold the change hostage for its whole
          // band walk. Ordinary settles keep finishing under plain
          // needsDraw as before — aborting those on every invalidation is
          // a churn risk this stabilization round doesn't take.
          if (quality !== "full" || (upgradeFrame && needsDraw)) {
            aborted = true;
            break;
          }
        ...
        if (aborted) {
          ...
          firePresent(); // banded composite presented — before the fence await
          ...
          if (await fencedSettle("band-composite")) {
            ...
            // #476 governor: honest banded-settle GPU time — but NOT from an
            // idle-release upgrade settle (quality time, not a perf signal:
            // feeding it would read the deliberate full-scale frame as a
            // violation and re-cut the scale — the owner's field loop).
            if (!upgradeFrame) observeSettleMs(bandMs);
            // Don't seed refinement when the idle upgrade is about to redo
            // this frame at a higher scale (idleUpgradePossible — evaluated
            // AFTER observeSettleMs so a downshift on this very measurement
            // counts): capped-scale samples would be thrown away, and the
            // wasted stage is the owner's "converges, stops, then sharpens".
            accumN = wantAccum && !idleUpgradePossible() ? 1 : -1;
            if (accumN === 1) accumSeeds++; // lifecycle diagnostics
            accumStartT = performance.now() + 250;
            accumGapUntil = 0; // fresh settle — heavy-band pacing restarts clean
          } else {
      ...
      if (bands <= 1) {
        ...
        // base frame (#460) — the recovery will redraw a coarser one. And a
        // governed settle the idle upgrade is about to redo at higher scale
        // doesn't seed either (see the banded twin above; lastFullMs here is
        // one settle stale, which at most mis-skips a seed the upgrade's own
        // re-seed replaces anyway).
        accumN =
          !settleHung && wantAccum && !idleUpgradePossible() ? 1 : -1;
        if (accumN === 1) accumSeeds++; // lifecycle diagnostics
        accumStartT = performance.now() + 250;
        accumGapUntil = 0; // fresh settle — heavy-band pacing restarts clean
      }
      ...
      busy = false;
      // Idle-release: the armed flag is one-shot — consumed by this draw
      // attempt whatever happened (upgrade landed, aborted, or the frame
      // turned out interactive). The next convergence re-queues.
      upgradeArmed = false;
    }
    ...
    if (tFull && !settleHung) {
      ...
      lastFullWork = lastRender
        ...
        : 0;
      measuredFor = formula;
      // #476 governor — but never from an idle-release upgrade settle (see
      // the banded twin above: quality time is not a perf signal).
      if (!upgradeFrame) observeSettleMs(dt);
    }
    ...
    if (
      ...
      (accumN >= 1 && accumN < liveAccumCap())
    ) {
      scheduled = true;
      requestAnimationFrame(pump);
    } else if (!scheduled) {
      // Nothing left to pump — a settle with no refinement to run (e.g. cap 0
      // at the governed scale) still earns the idle-release upgrade here.
      queueIdleUpgrade();
    }
  }
  ...
  // ── TINY PLANET framing ───────────────────────────────────────────────────
  // Measured, not guessed (app/scripts/planet-probe.mjs, real WebGPU frames).
  //
  // The projection ALONE cannot make a planet, and this is the arithmetic that
  // says so: under the map a subject of angular radius α fills tan(α/2)/k of
  // the frame's half-height. At a preset's own framing the Mandelbulb subtends
  // about 12°, so every real wrap leaves a marble in an empty sky — the probe
  // measured disc 0.257 at 90°, 0.110 at 180°, 0.026 at 300°, with the whole
  // border sky at every step. Correct rays, wrong picture.
  //
  // A little planet needs α near 60–90°, i.e. the ground filling the hemisphere
  // you are pointing at. That is a CAMERA fact — no ray-gen substitutes for it
  // — so switching the projection on flies the eye down the centre ray to just
  // above the surface, reusing the same zoom-to-surface probe the wheel already
  // marches (§5 navigation). Nothing else about the camera is touched: the
  // orbit target, the angles and the FOV are exactly where they were.
  //
  // It does NOT rewind on switch-off. The camera is the user's, and silently
  // undoing whatever they navigated to WHILE in the mode would be worse than
  // leaving them where they are — the double-click re-pin sets the same
  // precedent (it moves you and does not offer to move you back).
  // The angular radius we are aiming the subject at. 55° is measured, not
  // chosen: below ~45° the sky starts to dominate and the subject reads as a
  // marble; above ~65° the wrap runs off the frame edge and the sky closes
  // (probe: Menger at disc 1.000 / edgeSky 0.115 when an earlier, cruder rule
  // dived too far). 55° lands both the ball-shaped and the lattice presets in
  // the band where the border is all sky and the world still fills the middle.
  const PLANET_TARGET_ALPHA = (55 * Math.PI) / 180;
  const PLANET_ALPHA_MAX = (80 * Math.PI) / 180; // past this, "radius" is meaningless

  // How big is the subject, in ANGLE, from where we stand? This is the quantity
  // the projection actually cares about — disc = tan(alpha/2)/k — and it is not
  // the same thing as "distance to the surface straight ahead". A Menger
  // preset sits 8 units off its nearest on-axis face and STILL fills 65° of
  // sky, because the lattice wraps around the camera; a Mandelbulb at the same
  // relative distance subtends 12°. A rule written on the centre distance
  // cannot tell those apart, and the first one did not: it dived the Menger
  // from 9.0 to 1.3 and buried the camera in it.
  //
  // So: bisect for the widest off-axis angle whose ray still finds a surface.
  // Two azimuths (right and up) rather than one, because a lattice can be open
  // along one axis and solid along the other. Up to ~16 CPU-DE marches, ONCE,
  // on a toggle — the wheel already spends one per 150 ms while you scroll.
  const planetAngularRadius = (b) => {
    const P = probeFrame(b);
    const near = cam.dist * TNEAR_K;
    const far = cam.dist * (TFAR_K * 1.5);
    const hitsAt = (theta) => {
      const c = Math.cos(theta);
      const s2 = Math.sin(theta);
      for (const ax of [b.right, b.up]) {
        const d = [
          c * b.fwd[0] + s2 * ax[0],
          c * b.fwd[1] + s2 * ax[1],
          c * b.fwd[2] + s2 * ax[2],
        ];
        if (surfaceHitDist(P.de, P.o, d, near, far) != null) return true;
      }
      return false;
    };
    if (!hitsAt(0)) return 0; // looking at empty space — nothing to stand on
    if (hitsAt(PLANET_ALPHA_MAX)) return PLANET_ALPHA_MAX;
    let lo = 0;
    let hi = PLANET_ALPHA_MAX;
    for (let i = 0; i < 6; i++) {
      const m = 0.5 * (lo + hi);
      if (hitsAt(m)) lo = m;
      else hi = m;
  ...
  const dropToPlanetSurface = () => {
    const b = cam.basis();
    const alpha = planetAngularRadius(b);
    // Nothing framed (empty space ahead), or no CPU DE for this formula shape:
    // DECLINE and leave the camera alone. The projection still turns on — it is
    // then a wide fisheye of whatever is there, which is an honest result.
    if (!(alpha > 0)) return false;
    // Already standing on it (every lattice preset, and anything the user has
    // already zoomed into): moving would only bury the camera. This is the
    // guard that makes the assist safe to run on EVERY switch-on.
    if (alpha >= PLANET_TARGET_ALPHA) return false;
    // Small-object geometry: the subject's effective radius is dist·sin(alpha),
    // so the distance that would present it at the target angle is
    // dist·sin(alpha)/sin(target). Exact for a ball, close enough for the rest
    // — and self-limiting, since alpha >= target already returned above.
    let want = (cam.dist * Math.sin(alpha)) / Math.sin(PLANET_TARGET_ALPHA);
    // …but never THROUGH the surface straight ahead. surfaceAhead is the same
    // probe the wheel anchor uses; leaving 5% of it keeps the eye outside.
    const h = surfaceAhead(b);
    if (h > 0) want = Math.max(want, cam.dist - h * 0.95);
    want = Math.max(ptMinDist(), want);
    if (!(want < cam.dist * 0.999)) return false;
    cam.dist = want;
    return true;
  };
  ...
  const pixelRay = (clientX, clientY) => {
    ...
    // TINY PLANET — the fourth copy of the stereographic map (WGSL, GLSL, CPU
    // are the others). This MUST exist for the same reason #441's ortho branch
    // does: double-click repin and the cursor-anchored wheel zoom build their
    // ray here, so leaving it perspective would silently probe a different
    // pixel than the one the user clicked — the wrong feature, no error.
    // Single origin (the eye), so roOfs stays zero.
    // 360° EQUIRECT — the same obligation, one arm earlier (the three
    // projections are exclusive, so order is cosmetic; equirect leads to keep
    // the wide maps together). lon = ndcX·π because the map reads
    // lon = wx·(π/aspect) with wx = ndcX·aspect — the aspect cancels here,
    // exactly as it does for the GL tier's constant.
    if (equirectOn) {
      const lon = ndcX * Math.PI,
        lat = (ndcY * Math.PI) / 2;
      const cl = Math.cos(lat),
        sl = Math.sin(lat);
      const a = cl * Math.sin(lon),
        f = cl * Math.cos(lon);
      const r = [
        a * b.right[0] + sl * b.up[0] + f * b.fwd[0],
        a * b.right[1] + sl * b.up[1] + f * b.fwd[1],
        a * b.right[2] + sl * b.up[2] + f * b.fwd[2],
      ];
      const rl = Math.hypot(r[0], r[1], r[2]) || 1;
      return {
        b,
        ray: [r[0] / rl, r[1] / rl, r[2] / rl],
        ro: b.eye.slice(),
        roOfs: [0, 0, 0],
      };
    }
    if (planetFov > 0) {
      const k = planetK();
      const pu = ndcX * aspect * k,
        pv = ndcY * k;
      const pq = pu * pu + pv * pv;
      const pInv = 1 / (1 + pq);
      const a = 2 * pu * pInv,
        c = 2 * pv * pInv,
        f = (1 - pq) * pInv;
      const r = [
        a * b.right[0] + c * b.up[0] + f * b.fwd[0],
        a * b.right[1] + c * b.up[1] + f * b.fwd[1],
        a * b.right[2] + c * b.up[2] + f * b.fwd[2],
      ];
      const rl = Math.hypot(r[0], r[1], r[2]) || 1;
      return {
        b,
  ...
  canvas.addEventListener("pointermove", (e) => {
    ...
    // TINY PLANET is deliberately NOT dropped here — the one place it parts
    // company with ortho. Ortho is a snap-to-inspect mode you should never get
    // stuck in; the planet is a LOOK, and orbiting is how you spin it. The
    // pole rides camFwd, so a drag rotates the planet under you, which is the
    // gesture's whole point. (core/tinyplanet.test.mjs grep-gates this: the
    // orbit branch must clear orthoH and must NOT clear planetFov.)
    // 360° EQUIRECT is kept through an orbit for the same reason — the image
    // centre rides camFwd, so dragging IS looking around the panorama.
    // (core/equirect.test.mjs grep-gates it the same way.)
    const s =
  ...
  canvas.addEventListener(
    ...
    (e) => {
      ...
      // TINY PLANET — GATED for the same reason, one step further on. The
      // anchored path's whole premise is "slide the EYE down the cursor ray
      // and the point under the cursor does not move", which holds only
      // because a perspective image is a central projection FROM that eye.
      // Under the stereographic map a translation is not a projective map of
      // the image, so the anchor visibly slides — worst at the frame edge,
      // where the ray points sideways or backwards and the "zoom" would strafe
      // the camera instead. So: plain centre dolly, which under this
      // projection is exactly right (getting closer IS how the planet grows),
      // and skip the CPU-DE surface probe that would only be discarded.
      if (planetFov > 0) {
        wheelProbe = null;
        zoomAtCenter(factor);
        return;
      }
      // 360° EQUIRECT — same fork, sharper reason. "Zoom" has NO projection
      // meaning here at all: the frame is always the whole sphere (there is
      // no FOV to crop, deliberately — a cropped pano would no longer be the
      // 360 photo the mode exists to save). So the wheel maps to the one
      // thing that does mean something: a plain centre DOLLY through the
      // world, which moves the viewpoint the panorama is taken from. The
      // cursor-anchored premise fails here even harder than under the planet
      // (half the frame's rays point behind the eye), so the CPU-DE probe is
      // skipped identically.
      if (equirectOn) {
        wheelProbe = null;
        zoomAtCenter(factor);
  ...
  async function stillBlobTiled(opts = {}) {
    ...
    try {
      ...
      // 360° EQUIRECT — writeFrame sees the TILE's res, but the longitude
      // scale must be the PLATE's π·H/W (the tile-local aspect cannot recover
      // the full-image span — see equirectS). Parked for the whole export,
      // cleared in the finally below. A no-op while the projection is off.
      tiledFullDims = [W, H];
      writeFrame(
        formula,
        stillQualityParams(formula, (cam && cam.dist) || REF_DIST),
      ...
      tiledFullDims = null; // 360° EQUIRECT — back to res-derived scale
      canvas.width = prevW;
      canvas.height = prevH;
      busy = false;
      scheduleDraw();
  ...
  async function captureFrame(opts = {}) {
    ...
    if (renderer.renderToImage) {
      ...
      // opaque canvas.
      //
      // Field streamlines (lab): offline frames INHERIT the live toggle — if
      // the overlay is on screen it rides the export, matching stillBlob's
      // WYSIWYG (no extra export UI). The sim advances on the export's
      // VIRTUAL clock: opts.streamDt seconds per captured frame (default
      // 1/30 — the flythrough/GIF frame step), fixed and deterministic, not
      // wall time. Alpha exports stay clean (renderer-side guard too).
      const img = await renderer.renderToImage(
        W,
        H,
        samples,
        !!opts.alpha,
        true, // bakeDOF — the existing-caller default, unchanged
  ...
  function setOffline(b) {
    const was = offline;
    offline = !!b;
    // Field streamlines (lab): an offline export SESSION is exactly this
    // bracket, so it is where the overlay's deterministic-export contract lives
    // — arm on the way in (reseed + pre-roll happen on the first captured
    // frame), hand the live flow back on the way out.
    if (streamOn && offline !== was) {
      try {
        if (offline) {
          // Pin the sim world to the formula's OWN scale for the export's
          // duration. The live value follows the camera (deep-zoom spawning),
          // and a camera-derived world would make the same flight render
          // differently depending on where the viewer was standing when they
          // pressed Export — the one thing determinism must exclude. The cost
          // is that a deep-zoom flight exports at object scale; the overlay is
          // a lab toy and that is the honest trade.
          renderer.setStreamlines?.({ wscale: streamObjScale() });
          renderer.beginStreamOffline?.(streamSeedKey());
        } else {
          renderer.endStreamOffline?.();
          streamLastScale = 0; // re-push the live camera-aware world
          streamRefocus();
        }
      } catch {
        /* a lab overlay must never break an export */
      }
    }
    if (!offline) scheduleDraw();
  }

  // Read-only offline probe — lets an app-side live animator (the
  // Cross-section "Sweep with Spin" ticker) yield while an offline export
  // owns the canvas: a concurrent setColoring between the export's staging
  // and captureFrame would corrupt that frame. Appended to the API (no
  // existing caller changes shape).
  function isOffline() {
    return offline;
  }

  // The export session key: everything about THIS picture that the particle
  // seed should depend on. Stable string, no clock, no object identity — the
  // same formula loaded twice from a share link must hash the same.
  function streamSeedKey() {
    const f = formula || {};
    const ops = (f.ops || [])
      .map((o) => `${o.key}:${(o.values || []).join(",")}`)
      .join("|");
    return `${f.name || ""}#${f.iters || 0}#${ops}`;
  }
  ...
  async function renderThumbTile(p, W, H, look) {
    ...
    // Per-tile coloring override (curated preset looks): built HERE and passed
    // straight through writeFrame's `col` parameter — NEVER assigned to the
    // module `coloring`. The old swap-and-restore around the await below was a
    // data race that could park a tile's look as the session coloring (see
    // writeFrame's header — the "same link, sometimes blue sometimes green"
    // field report). Palette cycling is forced off first so the look's flat
    // colors are what the tile shows — unless the look itself carries a
    // palette (a saved formula's colors).
    const temp = look
      ? {
          ...coloring,
      ...
      : null;
    {
      ...
      writeFrame(
        ...
        [W, H],
        tileCam,
        temp, // per-tile look — threaded, not swapped (see above)
      );
  ...
  return {
    ...
    setOrtho(h) {
      ...
      if (v === orthoH) return;
      orthoH = v;
      if (v > 0) planetFov = 0; // one projection at a time (see setPlanet)
      if (v > 0) equirectOn = false; // …the exclusion is three-way now
      scheduleDraw();
    },
    isOrtho: () => orthoH > 0,
    // TINY PLANET — set the stereographic field of view in DEGREES; 0 (or any
    // non-finite/≤0 value) turns it off and restores the frustum. Clamped to
    // PLANET_FOV_MIN..MAX: 360 is the true degenerate (tan(90°) = ∞, the
    // antipode never reaches the frame), so the top of the range stops short.
    //
    // MUTUAL EXCLUSION with #441 ortho is enforced HERE, in the setters, not
    // in the UI — they are two answers to "how does a pixel become a ray" and
    // only one can win. That also means tesselava's Top/Front/Side buttons
    // clear the planet for free, without knowing it exists.
    // `frame` (the app passes it on SWITCH-ON only, never on a slider drag)
    // additionally flies the eye down to the surface — see the framing note at
    // dropToPlanetSurface for why the projection needs that to read as a
    // planet at all, and why it is one-way.
    setPlanet(fovDeg, { frame = false } = {}) {
      const d = Number.isFinite(fovDeg) ? fovDeg : 0;
      const v =
        d > 0
          ? (Math.min(PLANET_FOV_MAX, Math.max(PLANET_FOV_MIN, d)) * Math.PI) /
            180
          : 0;
      if (v === planetFov) return;
      planetFov = v;
      if (v > 0) {
        orthoH = 0; // one projection at a time
        equirectOn = false; // …three-way (see setEquirect)
        if (frame) dropToPlanetSurface();
      }
      scheduleDraw();
    },
    isPlanet: () => planetFov > 0,
    // Degrees, 0 when off — the inverse of setPlanet, for the UI readout.
    getPlanet: () => (planetFov > 0 ? (planetFov * 180) / Math.PI : 0),
    // 360° EQUIRECT — switch the full 360×180 lat-long projection on/off. A
    // boolean, deliberately: the full sphere IS the feature (a saved 2:1
    // frame is a 360 photo), so there is no angle to set and nothing to
    // remember across an off/on cycle.
    //
    // The MUTUAL EXCLUSION is three-way and lives HERE in the setters (the
    // setOrtho/setPlanet note): each of the three clears the other two, so
    // any caller — the View pane, tesselava's Top/Front/Side, the probe —
    // gets one live projection without knowing the others exist.
    //
    // `frame` (the app passes it on SWITCH-ON only) reuses the tiny planet's
    // dropToPlanetSurface: the IDEAS carry-forward note predicted every
    // wide-field mode meets the same wall ("a correct map still renders a
    // marble in an empty sky"), and it does — a panorama taken from a
    // preset's framing distance is a small blob at image centre. The same
    // measured assist (present the subject at ~55°, decline when it already
    // fills the view or nothing is framed) is exactly what "stand where the
    // 360 photo is worth taking" means, so it is reused rather than re-tuned.
    setEquirect(on, { frame = false } = {}) {
      const v = !!on;
      if (v === equirectOn) return;
      equirectOn = v;
      if (v) {
        orthoH = 0; // one projection at a time
        planetFov = 0;
    ...
    setFormula(f) {
      ...
      // Field streamlines: the sim world is sized per formula (R3 — Mandelbox
      // renders ~7× larger than a bulb); re-probe on every switch while on.
      if (streamOn && isNew) streamRescale();
    ...
    // n: 0 off | 1 march step-count heat | 2 overshoot/bracket | 3 ∇DE
    // instability | 4 fold membrane (#633 — the seam sheets; the mode-4 branch
    // is codegen-gated on seam ops, so on a seam-free formula the shader has
    // no case for it and the overlay falls through to mode 3's ∇DE view).
    ...
    setDebugView(n) {
      debugView = Math.max(0, Math.min(4, Math.round(Number(n) || 0)));
      scheduleDraw();
    },
    ...
    // IMGTEX (#631) — upload/clear a user image: slot "env" (equirect
    // environment map) or "surface" (triplanar surface tile); img is an
    // ImageBitmap (decode-capped by the app — envmap.js fitImageDims) or null
    // to clear. WebGPU tier ONLY: on WebGL2 the feature joins the documented
    // reduced set (no accum AA / DOF / bloom — app/src/main.ts §tiled-export
    // gate) and renderer_gl has no setEnvImage, so this is a silent no-op
    // there, exactly like the CPU/ASCII tier. The image itself never rides a
    // share link or PNG metadata ("settings yes, pixels no" — #631); only the
    // sliders travel, so a shared look opens with the image slot empty and
    // the codegen latch off (byte-identical shader).
    setEnvImage(slot, img) {
      if (!renderer.setEnvImage) return; // GL/ASCII tier — feature off
      renderer.setEnvImage(slot, img);
      // A swapped image invalidates the settled/accumulated frame exactly like
      // a light-slider edit: re-enter the interactive tier, then re-settle.
      bumpInteract();
    },
    hasEnvImage: (slot) => !!renderer.hasImgTex?.(slot),
    // Field streamlines (lab — core/streamlines.js): luminous particles
    // advected along the current formula's DE gradient, composited over the
    // render. WebGPU tier ONLY (the accepted scope divergence — no WebGL2/CPU
    // mirror): returns false on the other tiers so the app can leave the
    // control dark. o = { on, count, intensity } (plus tuning knobs the
    // controller clamps). Off by default; zero cost until first enabled.
    setStreamlines(o) {
      if (!renderer.setStreamlines) return false; // GL/ASCII/headless tier
      if (!renderer.setStreamlines(o || {})) return false;
      streamOn = !!(o && o.on);
      if (streamOn && !streamRaf) streamRaf = requestAnimationFrame(streamTick);
      if (!streamOn && streamRaf) {
        cancelAnimationFrame(streamRaf);
        streamRaf = 0;
      }
      if (streamOn) streamRescale(); // size the sim world to THIS fractal
      // Toggling reconfigures the swap chain (COPY_SRC) — repaint so the
      // overlay (or its absence) lands immediately.
      scheduleDraw();
      return true;
    },
    // Streamlines pipeline states + captured failures (null off the WebGPU
    // tier / before first enable) — the probe and diag read this.
    streamlinesInfo: () => renderer.streamlinesInfo?.() ?? null,
    // Per-axis world extents of the CURRENT formula's rendered body
    // (measureWorldExtents above — the MRI sweep's amplitude source), or
    // null when the CPU DE can't measure it. Cached per formula object: the
    // scan is ~tens of ms, fine on an export click, never per frame.
    worldExtents() {
      const f = formula;
      if (!f) return null;
      let e = _worldExtCache.get(f);
      if (e === undefined) {
        e = measureWorldExtents(getCpuDE());
        _worldExtCache.set(f, e);
      }
      return e;
    },
    // Probe-only particle census (GPU readback) — the tuning probe's "where
    // did the particles go" instrument.
    streamlinesStats: async () =>
      renderer.streamlinesStats ? renderer.streamlinesStats() : null,
    setAutoRotate(b) {
      autoRotate = b;
    ...
    renderInfo: () => ({
      ...
      deviceLost,
      // P2 refinement lifecycle (the "not sharpening" field class): current
      // sample count (−1 idle), the cap the LIVE pump refines toward, and the
      // session seed/tick totals. Healthy idle: accumN pinned at the cap,
      // seeds stable; seeds growing while untouched = a re-settle loop is
      // resetting the average (never converges); draws never moving with
      // accumN ≥ 1 = refinement armed but starved. The sharpen probe asserts
      // convergence from these.
      accumN,
      accumCapNow: liveAccumCap(),
      accumSeeds,
      accumDraws,
      last: lastRender,
    }),
    ...
    cruising: () => !!cruise,
    // Runtime (re)binding of the live-present hook (see firePresent above) —
    // settable so a lazily-initialized consumer (e.g. a view overlay) can
    // attach without owning the createPreview options.
    setOnPresent(cb) {
      onPresentCb = cb || null;
    },
    captureFrame,
