...
export async function createRendererGL(canvas, opts = {}) {
  ...
  function writeOps(ops) {
    ...
    rebuildWith = (ex) => {
      linkProgram(buildFragGL(clipped, undefined, undefined, { envx: ex }), bulkLayout({}));
      progEnvx = ex;
    };
    // Structural changes rebuild with the CURRENT envx want (so a pending
    // downgrade rides along free); a pure envx upgrade is applyUniforms' job.
    if (sig !== opSig || !program) {
      rebuildWith(envxWant);
      opSig = sig;
    }
  ...
  function writeScene(objects) {
    ...
    const sig =
      ...
      norm
        ...
        .join("|");
    rebuildWith = (ex) => {
      linkProgram(buildSceneFragGL(objects, { envx: ex }), bulkLayout({ scene: true }));
      progEnvx = ex;
    };
    if (sig !== opSig || !program) {
      rebuildWith(envxWant);
  ...
  function applyUniforms(res) {
    ...
    envxWant = !!d.envx;
    if (envxWant && !progEnvx && rebuildWith) {
      rebuildWith(envxWant);
      gl.useProgram(program);
    }
