// PNG text-chunk read/write — pure, dependency-free (no build).
//
...
// short, UNCOMPRESSED text here, so this file has no zTXt / deflate path.
//
// That is a statement about text chunks, not about core: core/pngstream.js sits
// beside this file and deflates whole IDATs. Core gets deflate from the platform
// CompressionStream (precedent core/spzsplat.js), which is how it compresses
// without breaking the no-dependency invariant — so do not read the line above
// as licence to reach for `fflate` (TILED_EXPORT.md §2.4.3). The chunk
// serializer and crc32 below are shared with that encoder.
//
// Contract:
...
// The 8-byte file signature, for a writer that authors a PNG from scratch
// (pngstream.js) rather than splicing into one we already produced.
export const PNG_SIGNATURE = Object.freeze(PNG_SIG.slice());
...
// One serialized PNG chunk: length + type + data + CRC. Exposed because
// core/pngstream.js authors IHDR/IDAT/IEND and must not carry a second copy of
// the framing or the CRC (TILED_EXPORT.md §2.4.3).
export function pngChunk(type, data) {
  return chunkBytes(type, data);
}

// One serialized tEXt/iTXt chunk from the same {type, keyword, text, …} spec
// embedChunks() takes — so a streamed file and a spliced one carry byte-identical
// metadata, written by the same code.
export function pngTextChunk(spec) {
  return chunkBytes(spec.type, textChunkData(spec));
}
