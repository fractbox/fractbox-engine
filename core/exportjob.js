...
export function normalizeExportJob(raw) {
  ...
  // #543: raw.coloring used to ride straight through to the uniform writers —
  // the same untrusted-passthrough gap #542 closed for sanitizeScene, just
  // reachable here via the export-job JSON intake (tesselava / CLI --job)
  // instead of a share link. sanitizeColoring is SHAPE-PRESERVING (only keys
  // present on the input are emitted) and returns undefined for garbage, so a
  // job with an unusable coloring simply loads without one rather than
  // throwing — the job's formula/opts are still good.
  const coloring =
    raw.coloring != null ? sanitizeColoring(raw.coloring) : undefined;
