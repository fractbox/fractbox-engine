...
import {
  ...
  MAX_LEAF_PARAMS_INLINE,
} from "./limits.js";

// ── The op-param overflow lane (docs/planning/OP_PARAM_ENCODING.md §5.2) ─────
// `struct Op` stays 16 B forever. Params past MAX_OP_PARAMS_INLINE ride a
// parallel `opAux : array<vec4f>` storage buffer, addressed by the SAME index
// that reads `ops` — verified by the standing pin that all 17 op-read sites in
// this file are literally `let op = ops[o];` (core/shaderlint.test.mjs). No
// prefix sum, no base index in the op, no bits stolen from opType, and so NOT
// ONE of the 17 sites changes. The df64/pt switches inherit it for free: they
// wrap the same `op` from the same loop.
//
// Author-facing syntax is uniform — an op body writes `op.p3` exactly as it
// writes `op.p0`, and this splicer rewrites indices >= 3 on the way out. Moving
// a param across the inline/overflow boundary is then a no-op for the op author.
//
// EXPORTED because app/scripts/hybrid-nslot-verify.mjs hand-assembles its own
// op switch by splicing the same raw registry bodies into a compute kernel. It
// must perform the IDENTICAL rewrite, and sharing this function is the only way
// the two splicers cannot drift.
const AUX_SWIZZLE = ["x", "y", "z", "w"];
export const spliceAux = (body, key) =>
  String(body).replace(/\bop\.p(\d+)\b/g, (whole, n) => {
    const i = Number(n);
    if (i < MAX_OP_PARAMS_INLINE) return whole; // p0..p2 stay on the struct
    if (i >= MAX_OP_PARAMS)
      throw new Error(
        `operator "${key}" references ${whole} but the cap is ${MAX_OP_PARAMS} params (p0..p${MAX_OP_PARAMS - 1})`,
      );
    return `opAux[o].${AUX_SWIZZLE[i - MAX_OP_PARAMS_INLINE]}`;
  });

// Does this op-set need the lane? Mirrors the emission test EXACTLY so the
// renderer's bind-group entry can never disagree with the shader (an `auto`
// pipeline layout PRUNES a declared-but-unread storage binding, and binding one
// that isn't in the layout is a hard validation error — measured, not assumed).
// `ops` is buildWGSL's own option: an array of op ids, or null = full switch.
export const usesOpAux = (ops) => {
  const allow = ops ? new Set(ops) : null;
  return OPERATORS.some(
    (op) =>
      (!allow || allow.has(op.id)) && op.params.length > MAX_OP_PARAMS_INLINE,
  );
};

// #633 — does this op-set contain a seam-declaring op (operators.js `seam`
// flag: discontinuous map + tear-distance report)? Mirrors usesOpAux so the
// emission of the whole seam channel — the private accumulator, the clamped
// march step and the fold-membrane debug view — is keyed on the SAME test:
// structural pay-per-use (#125's lesson), not a uniform-gated dead branch. A
// seam-free variant emits not one seam token and stays byte-identical.
// `ops` is buildWGSL's own option: an array of op ids, or null = full switch.
export const usesSeam = (ops) => {
  const allow = ops ? new Set(ops) : null;
  return OPERATORS.some((op) => (!allow || allow.has(op.id)) && op.seam);
};

// The leaf mirror of usesOpAux (#627 — objAux lane): does this VARIANT's
// leaf-id set contain a leaf that declares more than the inline 4 params?
// Mirrors leafFnsWGSL's inclusion rule EXACTLY (true = all leaves, array =
// those ids, else none) so the renderer's bind-group entry can never disagree
// with the shader — the same auto-layout-prunes-unread-bindings trap opAux
// documents above.
export const usesObjAux = (leaves) => {
  const use =
    leaves === true
      ? LEAVES
      : Array.isArray(leaves)
        ...
        : [];
  return use.some((l) => (l.params?.length ?? 0) > MAX_LEAF_PARAMS_INLINE);
};
  ...
  const use =
    leaves === true
      ...
      : Array.isArray(leaves)
        ? LEAVES.filter((l) => leaves.includes(l.id))
        : [];
  // #627 — when the set contains a fat (>4-param) leaf, every leaf fn and the
  // dispatcher gain a second vec4 (prm2 = objAux lane). Thin-leaf BODIES are
  // untouched (they never read prm2); sets with no fat leaf emit the exact
  // legacy text, so existing scenes stay byte-identical.
  const fat = use.some((l) => (l.params?.length ?? 0) > MAX_LEAF_PARAMS_INLINE);
  const sig2 = fat ? ", prm2: vec4f" : "";
  const arg2 = fat ? ", prm2" : "";
  return (
    use
      .map(
        (l) => `fn leaf_${l.key}(p: vec3f, prm: vec4f${sig2}) -> f32 {
  ${l.wgsl}
}`,
      )
      .join("\n") +
    `
fn leafDist(id: u32, p: vec3f, prm: vec4f${sig2}) -> f32 {
${use.map((l, i) => `  ${i ? "else " : ""}if (id == ${l.id}u) { return leaf_${l.key}(p, prm${arg2}); }`).join("\n")}
...
export function buildWGSL({
  ...
  // directional environment. CODEGEN-gated, not uniform-gated: default false
  // emits byte-identical text (the #125 lesson at the top of this file — a
  // never-executing uniform branch still cost Mandelbulb +31%). The renderer
  // latches it per frame from the look (stars/band/zenith all zero → off) and
  // keys the variant on it, exactly like the numeric/leaves levers.
  envx = false,
  // Self-reflection (#630, Catoptron parity): marched mirror bounces. The
  // live fs is restructured into a bounce loop that re-marches reflect(rd,
  // nrm) from each hit, Fresnel-weighted, with the ONE shared shading body
  // serving every bounce (no second lighting path to drift — spec review 2a).
  // CODEGEN-gated exactly like envx: default false emits byte-identical text;
  // the renderer latches it from the look (reflBounces > 0) and keys the
  // variant on it. The bounce COUNT is a runtime word (lightC.w) INSIDE the
  // opt-in variant — one boolean feature bit, no 0–6 key-space blowup.
  sreflect = false,
  // IMGTEX (#631): user-image environment map (`envMap` — equirect texture at
  // binding 9 replacing the procedural sky inside envColor, so background,
  // IBL ambient and GGX reflections all see it) and triplanar surface texture
  // (`surfTex` — repeat-tiled image at binding 11 blended over the albedo).
  // Both share the sampler at binding 10 (9-11 are clear of capture's 3-5,
  // perturb's 6, opAux 7 and objAux 8; 12-13 are reserved for #630). Same
  // contract as envx: CODEGEN-gated, default false ⇒ byte-identical text —
  // the renderer latches each flag per frame from "an image is loaded AND its
  // amount > 0" and keys the variant on it.
  envMap = false,
  surfTex = false,
  // NEON (IDEAS 2026-08-21) — emissive surface glow. CODEGEN-gated exactly
  // like envx (#125: a never-executing uniform branch is not free): default
  // false emits byte-identical text. The on-variant records the coloring
  // signal surfaceAlbedo ALREADY computes (post auto-levels, pre-iridescence/
  // phase — the signal's height, not its hue position) in a module private,
  // then adds one HDR emission term after the lighting stack:
  //   col += albedo · gain · sig²
  // albedo IS the palette at the signal (emission color for free), the
  // quadratic concentrates glow in high-signal regions, and the gain rides
  // p3ctl.w — a reserved lane, the #630 runtime-word precedent — in
  // pre-tonemap HDR units so bright emission crosses the P3 bloom threshold
  // (fog.w) and halos through the EXISTING bloom composite. Flat formulas
  // only in V1 (the renderer never latches neon on scenes — the iridescence
  // S6 precedent); Painter records sig = 1 (its per-iteration palette blend
  // carries the spatial variation). The renderer latches it per frame from
  // the look (neonGain > 0) and keys the variant on it, like envx.
  neon = false,
  // AURORA (ENVX P6, IDEAS 2026-08-21 "Aurora/nebula ENVX layer") — colored
  // fbm sky behind the fractal: aurora curtains + nebula clouds, added at the
  // background COMPOSITE (bgOut), deliberately NOT inside envColor — see the
  // auroraSky comment. CODEGEN-gated exactly like envx (#125): default false
  // emits byte-identical text. The renderer latches it per frame from the
  // look (aurora or nebula amount > 0) and keys the variant on it.
  aurora = false,
  // THIN FILM (IDEAS 2026-08-21) — angle-driven interference material: the
  // soap-bubble / beetle-shell sheen. NOT the S6 "Iridescence" slider
  // (p3ctl.x — an orbit-trap modulator on the Glow SIGNAL): this term is pure
  // GEOMETRY — phase ∝ cos(view, normal) — so its spectral bands sweep as the
  // camera orbits, and the two stack (S6 shifts the palette lookup, thin film
  // modulates the lit color by incidence angle). CODEGEN-gated exactly like
  // neon (#125: a never-executing uniform branch is not free): default false
  // emits byte-identical text. One modulation splice after the lighting
  // stack; the slider rides morphX.w — a reserved lane, the neon/p3ctl.w
  // runtime-word precedent. Flat formulas only in V1 (the renderer never
  // latches thinFilm on scenes — the neon/S6 precedent). The renderer latches
  // it per frame from the look (thinFilm slider > 0), like envx.
  thinFilm = false,
  // TINY PLANET (IDEAS 2026-08-21 wave) — stereographic "little planet" RAY
  // GENERATION. Replaces the perspective frustum with an inverse-stereographic
  // map from the image plane onto the sphere of directions around the eye, so
  // (up to) the whole sphere folds into frame: the subject wraps into a disc
  // with sky around it.
  //
  // CODEGEN-gated, NOT uniform-gated — and deliberately NOT the shape #441
  // orthographic used. Ortho's branch is two lines that reuse the operands the
  // perspective path already has; the stereographic map adds its own live
  // range (u, v, q, inv) across the whole ray-gen, and ray generation sits at
  // the TOP of the fragment shader that also holds the march — exactly the
  // register-pressure surface the #125 / d00d9a7 finding is about (a
  // never-executing uniform branch there cost Mandelbox +7%, Mandelbulb +31%).
  // Gating in codegen makes "off" byte-identical text, which is a stronger
  // result than any timing (core/tinyplanet.test.mjs pins it across the
  // feature matrix). The planet FOV itself stays a RUNTIME word inside the
  // on-variant (camRight.w — the #630 runtime-word precedent), so dragging
  // the slider is a uniform write, never a recompile.
  planet = false,
  // CLIP (IDEAS "Clipping-plane cross-section" + the wave's MRI sweep) — a
  // world-space plane cuts the fractal open: march hits in the clipped
  // half-space are never produced, and rays that reach the plane INSIDE the
  // solid land on a flat CUT FACE that shows the interior cross-section.
  //
  // THE MATH — CSG intersection with a half-space, in the march itself:
  //   d = max(mapDE(p)·deScale, dot(n, p + offset) − h)
  // Both operands are lower bounds of the distance to their sets, so the max
  // is a lower bound of the distance to (fractal ∩ keep-half-space) — the
  // standard sdIntersect argument — and the plane term is EXACT, so the
  // combined march is precisely as DE-safe as the plain one (the catalog row
  // scores DE-safety low; this form keeps it there). The same expression
  // replaces the bisection refinement's probe: refining against the UNCLIPPED
  // mapDE would walk the bracket back through the cut face (interior DE ≈ 0
  // reads as "hit" everywhere behind the plane).
  //
  // THE CUT FACE — at the landed hit, the plane term binding the max
  // (planeD ≥ mapDE·deScale) means the surface is the cross-section, not the
  // fractal shell: the normal becomes the plane normal (an interior DE
  // gradient is noise) and the shade is a FLAT, shadow/AO-free lightbox read
  // of surfaceAlbedo AT the cut point — the coloring signal sampled inside
  // the body is what makes it read as an MRI slice. AO/soft-shadow are
  // skipped for cut pixels deliberately: both march the UNCLIPPED mapDE from
  // a point inside the original solid, so they would return ~0 and paint the
  // face black.
  //
  // CODEGEN-gated exactly like planet (#125: a never-executing uniform branch
  // in the shader that holds the march is not free): default false emits
  // byte-identical text (core/clipplane.test.mjs pins it across the feature
  // matrix and the preset catalog). The plane itself is RUNTIME words inside
  // the on-variant (clipU, tail row CLIP_WORD — the envx tail contract), so
  // dragging the offset slider — and the Loop Lab MRI sweep driving it every
  // frame — is a uniform write, never a recompile. No scene guard: the plane
  // is march geometry (the planet/aurora contract, not neon's flat-only one).
  clip = false,
  // CLIP JAGGED (the owner's "noisy/jagged plane, so it consumes the shape")
  // — the cut is eroded by STATIC world-space value noise instead of being
  // mathematically flat, so a sweep reads as disintegration and a still cut
  // reads as weathered. The plane term becomes
  //   (dot(n, pw) − h − amp·clipJagNoise(pw·freq)) · invD
  // with pw the world point and invD = 1/(1 + L·amp·freq) the Lipschitz
  // correction that keeps the term a LOWER BOUND of the distance to the
  // noised half-space (L = 7 bounds the 2-octave smoothstep value noise —
  // see frameparams' CLIP_JAG_LIP derivation), so the march stays exactly as
  // DE-safe as the flat clip. Its OWN codegen latch on the same #125
  // rationale: the noise is evaluated once per march step, so a flat cut
  // must not carry it — clipJag:false emits the flat-clip text byte for
  // byte. amp/freq/invD are RUNTIME words in clipS.yzw (lanes already
  // allocated to clip — no new row), so dragging the Jagged slider (and the
  // sweep animating the offset through the static field) never recompiles
  // within a variant.
  clipJag = false,
  // 360° EQUIRECTANGULAR (IDEAS "360°/VR" row, mono v1) — full 360×180
  // lat-long RAY GENERATION: pixel → (longitude, latitude) → direction in the
  // camera's basis, so a saved 2:1 frame IS a 360 photo. The second arm on the
  // ray-gen seam tiny planet (#670) cut — CODEGEN-gated for exactly the same
  // reason (the map's own live range sits at the top of the fragment shader
  // that holds the march, the #125 / d00d9a7 register-pressure surface), and
  // "off" emits byte-identical text (core/equirect.test.mjs pins it across the
  // feature matrix, planet included). The one runtime word is the LONGITUDE
  // SCALE (camUp.w = π·H/W of the FULL image — a spare lane zeroed every
  // frame, the camRight.w precedent), which is what lets a TILE render exactly
  // (the tile-local aspect cannot recover the full-image span). Mutually
  // exclusive with both planet and ortho, enforced in preview.js's setters.
  // Stereo/VR is deliberately deferred — this is the mono emitter only.
  equirect = false,
  // #633 seam channel override. undefined (the default) derives the channel
  // from the op-set exactly as before — usesSeam(ops), i.e. armed for any
  // specialized op-set containing a seam op, and armed for the FULL switch
  // (ops: null), which is the conservative fallback every non-renderer caller
  // keeps. `false` emits the seam-FREE full switch: seam-op cases keep their
  // fold bodies but drop the seam report (each op's `wgslSeam` splice), and
  // the march loop steps plain — the browse GENERAL variant passes this when
  // the live formula has no seam ops, so seam-free content never pays the
  // channel (the #125 off-state discipline; the live cost was measured, not
  // guessed — Mandelbulb browsing +31%-class, and the whole "not sharpening"
  // v1.4 field regression). The renderer routes seam formulas to seam-armed
  // variants by key (F_SEAM), so a mis-route can only degrade to the
  // pre-#633 march, never to a dropped op.
  seam = undefined,
} = {}) {
  ...
  // Two answers to "how does a pixel become a ray" cannot both win. The
  // renderer never keys this combination (the preview setters are mutually
  // exclusive); throwing keeps the invariant loud for any other caller.
  if (planet && equirect)
    throw new Error("buildWGSL: planet and equirect are exclusive projections");
  // #630 eligibility domain (spec review 2f): a reflected ray's purpose is to
  // travel AWAY from the hit — out of the neighbourhood the df64 switchover /
  // perturbation reference orbit linearises around. It would not error; it
  // would silently return garbage DE and the mirror would show plausible
  // noise. The renderer never keys this combination; throwing keeps the
  // invariant loud for any other caller.
  if (sreflect && (df64 || perturb))
    throw new Error(
      "buildWGSL: sreflect requires the plain f32 tier (df64/perturb reflect garbage DE off the reference-orbit neighbourhood)",
    );
  ...
  const allow = ops ? new Set(ops) : null;
  // #633 — the seam channel gate, hoisted above the case emission because the
  // seam-op case bodies themselves splice their seam report (`wgslSeam`) only
  // when the channel exists (a report into an undeclared accumulator would not
  // compile). See the `seam` option above for the override contract.
  const seamUsed = seam ?? usesSeam(ops);
  const cases = OPERATORS.filter((op) => !allow || allow.has(op.id))
    .map(
      (op) =>
        `      case ${op.id}u: {${spliceAux(
          op.wgsl + (seamUsed && op.wgslSeam ? op.wgslSeam : ""),
          op.key,
        )}
      }`,
    )
    .join("\n");
  // The overflow lane is emitted ONLY when some op in THIS shader's op-set
  // actually reads it (OP_PARAM_ENCODING.md §5.2 + §11 R4). A formula of
  // ≤3-param ops therefore produces a shader with not one `opAux` token — the
  // pay-per-use guarantee is structural, not hopeful (#125: uniform-gated heavy
  // code cost Mandelbulb +31% while "never executing").
  const auxUsed = usesOpAux(ops);
  // #627 — same structural pay-per-use rule for the LEAF lane: emitted only
  // when this variant's leaf-id set contains a >4-param leaf.
  const objAuxUsed = usesObjAux(leaves);
  // #633 — the SEAM channel follows the same structural pay-per-use rule: the
  // private accumulator, the band-gated march step, the per-case seam reports
  // and the fold-membrane debug view are all keyed on `seamUsed` (hoisted
  // above the case emission — see the `seam` option). A seam-free variant
  // emits not one seam token and is byte-identical to the pre-#633 build.
  ...
  const casesDf = df64
    ? OPERATORS.filter((op) => op.wgslDf && (!allow || allow.has(op.id)))
        .map(
          (op) =>
            `        case ${op.id}u: {${spliceAux(op.wgslDf, op.key)}
        }`,
        )
        .join("\n")
    : "";
...
${cases}
  ...
  const casesPt = perturb
    ? OPERATORS.filter((op) => op.wgslPt && (!allow || allow.has(op.id)))
        .map(
          (op) =>
            `          case ${op.id}u: {${spliceAux(op.wgslPt, op.key)}
          }`,
        )
        .join("\n")
    : "";
...
${cases}
  ...
  // ── Self-reflection (#630) — the sreflect codegen splices ──────────────────
  // Six splice points restructure the live fs into a marched-bounce loop when
  // sreflect is on; every splice degrades to the EXACT pre-#630 text when off
  // (byte-identity is the gate — the #125 lesson, same standard as envx).
  // Design decisions, each answering a spec-review blocker on issue #630:
  //   2a  ONE shared shading body: the whole march+shade text sits inside the
  //       bounce loop, so a mirror shows the SAME lit geometry as the direct
  //       view and future lighting edits happen once. No shadeHit refactor —
  //       the off variant is untouched text, a stronger identity than any
  //       pixel diff.
  //   2b  t is hoisted OUT of the loop and never reset: it accumulates path
  //       length across bounces, so the eps*t hit shell, t-scaled normal/AO/
  //       shadow probes, far fade and fog keep their ray-cone meaning on
  //       secondary rays. tFar stays the total-path cap (bounded cost);
  //       each bounce marches with a fresh maxSteps budget.
  //   2c  the reflected ray launches from a NORMAL-lift at the tuned
  //       softShadow offset (max(eps*t*12, 2e-3)) — a direction-offset hugs
  //       the surface at grazing incidence, exactly the acne class the
  //       shadow march documents. The origin is then back-projected by t
  //       along the new direction so the march loop text needs no change.
  //   2d/2e  bounces are refused off suspect surfaces: srDegen (calcNormal's
  //       fabricated-normal fallback) and a back-facing normal (srCos <= 0,
  //       the carved-wall overstep class) both fall back to the env term
  //       instead of marching a garbage direction.
  //   2h  no new mapDE/calcNormal call SITES: the loop re-runs the existing
  //       single textual sites, so the op switch is not inlined again and
  //       the #218 compile-time lesson stays honored.
  // The runtime words (all pre-existing reserved lanes — no struct growth, no
  // new binding): lightC.w = bounce count 0..6, light2c.w = reflectivity,
  // light3c.w = Fresnel edge, colorX.w = metal tint.
  const srHead = sreflect
    ? `  // Self-reflection (#630, sreflect variants only): marched mirror bounces.
  // One shared shading body serves every bounce; t accumulates path length
  // across the whole reflection path (never reset — see the launch site at
  // the bottom of the loop). bounceN is a runtime word INSIDE this opt-in
  // variant; the codegen gate is the reflBounces > 0 latch itself.
  var accC = vec3f(0.0);
  var thruR = vec3f(1.0);
  var outA = 1.0;
  let bounceN = u32(clamp(G.lightC.w, 0.0, 6.0));
  var t = G.res.w;
  for (var bnc = 0u; bnc <= bounceN; bnc = bnc + 1u) {`
    : `  var t = G.res.w;`;
  const srMiss = sreflect
    ? `    if (bnc == 0u) { outA = 0.0; }
    accC = accC + (thruR * skyOut);
    break;`
    : `    return vec4f(skyOut, 0.0);`;
  const srSplit = sreflect
    ? `  // Self-reflection split (#630): how much of this hit's outgoing light
  // the MARCHED reflection carries. Standard Schlick edge shape: the
  // reflectivity word sets the base amount, the Fresnel-edge word blends
  // between a uniform mirror (0) and pure grazing-edge reflection (1), and
  // the metal-tint word pulls the reflected image toward the surface albedo
  // (0 = silvered/neutral, 1 = fully tinted).
  let srCos = max(dot(nrm, -rd), 0.0);
  let srKR = clamp(G.light2c.w, 0.0, 1.0)
      * mix(1.0, pow(1.0 - srCos, 5.0), clamp(G.light3c.w, 0.0, 1.0));
  let srThruN = thruR * (srKR * mix(vec3f(1.0), albedo, clamp(G.colorX.w, 0.0, 1.0)));
  // March another bounce only while it can still change the image: bounces
  // left in the budget, a REAL normal to reflect off (srDegen = calcNormal
  // fabricated one; srCos <= 0 = back-facing, the carved-wall overstep
  // class — both would paint a hard structured wrong image), and
  // non-negligible throughput (0.004 is under one 8-bit LSB).
  let srMore = (bnc < bounceN) && !srDegen && (srCos > 1e-4)
      && (max(srThruN.x, max(srThruN.y, srThruN.z)) > 0.004);
`
    : "";
  const srEnvGuard = sreflect ? "!srMore && " : "";
  const srTail = sreflect
    ? `  // Deposit this hit's shaded light. While another bounce follows, the kR
  // fraction leaves with the reflected ray instead (an energy split, not
  // double counting); the final bounce keeps everything — its env-reflection
  // term above IS the fallback at the bounce cap, and a reflected MISS lands
  // in the sky deposit at the top of the loop.
  accC = accC + (thruR * max(col, vec3f(0.0))) * select(1.0, 1.0 - srKR, srMore);
  if (!srMore) { break; }
  thruR = srThruN;
  // Launch the reflected ray: normal-lift at the tuned softShadow offset
  // (eps*t-scaled, deep-zoom-safe), then back-project the origin by t along
  // the new direction so the march loop's p = ro + rd * t continues with t
  // as ACCUMULATED path length — hit shell, tFar cap and fog all stay
  // consistent with no march-text change.
  rd = reflect(rd, nrm);
  ro = (p + nrm * max(G.prm.y * t * 12.0, 2e-3)) - (rd * t);
  }
  return vec4f(max(accC, vec3f(0.0)), outA);`
    : `  return vec4f(max(col, vec3f(0.0)), 1.0);`;
  // calcNormal's degenerate-gradient tap (spec review 2e): where the
  // tetrahedron gradient collapses, the returned normal is FABRICATED
  // (radial/world-up) — fine as a subtle shading fallback, a hard structured
  // error as a mirror. calcNormal records the condition; the bounce decision
  // reads it. A module-scope private, so the calcNormal SIGNATURE (shared
  // with the capture fs) never changes.
  const srDegenDecl = sreflect
    ? `// #630 — set by calcNormal when its gradient collapsed and the normal it
// returned is fabricated; the bounce loop then refuses to mirror off it.
var<private> srDegen : bool = false;
`
    : "";
  const srDegenStore = sreflect ? `  srDegen = !(L > 1e-20);\n` : "";

  // ── NEON — the emissive-glow codegen splices (see the option doc above) ────
  // Four splice points, every one the empty string when off (byte-identity is
  // the gate — the envx/#630 standard). The signal record rides a module
  // private so the surfaceAlbedo SIGNATURE (shared with the capture fs) never
  // changes — the srDegen pattern.
  const neonDecl = neon
    ? `// NEON — the coloring signal surfaceAlbedo records for the emission term
// (module private, the srDegen pattern: no signature change on a shared fn).
var<private> neonSig : f32 = 0.0;
`
    : "";
  const neonSigLine = neon
    ? `    neonSig = m; // NEON — record the signal (pre-iridescence/phase)\n`
    : "";
  const neonPainterLine = neon
    ? `
    neonSig = 1.0; // NEON — Painter's palette blend carries the variation`
    : "";
  const neonEmit = neon
    ? `  // NEON — emissive glow: self-luminous (no AO/shadow/cavity), after the
  // object-intensity multiply (the Neon slider is the sole gain control) and
  // before fog, so haze attenuates the glow like any other surface light.
  // p3ctl.w = pre-mapped HDR gain (frameparams neonGain; 0 never reaches
  // this variant — the renderer latches neon off).
  col = col + albedo * (G.p3ctl.w * neonSig * neonSig);
`
    : "";

  // ── THIN FILM — the interference-material codegen splice ────────────────────
  // One splice, the empty string when off (byte-identity is the gate — the
  // neon/envx standard). The classic cheap thin-film approximation: the
  // optical path difference of a film is 2·n·d·cosθ, and the reflected
  // spectrum at wavelength λ goes as cos(2π·OPD/λ) — so sample the ramp at
  // three wavelengths via inverse-λ ratios (R:G:B = 650/650 : 650/532 :
  // 650/450 ≈ 1 : 1.2218 : 1.4444) and the fringe ORDER comes out physical
  // (long-λ red bands widest, blue tightest). filmPh is the R-channel phase
  // in radians; 12·cosθ ≈ two fringe orders center-to-limb, which reads as
  // bands, not rainbow noise. The weight leans toward grazing incidence
  // (real film response strengthens at the limb) with a floor so face-on
  // surfaces keep a visible tint; the factor is a MODULATION with mean 1
  // over phase (mix toward 2·filmCol) — energy-conserving-ish, bounded at
  // 2×, never an unbounded HDR add (that's Neon's job). Applied after the
  // object-intensity multiply, before the NEON emission (interference tints
  // reflected light, not self-luminance) and before fog.
  const filmMod = thinFilm
    ? `  // THIN FILM — angle-driven interference tint (thinFilm variants only; see
  // the splice doc at filmMod). morphX.w = the slider (frameparams filmAmt;
  // 0 never reaches this variant — the renderer latches thinFilm off).
  let filmCos = clamp(dot(nrm, -rd), 0.0, 1.0);
  let filmPh = 12.0 * filmCos;
  let filmCol = 0.5 + 0.5 * cos(vec3f(1.0, 1.2218, 1.4444) * filmPh);
  let filmW = G.morphX.w * (0.3 + 0.7 * pow(1.0 - filmCos, 2.0));
  col = col * mix(vec3f(1.0), filmCol * 2.0, filmW);
`
    : "";
  // ── CLIP (cross-section) march terms — see buildWGSL's clip flag ──────────
  // The clipped march distance: the CSG max of the scaled DE and the exact
  // plane distance, in WORLD space (p is the recentered residual, so the
  // offset is added back — the surfTex world-anchor precedent). clip:false
  // emits the pre-clip expression byte for byte in BOTH the march loop and
  // the bisection refinement (core/clipplane.test.mjs pins it).
  // The jag substitution is one shared helper so the march, the bisection AND
  // the cut-face classification use the identical expression — classification
  // compares the operand that actually bound the max. clipJag:false emits the
  // FLAT-clip term byte for byte (the noise never rides a flat variant).
  const clipPlaneAt = (pt) =>
    clipJag
      ? `(dot(G.clipU.xyz, ${pt} + G.offset.xyz) - G.clipU.w - G.clipS.y * clipJagNoise((${pt} + G.offset.xyz) * G.clipS.z)) * G.clipS.w`
      : `dot(G.clipU.xyz, ${pt} + G.offset.xyz) - G.clipU.w`;
  const clipDE = clip
    ? `max(mapDE(p) * G.prm.z, ${clipPlaneAt("p")})`
    : `mapDE(p) * G.prm.z`;
  const clipDEMid = clip
    ? `max(mapDE(ro + rd * mid) * G.prm.z, ${clipPlaneAt("ro + rd * mid")})`
    : `mapDE(ro + rd * mid) * G.prm.z`;
  const cutShade = clip
    ? `  // CLIP — the MRI cut-face shade (clip variants only): a flat, lightbox
  // read of the coloring AT the cut point. The full lighting stack above is
  // discarded for cut pixels on purpose — AO/shadow were skipped (they march
  // the UNCLIPPED mapDE from inside the body and would black the face), the
  // GGX/rim terms are shell material, and a flat fill is what makes the
  // slice read as a scan rather than a lit wall. The plane-normal diffuse
  // term is CONSTANT across the whole face (one lightbox level, keyed to the
  // light rig); the spatial variation is all albedo — the coloring signal
  // sampled INSIDE the body, which is the interior structure. clipS.x is a
  // reserved brightness gain (frameparams writes 1 today). Fog still applies
  // below, so a deep sweep keeps its depth cue.
  if (cutFace) {
    col = albedo * ((0.42 + 0.58 * max(dot(nrm, lightDir), 0.0)) * G.clipS.x) * G.lprm.w;
  }
`
    : "";
  // ── Ray generation ────────────────────────────────────────────────────────
  // TINY PLANET splices the WHOLE ray-gen region (direction, origin, DOF
  // guard) rather than adding a branch inside it, so `planet:false` emits the
  // pre-tiny-planet text BYTE FOR BYTE — the perspective/ortho arm below is a
  // verbatim copy, and core/tinyplanet.test.mjs hashes both arms across the
  // feature matrix to keep it that way.
  //
  // THE MAP (mirrored in core/shader_gl.js mainGL, core/cpu.js traceGrid and
  // core/preview.js pixelRay — four hand-kept copies, none compared by CI):
  //
  //   plane point (u,v), q = u·u + v·v
  //   dir = ( 2u·right + 2v·up + (1−q)·fwd ) / (1 + q)
  //
  // the inverse stereographic projection from the plane onto the unit sphere,
  // projected FROM the antipode of `fwd`. It is exact and unit-length by
  // construction (no normalize needed — one is kept anyway for f32 hygiene,
  // matching the perspective arm). Landmarks:
  //
  //   (0,0)      → +fwd          the pole: screen centre looks at the target
  //   |(u,v)| =1 → the equator   90° off-axis, the "horizon" circle
  //   |(u,v)|→∞  → −fwd          the antipode, behind the camera
  //
  // The pole is camFwd, i.e. the orbit TARGET direction — that is what makes
  // the subject read as a planet rather than a smeared fisheye: the thing you
  // are looking at wraps into the central disc and everything else becomes
  // sky around it.
  //
  // planetK (camRight.w) = tan(planetFov/4): the plane radius the frame's
  // vertical half-extent reaches, so a pixel at |wy| = 1 looks out at polar
  // angle 2·atan(planetK) = planetFov/2 and the frame spans planetFov top to
  // bottom. k = tan(fov/2)/2 reproduces the perspective view's CENTRE
  // magnification exactly (both are dθ/dw = tanF there), which is why the
  // slider's mild end changes nothing about the subject's size and only bends
  // the periphery.
  //
  // Tiling composes EXACTLY: G.tile is a linear window on (wx,wy), i.e. on the
  // stereographic PLANE, so a tile is a true crop of the final image — the
  // same argument §2.1.3 makes for the perspective arm, and it survives the
  // non-linear map because the map is applied AFTER the window.
  //
  // Every product is parenthesised on purpose — an unparenthesised multiply
  // has blacked out every scene in this repo before. And NO BACKTICKS in
  // comments here: this WGSL is a JS template literal.
  // tanF is the PERSPECTIVE half-frame tangent; the stereographic arm has no
  // use for it, so the planet variant does not emit the declaration at all
  // (an unused local is legal on both tiers, but a dead tan() in the shader
  // that also holds the march is exactly the kind of thing this file's header
  // asks not to leave lying around). The off arm keeps the line, in place,
  // byte for byte.
  // 360° EQUIRECTANGULAR (the second arm on this seam — see buildWGSL's
  // equirect flag). THE MAP (mirrored in core/shader_gl.js mainGL,
  // core/cpu.js traceGrid and core/preview.js pixelRay, like the planet's):
  //
  //   lon = wx · eqS          eqS (camUp.w) = π·H/W of the FULL image,
  //   lat = wy · π/2          so the frame spans 360° × 180° edge to edge
  //   dir = cos(lat)·( sin(lon)·right + cos(lon)·fwd ) + sin(lat)·up
  //
  // Landmarks: (0,0) → +fwd (image centre looks at the orbit target, so
  // orbiting rotates the panorama naturally); wy = ±1 → ±up (the top/bottom
  // rows all converge on the poles); wx at the frame's horizontal edge → −fwd
  // from both sides (the left and right edges look BEHIND the camera and must
  // agree — the wraparound the probe checks). The basis is the CAMERA's, pitch
  // included, so the horizon is a straight line exactly when the camera is
  // level and bends honestly when it is not (a camera-aligned pano, the
  // planet's full-basis precedent — not a gravity-locked one).
  //
  // eqS is a RUNTIME word because of TILING: G.tile is a linear window on
  // (wx,wy) and the map is applied after the window, so a tile is an exact
  // crop — but wx spans ±(full-image aspect), which the tile-local
  // G.res.x/G.res.y cannot recover. π·H/W of the full frame is therefore
  // computed JS-side (core/preview.js, one derivation) and carried in
  // camUp.w, the camRight.w precedent: a spare lane that writeGlobals was
  // already zeroing every frame, so the packer is part of the change.
  //
  // dir is unit-length by construction (cos²·(sin²+cos²) + sin² = 1); the
  // normalize is kept for f32 hygiene, matching the other arms.
  const tanFDecl =
    planet || equirect ? "" : "  let tanF = tan(0.5 * G.res.z);\n";
  const rayGenEquirect = `  // 360 EQUIRECT — lat-long ray generation (see buildWGSL's equirect flag).
  // camUp.w is the longitude scale of the FULL image, pi*H/W; it is a RUNTIME
  // word inside this variant so tiled export composes exactly (the tile-local
  // aspect cannot recover the full-image span).
  let eqS = G.camUp.w;
  let eqLon = wx * eqS;
  let eqLat = wy * 1.5707963267948966;
  let eqCl = cos(eqLat);
  var rd = normalize((eqCl * sin(eqLon)) * G.camRight.xyz
      + (sin(eqLat)) * G.camUp.xyz
      + (eqCl * cos(eqLon)) * G.camFwd.xyz);
  // Deep zoom §3.1 — camPos carries the RESIDUAL ro_rel = eye−offset, not the
  // absolute eye. Equirect is a SINGLE-ORIGIN projection (only the direction
  // is remapped, the tiny-planet argument verbatim), so the residual is
  // untouched and every reconstruction downstream is as under perspective.
  var ro = G.camPos.xyz;
  // P4 thin-lens DOF composes mechanically (one eye, per-pixel directions —
  // the planet's argument), and is kept for parity with that arm. In a 360
  // photo a shallow lens is an odd look, but declining it here would make
  // this the one arm that silently ignores a live slider.
  if (G.dof.x > 0.0) {
    let fp = ro + rd * G.dof.y;
    ro = ro + (G.camRight.xyz * G.dof.z + G.camUp.xyz * G.dof.w) * G.dof.x;
    rd = normalize(fp - ro);
  }`;
  const rayGen = equirect
    ? rayGenEquirect
    : planet
      ? `  // TINY PLANET — stereographic ray generation (see buildWGSL's planet flag).
  // camRight.w is the plane radius at the frame's vertical half-extent,
  // tan(planetFov/4); it is a RUNTIME word inside this variant, so moving the
  // slider is a uniform write and never a recompile.
  let planetK = G.camRight.w;
  let pu = wx * planetK;
  let pv = wy * planetK;
  let pq = (pu * pu) + (pv * pv);
  let pInv = 1.0 / (1.0 + pq);
  var rd = normalize(((2.0 * pu) * pInv) * G.camRight.xyz
      + ((2.0 * pv) * pInv) * G.camUp.xyz
      + ((1.0 - pq) * pInv) * G.camFwd.xyz);
  // Deep zoom §3.1 — camPos carries the RESIDUAL ro_rel = eye−offset, not the
  // absolute eye. Tiny planet is a SINGLE-ORIGIN projection (only the
  // direction is remapped), so the residual is untouched and the deep-zoom
  // reconstruction downstream is exactly as it is under perspective.
  var ro = G.camPos.xyz;
  // P4 thin-lens DOF composes: this is still one eye with per-pixel
  // directions, so the lens disk offset and the re-aim at the focus plane
  // mean what they always did (unlike ortho, whose parallel rays have no
  // lens). Bokeh at the frame edge is stretched by the projection, which is
  // what a real fisheye does too.
  if (G.dof.x > 0.0) {
    let fp = ro + rd * G.dof.y;
    ro = ro + (G.camRight.xyz * G.dof.z + G.camUp.xyz * G.dof.w) * G.dof.x;
    rd = normalize(fp - ro);
...
${
  envx || envMap || surfTex || aurora || clip
    ? `  starsU  : vec4f,   // ENVX (backgrounds P5) — x = star amount, y = cell
                      // density D, z = seed offset, w reserved (twinkle).
  bandU   : vec4f,   // ENVX — xyz = Milky-Way plane normal (unit, Z-up world),
                      // w = band amount.
  zenU    : vec4f,   // ENVX — rgb = zenith color (sRGB), w = blend toward it
                      // (0 = the legacy 0.35·bgc zenith exactly). These three
                      // rows exist ONLY in envx variants; the globals BUFFER is
                      // always allocated at GLOBALS_WORDS_ALLOC and the tail
                      // words are always written, so binding size ≥ struct
                      // size holds for every variant (mirror of offsetLo's
                      // "unread words render byte-identically" contract).
`
    : ""
}${
      envMap || surfTex || aurora || clip
        ? `  emapU   : vec4f,   // IMGTEX (#631) — env map: x = amount (0..1 blend of
                      // the image over the procedural sky), y = brightness
                      // (0..3), z = rotation (0..1 = one turn), w reserved.
                      // These two rows exist only in envMap/surfTex variants,
                      // at FIXED words 51/52 — which is why the ENVX rows
                      // above are emitted (as dormant padding) whenever an
                      // image row is: struct offsets must match the one
                      // buffer layout writeGlobals always writes.
  triU    : vec4f,   // IMGTEX — surface texture: x = amount (0..1 albedo
                      // blend), y = tile size in world units (0.02..3),
                      // zw reserved.
`
        : ""
    }${
      aurora || clip
        ? `  aurU    : vec4f,   // AURORA (ENVX P6) — x = aurora amount, y = nebula
                      // amount, z reserved (fbm scale), w = drift phase
                      // (0 today — reserved for slow time drift; no per-frame
                      // writer exists, matching starsU.w's dormant twinkle).
  aurA    : vec4f,   // AURORA — rgb = curtain-floor color (sRGB, derived from
                      // the ONE hue control in frameparams.js), w reserved.
  aurB    : vec4f,   // AURORA — rgb = curtain-tip / nebula-accent color
                      // (sRGB, hue shifted +0.42), w reserved. These rows
                      // exist ONLY in aurora variants — which therefore also
                      // carry the ENVX/IMGTEX rows above as dormant padding,
                      // so every word keeps its fixed offset against the one
                      // buffer layout writeGlobals always writes (the same
                      // always-written / read-only-by-its-variants contract).
`
        : ""
    }${
      clip
        ? `  gradeA  : vec4f,   // CINE GRADE rows 56..59 — DORMANT PADDING here: the
  gradeB  : vec4f,   // grade rows are post-pass words (buildPostWGSL), never
  gradeC  : vec4f,   // read by a march shader, but the clip rows below sit at
  gradeD  : vec4f,   // words 60+, so a clip variant must declare 56..59 to
                      // keep every offset fixed against the one buffer layout
                      // writeGlobals always writes (the emapU-in-aurora rule).
  clipU   : vec4f,   // CLIP (cross-section) — xyz = plane unit normal (world
                      // Z-up; frameparams derives it from axis+flip), w = the
                      // plane offset along it. KEEP the half-space n·x <= w;
                      // cut away n·x > w. These rows exist ONLY in clip
                      // variants — which therefore carry every tail row above
                      // as dormant padding (same contract as all of them).
  clipS   : vec4f,   // CLIP — x = cut-face brightness gain (reserved; 1
                      // today, no UI writer). JAGGED lanes (read only by
                      // clip-jag variants; flat clip reads x alone): y =
                      // noise amplitude (world units), z = noise frequency,
                      // w = the 1/(1 + CLIP_JAG_LIP·amp·freq) Lipschitz
                      // divisor (1 whenever jag is off) — frameparams owns
                      // all three.
`
        : ""
    }};
...
@group(0) @binding(2) var<storage, read> objects : array<Obj>;${
      auxUsed
        ? `
// Op-param overflow lane (OP_PARAM_ENCODING.md §5.2) — p3,p4,p5,(reserved) for
// the op at the SAME index as ops[]. Emitted only when an op in this shader's
// op-set declares more than ${MAX_OP_PARAMS_INLINE} params; binding 7 is clear of the capture
// block's 3-5 and perturb's 6. renderer.js binds it on the same predicate.
@group(0) @binding(7) var<storage, read> opAux : array<vec4f>;`
        : ""
    }${
      objAuxUsed
        ? `
// Leaf-param overflow lane (#627 — the leaf mirror of the op lane above) —
// sp4..sp7 for the OBJECT at the same index as objects[]. Emitted only when
// this variant's leaf-id set contains a leaf declaring more than
// ${MAX_LEAF_PARAMS_INLINE} params; binding 8 is clear of the capture block's 3-5, perturb's 6
// and the op lane's 7. renderer.js binds it on the same predicate (usesObjAux).
@group(0) @binding(8) var<storage, read> objAux : array<vec4f>;`
        : ""
    }${
      seamUsed
        ? `
// #633 seam channel — the running-min distance to the nearest fold tear,
// pulled back to formula space (the seam-clamped DE contract, operators.js
// id 68). Module-private so the SAME op bodies compile in every loop copy
// with zero signature changes (the g_wq convention of the GL tier): seam-op
// cases write it, mapDE resets it per call, and the ONLY reader is the
// primary march step directly after its own mapDE call — every other caller
// (normals, shadows, AO, coloring, capture) merely overwrites it, by design.
var<private> seamOut : f32 = 1e30;`
        : ""
    }${
      envMap
        ? `
// IMGTEX (#631) — the user's environment image (equirect, rgba8unorm-srgb so
// samples arrive linear). Emitted ONLY when the renderer latched an image as
// loaded + active (the envx pay-per-use contract: the flag-off shader carries
// not one texture token). Bindings 9-11 are clear of capture's 3-5, perturb's
// 6, opAux 7 and objAux 8 (12-13 reserved for #630).
@group(0) @binding(9) var envTex : texture_2d<f32>;`
        : ""
    }${
      envMap || surfTex
        ? `
// IMGTEX — shared sampler (linear, repeat both axes: equirect u wraps a full
// turn and the triplanar taps tile).
@group(0) @binding(10) var imgSamp : sampler;`
        : ""
    }${
      surfTex
        ? `
// IMGTEX (#631) — the user's surface image, triplanar-projected at shading.
@group(0) @binding(11) var triTex : texture_2d<f32>;`
        : ""
    }
...
//__GATE:scene1:START
fn objIterDE(p0: vec3f, ob: Obj${objAuxUsed ? ", prm2: vec4f" : ""}) -> f32 {
  var pos = p0;
  var w = 1.0;
...
${cases}
    ...
    if (addC) { pos = pos + c; }
    if (iterShape) { dmin = min(dmin, leafDist(shapeId, pos, prm${objAuxUsed ? ", prm2" : ""}) / max(abs(w), 1e-9)); }
    if (dot(pos, pos) > G.prm.x) { break; }
  }
  let r = length(pos);
  ...
  if (shapeId > 0u) {
    ...
    return leafDist(shapeId, pos, prm${objAuxUsed ? ", prm2" : ""}) / max(abs(w), 1e-9);
  }
  return r / max(abs(w), 1e-9);
}
//__GATE:scene1:END
...
fn objDist(p0: vec3f, ob: Obj${objAuxUsed ? ", aux: vec4f" : ""}) -> f32 {
  let qinv = vec4f(-ob.qx, -ob.qy, -ob.qz, ob.qw);
  let pk = qrot(qinv, p0 - vec3f(ob.ox, ob.oy, ob.oz)) / ob.uscale;${
    seamUsed
      ? `
  // #633 — the object's op chain reports seams in LOCAL units; rescale by
  // uscale (the placement is rigid + uniform scale, exactly like the DE) and
  // fold into the scene-level accumulator around the child call.
  let seam0 = seamOut;
  seamOut = 1e30;
  let objd = objIterDE(pk, ob${objAuxUsed ? ", aux" : ""}) * ob.uscale;
  seamOut = min(seam0, seamOut * ob.uscale);
  return objd;`
      : `
  return objIterDE(pk, ob${objAuxUsed ? ", aux" : ""}) * ob.uscale;`
  }
}
...
fn mapDE(p_rel: vec3f) -> f32 {${
      seamUsed
        ? `
  seamOut = 1e30; // #633 — fresh seam accumulator per DE evaluation`
        : ""
    }
  ...
  for (var k: u32 = 0u; k < G.scene.x; k = k + 1u) {
    let ob = objects[k];
    let dk = objDist(p0, ob${objAuxUsed ? ", objAux[k]" : ""});
...
${srDegenDecl}fn calcNormal(p_rel: vec3f, t: f32) -> vec3f {
  ...
  let L = length(g);
${srDegenStore}  if (L > 1e-20) { return g / L; }
  let pl = length(p_rel);
...
${
  clip && clipJag
    ? `
// CLIP JAGGED — 2-octave smoothstep value noise for the eroded cut (clip-jag
// variants only; a flat clip carries not one of these tokens). WORLD-space
// and static, so the bite pattern stays put while the sweep advances the
// plane through it. Output in [-1, 1]; worst-case Lipschitz ≈ 7 per unit
// frequency (per-axis smoothstep slope 1.5 × corner span 2 → √3·3 ≈ 5.2 for
// one octave, × 2/1.5 for the octave sum) — frameparams' CLIP_JAG_LIP bakes
// that bound into the clipS.w divisor, which is what keeps the noised plane
// term a true lower bound (see clipPlaneAt).
fn clipJagHash(q: vec3f) -> f32 {
  return fract(sin(dot(q, vec3f(127.1, 311.7, 74.7))) * 43758.5453);
}
fn clipJagVal(q: vec3f) -> f32 {
  let i = floor(q);
  let f = fract(q);
  let u = f * f * (3.0 - 2.0 * f);
  let a = mix(
    mix(mix(clipJagHash(i), clipJagHash(i + vec3f(1.0, 0.0, 0.0)), u.x),
        mix(clipJagHash(i + vec3f(0.0, 1.0, 0.0)), clipJagHash(i + vec3f(1.0, 1.0, 0.0)), u.x), u.y),
    mix(mix(clipJagHash(i + vec3f(0.0, 0.0, 1.0)), clipJagHash(i + vec3f(1.0, 0.0, 1.0)), u.x),
        mix(clipJagHash(i + vec3f(0.0, 1.0, 1.0)), clipJagHash(i + vec3f(1.0, 1.0, 1.0)), u.x), u.y),
    u.z);
  return a * 2.0 - 1.0;
}
fn clipJagNoise(q: vec3f) -> f32 {
  return (clipJagVal(q) + 0.5 * clipJagVal(q * 2.03 + vec3f(13.5, 7.2, 3.1))) / 1.5;
}
`
    : ""
}
${
  envx
    ? `// ENVX (backgrounds P5) — procedural starfield + Milky-Way band, in DIRECTION
// space so it lives in the same one-world environment as the sun glow: stars
// parallax with camera rotation, appear in env reflections/IBL, and fog
// occludes them like any other miss. Cell-hash constellation: rd scaled by
// density D lands in a 3D cell; each cell owns one star jittered inside it
// (jitter 0.25 cells keeps the disc support clear of cell boundaries, so
// boundary clipping can't show). Returns LINEAR color, pre-scaled by amount.
fn hashE(p: vec3f) -> f32 {
  return fract(sin(dot(p, vec3f(127.1, 311.7, 74.7))) * 43758.5453);
}
fn starLayer(rd: vec3f, D: f32, sOff: vec3f, kAng: f32) -> vec3f {
  let cell = floor(rd * D + sOff);
  let j = vec3f(hashE(cell + 0.13), hashE(cell + 7.31), hashE(cell + 3.77));
  let s = normalize(cell + 0.5 + (j - 0.5) * 0.5 - sOff);
  // Magnitude: a ~30% presence cull + a STEEP curve (m⁶ over a 5% floor) —
  // most surviving cells are faint pinpricks, a few blaze. Peak 0.8, ON
  // PURPOSE below the bloom threshold (1.0): with Glow up, a star that
  // crosses it smears into exactly the splotch this tuning removes.
  let m = hashE(cell + 11.7);
  let live = step(0.3, hashE(cell + 17.3));
  let a2 = dot(rd - s, rd - s);
  let m2 = m * m;
  let b = exp(-a2 * kAng) * (0.05 + 0.75 * m2 * m2 * m2) * live;
  // Per-star temperature — kept SUBTLE (half-mixed to white): strong color
  // reads as "near"; a mountain sky's stars are almost white.
  let tint = mix(vec3f(1.0), mix(vec3f(1.0, 0.84, 0.66), vec3f(0.68, 0.82, 1.0), hashE(cell + 5.19)), 0.5);
  return tint * b;
}
fn starField(rd: vec3f) -> vec3f {
  // Disc size is fixed in SCREEN PIXELS (σ ≈ 1.0 px), like the point sources
  // real star photos show at any focal length: σ_angle = σ_px·fov/H, so the
  // gaussian's k = H²/(2σ_px²·fov²) = H²/(2.0·fov²). Angular- or cell-scaled
  // discs zoom into bokeh balls (the first cut did) — this stays a star.
  let kAng = G.res.y * G.res.y / (2.0 * G.res.z * G.res.z);
  let sOff = vec3f(G.starsU.z);
  // Milky-Way band: star brightness lifts near the plane, plus a faint cool
  // haze — the galaxy arc. bandU.w = 0 leaves both terms untouched.
  let bandW = 1.0 - smoothstep(0.0, 0.45, abs(dot(rd, G.bandU.xyz)));
  let bandLift = mix(1.0, 0.25 + 1.75 * bandW, G.bandU.w);
  // TWO depth layers — a bright sparse field over a denser, dimmer dust layer
  // (1.7× density, 0.55× brightness, capped at 150 cells/unit so the dust
  // never collapses into noise). One layer read as a lattice; depth is what
  // makes a mountain sky feel vast.
  var c = starLayer(rd, G.starsU.y, sOff, kAng);
  c = c + starLayer(rd, min(G.starsU.y * 1.7, 150.0), sOff + vec3f(31.7), kAng) * 0.55;
  return c * (bandLift * G.starsU.x)
       + vec3f(0.72, 0.78, 1.0) * (bandW * bandW * G.bandU.w * 0.12);
}
`
    : ""
}${
      aurora
        ? `// AURORA (ENVX P6, IDEAS 2026-08-21) — colored fbm sky: aurora curtains +
// nebula clouds, in the same Z-up DIRECTION space as the starfield. 3-octave
// value-noise fbm, evaluated ONCE per pixel at the background COMPOSITE
// (bgOut) — never in the march loop, the shading stack, or a reflection
// bounce. That placement is a deliberate divergence from the starfield
// (which lives inside envColor and so rides every IBL/reflection env tap):
// fbm per env call would tax hit pixels, and a sky wash contributes no
// legible detail to a reflection anyway. Own hash name (hashAu) so an
// envx+aurora build never collides with hashE. Colors arrive DERIVED from
// one hue control (frameparams.js — both GPU tiers upload identical words).
// Returns LINEAR color, pre-scaled by the amounts; peaks tuned UNDER the
// bloom threshold 1.0 (the 8e38804 star lesson: a peak that crosses it
// smears into a bloom splotch under Glow).
fn hashAu(p: vec3f) -> f32 {
  return fract(sin(dot(p, vec3f(157.31, 113.97, 271.89))) * 43758.5453);
}
fn vnoiseAu(p: vec3f) -> f32 {
  let i = floor(p);
  let f = fract(p);
  let u = f * f * (3.0 - 2.0 * f);
  return mix(
    mix(mix(hashAu(i), hashAu(i + vec3f(1.0, 0.0, 0.0)), u.x),
        mix(hashAu(i + vec3f(0.0, 1.0, 0.0)), hashAu(i + vec3f(1.0, 1.0, 0.0)), u.x), u.y),
    mix(mix(hashAu(i + vec3f(0.0, 0.0, 1.0)), hashAu(i + vec3f(1.0, 0.0, 1.0)), u.x),
        mix(hashAu(i + vec3f(0.0, 1.0, 1.0)), hashAu(i + vec3f(1.0, 1.0, 1.0)), u.x), u.y),
    u.z);
}
fn fbmAu(p: vec3f) -> f32 {
  var s = vnoiseAu(p) * 0.5;
  s = s + vnoiseAu(p * 2.03 + vec3f(19.1)) * 0.25;
  s = s + vnoiseAu(p * 4.07 + vec3f(7.7)) * 0.125;
  return s * 1.1428571; // ÷0.875 — normalize the octave sum to ~0..1
}
fn auroraSky(rd: vec3f) -> vec3f {
  var c = vec3f(0.0);
  let drift = vec3f(G.aurU.w);
  // Curtains: fbm over the unit CYLINDER (rd.xy ring × compressed height) —
  // continuous in direction space (no atan2 seam), and the anisotropy (ring
  // frequency ≫ height frequency) is what stretches the folds VERTICALLY.
  let ring = rd.xy / max(length(rd.xy), 1e-4);
  let n = fbmAu(vec3f(ring.x * 4.0, ring.y * 4.0, rd.z * 0.6) + drift);
  // A soft threshold separates fbm into distinct curtains; each fold's own
  // noise value sets how high it reaches. The base sits BELOW the horizon
  // (-0.3) so the arc rises from behind the scene and stays in frame at the
  // shallow camera pitches fractal framing actually uses (probe round 2:
  // a base above the horizon left whole presets with an empty layer).
  let curt = smoothstep(0.32, 0.72, n);
  // Fine streak ripple — a second, azimuth-denser fbm multiplying the fold:
  // the thin vertical striations that make curtains read as CURTAINS instead
  // of a green haze (probe round 3: structure at one frequency alone blurs
  // into a wash). Floor 0.35 so streak gaps dim, never cut, a fold.
  let st = fbmAu(vec3f(ring.x * 13.0, ring.y * 13.0, rd.z * 1.1) + drift + vec3f(5.0));
  let ripple = 0.25 + 0.75 * smoothstep(0.3, 0.7, st);
  let reach = 0.35 + 0.75 * n;
  let v = max((rd.z + 0.3) / reach, 0.0);
  // Sharp lower border, diffuse top (the real thing): fast rise, exp falloff.
  // The ×1.55 renormalizes the rise·decay product so a full-slider curtain
  // CORE actually reaches the 0.8 peak (probe round 1 measured ~0.2 —
  // "technically enabled", not aurora); min() caps the product at 1 so the
  // peak stays bloom-safe.
  let prof = min(smoothstep(0.0, 0.1, v) * exp(-1.7 * v) * 1.55, 1.0);
  // Floor color → hue-shifted tip by height above the horizon (green feet,
  // purple tips at the default hue). Peak 0.8 — under the bloom threshold.
  let aCol = mix(G.aurA.rgb, G.aurB.rgb, clamp(rd.z * 2.2, 0.0, 1.0));
  c = s2l(aCol) * (curt * ripple * prof * 0.8 * G.aurU.x);
  // Nebula: soft 3D clouds over the whole dome, tinted floor→accent by their
  // own density — subtler than the curtains (peak 0.45) and static.
  let nb = fbmAu(rd * 2.3 + vec3f(37.7) + drift);
  let cl = smoothstep(0.3, 0.8, nb);
  // Accent tint only in the DENSE cores (nb > 0.5): a mid-blend of the two
  // hues sags through gray (probe round: the whole nebula read as fog), so
  // thin cloud stays floor-colored and only the cores shift.
  let nCol = mix(G.aurA.rgb, G.aurB.rgb, clamp(nb * 2.2 - 1.1, 0.0, 1.0));
  c = c + s2l(nCol) * (cl * cl * 0.45 * G.aurU.y);
  // Both layers set below the horizon (the starfield's shape).
  return c * smoothstep(-0.35, 0.12, rd.z);
}
`
        : ""
    }${
      envMap
        ? `// IMGTEX (#631) — the user's image as the environment: equirect lookup in
// the SAME Z-up direction space as envColor (u = azimuth around +Z plus the
// rotation slider — the repeat sampler wraps the turn; v = polar angle from
// the +Z pole, clamped a hair off the poles so repeat addressing can't wrap
// the top row onto the bottom). textureSampleLevel (explicit LOD 0), NOT
// textureSample: envColor is called from non-uniform control flow (hit/miss
// branches, the reflection term), where implicit-derivative sampling is
// invalid WGSL. rgba8unorm-srgb makes the sample linear; emapU.y scales it.
fn envImage(rd: vec3f) -> vec3f {
  let u = atan2(rd.y, rd.x) * 0.15915494 + 0.5 + G.emapU.z;
  let v = clamp(acos(clamp(rd.z, -1.0, 1.0)) * 0.31830989, 0.001, 0.999);
  return textureSampleLevel(envTex, imgSamp, vec2f(u, v), 0.0).rgb * G.emapU.y;
}
`
        : ""
    }${
      surfTex
        ? `// IMGTEX (#631) — triplanar surface projection: the image is projected along
// each world axis and the three taps blend by normal weights sharpened to the
// 4th power ((n²)² — two squarings, no pow, no abs), so faces commit to one
// projection and the cross-fade bands stay narrow. triU.y is the tile size in
// world units. Mirrored in JS by envmap.js triplanarWeights (test-pinned).
fn triplanarImage(p: vec3f, n: vec3f) -> vec3f {
  var w = n * n;
  w = w * w;
  w = w / (w.x + w.y + w.z);
  let k = 1.0 / max(G.triU.y, 1e-3);
  let cx = textureSampleLevel(triTex, imgSamp, p.yz * k, 0.0).rgb;
  let cy = textureSampleLevel(triTex, imgSamp, p.zx * k, 0.0).rgb;
  let cz = textureSampleLevel(triTex, imgSamp, p.xy * k, 0.0).rgb;
...
fn envColor(rd: vec3f) -> vec3f {
  ...
  var sky = s2l(mix(${envx ? "mix(G.bgc.rgb * 0.35, G.zenU.rgb, G.zenU.w)" : "G.bgc.rgb * 0.35"}, G.bgc.rgb, tg));
  sky = mix(sky * (1.0 - G.env.z), sky, smoothstep(-0.35, 0.12, rd.z)); // ground dim
${envx ? "  sky = sky + starField(rd) * smoothstep(-0.35, 0.12, rd.z); // stars set below the horizon\n" : ""}${envMap ? "  // IMGTEX (#631): the image replaces the procedural sky by its amount HERE,\n  // inside the one choke point — so IBL ambient (env.w), GGX reflections\n  // (env.x) and the fog target all see the same image world as the background.\n  sky = mix(sky, envImage(rd), clamp(G.emapU.x, 0.0, 1.0));\n" : ""}  let sunAmt = pow(max(dot(rd, normalize(G.light.xyz)), 0.0), 24.0);
...
${neonDecl}fn surfaceAlbedo(p: vec3f, nrm: vec3f, t: f32) -> vec3f {
  ...
  if (G.scene.x > 0u) {
    ...
    albedo = orbitPainter(p);${neonPainterLine}
  } else {
    ...
    var m = normSig(mixT);
${neonSigLine}    if (G.prm.w > 0.5 && G.prm.w < 1.5 && G.p3ctl.x > 0.001) {
...
${tanFDecl}  // TILED_EXPORT §2.1.3 — the off-axis window, ×scale + bias on the plane
...
${srHead}
  ...
  var overshoot = 0.0; // debug #370 — last-step span vs the eps·t hit shell${
    seamUsed
      ? `
  // #633 — seam-clamped march, BAND-GATED (the v1.4 "not sharpening" field
  // regression fix). The DE value is UNTOUCHED (hit test, the bisection
  // below, normals, shadows, AO and capture all read the unclamped d — the
  // seam-clamp contract, operators.js id 67); the STEP brakes to the tear
  // distance mapDE just published, but ONLY while that bound is ACTIONABLE —
  // seamOut in [d/4, d), where braking actually lands the ray short of the
  // tear. Below d/4 the bound is degenerate (a tear at iteration k has
  // exponentially many fold-preimages, so the running-min seam is ~0 along
  // the ENTIRE approach to the attractor) and the old d/4 FLOOR there bought
  // no crossing guarantee — it stepped d/4 across the tear regardless —
  // while quartering the pace of most of the march. Measured on the two
  // 2026-08 field-report scenes (owner share links, M4 + Chrome/Metal,
  // 1100×800): floored clamp 130-190 ms settle → accumCap()=0, idle
  // refinement PERMANENTLY off, and the un-adjusted step budget exhausted
  // into the softA fog wash; band-gated 72-82 ms → refinement alive, wash
  // gone, picture byte-comparable to the unclamped reference (<0.7% px).
  // The eps·t floor history still stands: an ABSOLUTE floor made seam
  // formulas crawl (3759 steps vs 3 plain on Hinged Bastion — near-black at
  // 6 fps); the band-gate keeps the d-relative shape and simply refuses to
  // pay for protection the degenerate bound cannot deliver.
  // membraneT records the first crossing for the debug view (p3ctl.z mode 4).
  var membraneT = -1.0;
  for (steps = 0u; steps < maxSteps; steps = steps + 1u) {
    ...
    if (seamOut < eps * t && membraneT < 0.0) { membraneT = t; }
    tPrev = t;
  ...
  if (hit && t > tPrev) {
    ...
    for (var r = 0u; r < 8u; r = r + 1u) {
      let mid = 0.5 * (lo + hi);
      if (${clipDEMid} < eps * mid) { hi = mid; } else { lo = mid; }
    }
    t = hi;
  }
...
${
  envx
    ? `  // ENVX: stars are NOT hostage to the Sky blend. envColor's star term is
  // scaled by env.x above, so add the complement here — total star brightness
  // is the Stars slider alone at ANY Sky (field report: Sky 0.09 crushed the
  // field to 9%). Reflections/IBL keep the sky-scaled envColor version.
  bgOut = bgOut + starField(rd) * smoothstep(-0.35, 0.12, rd.z) * (1.0 - clamp(G.env.x, 0.0, 1.0));
`
    : ""
}${
      envMap
        ? `  // IMGTEX (#631): like the ENVX stars, the image is NOT hostage to the Sky
  // blend — envColor's image mix reaches bgOut only scaled by env.x, so mix
  // the (1 − sky) complement here: the background shows the image at emapU.x
  // whether or not the Sky slider is up (and at Sky 1 this adds nothing).
  bgOut = mix(bgOut, envImage(rd), clamp(G.emapU.x, 0.0, 1.0) * (1.0 - clamp(G.env.x, 0.0, 1.0)));
`
        : ""
    }${
      aurora
        ? `  // AURORA (ENVX P6): added at the background COMPOSITE, full amount at any
  // Sky (nothing to be hostage to — envColor never carries it, so no
  // complement math is needed). bgOut is also the fog target, so fogged hits
  // fade toward the same aurora sky and no silhouette seam appears; the
  // IBL/reflection env taps stay fbm-free (the divergence at auroraSky).
  bgOut = bgOut + auroraSky(rd);
`
        : ""
  ...
  if (!hit) {
    ...
    // ones — only genuine sky rays become transparent.${
      seamUsed
        ? `
    // #633 fold-membrane view (p3ctl.z mode 4): a missed ray that crossed a
    // seam still paints the translucent sheet over the sky, so membranes in
    // empty space are visible to the op author too.
    if (G.p3ctl.z > 3.5 && membraneT >= 0.0) {
      return vec4f(mix(skyOut, s2l(vec3f(0.25, 0.85, 1.0)), 0.55), 1.0);
    }`
        : ""
    }
${srMiss}
...
${
    clip
      ? `  // CLIP — cut-face classification (clip variants only). The plane term
  // binding the max at the landed hit (planeD >= the scaled DE) means this
  // pixel is the CROSS-SECTION, not the fractal shell: an interior DE
  // gradient is noise, so the normal becomes the plane normal, flipped to
  // face the ray (the cut is only ever seen from the clipped side, but the
  // select keeps a grazing ray finite).
  let nrm0 = calcNormal(p, t);
  let clipPl = ${clipPlaneAt("p")};
  let cutFace = hit && clipPl >= mapDE(p) * G.prm.z;
  let clipN = normalize(G.clipU.xyz) * select(1.0, -1.0, dot(rd, G.clipU.xyz) > 0.0);
  let nrm = select(nrm0, clipN, cutFace);`
  ...
  if (dbg > 0.5) {
    ...
    if (dbg < 1.5) {
      ...
      // 4 — #633 fold membrane (emitted only for seam op-sets, like the whole
      // channel): neutral facing-shade for the hit surface, with the seam
      // sheet blended over any ray that crossed a membrane on the way in —
      // "exactly where space is glued". Author-facing (?diag), WebGPU tier.
      let base = vec3f(0.2 + 0.6 * max(dot(nrm, -rd), 0.0));
      let mem = select(0.0, 0.55, membraneT >= 0.0);
      return vec4f(s2l(mix(base, vec3f(0.25, 0.85, 1.0), mem)), 1.0);
    }`
        : `{
      // 3 — ∇DE instability: |∇DE| deviation from 1 (unit gradient = clean SDF).
      hv = clamp(abs(deGradMag(p, t) - 1.0), 0.0, 1.0);
  ...
  var occ = 1.0;
  if (G.mat.w > 0.0${clip ? " && !cutFace" : ""}) { occ = calcAO(p, nrm, t); }
  var sh = 1.0;
  if (G.mat.z > 0.5${clip ? " && !cutFace" : ""}) { sh = 0.15 + 0.85 * softShadow(p, nrm, lightDir, t, G.mat.y); }
...
${
  surfTex
    ? `  // IMGTEX (#631): blend the triplanar image over the albedo (amount 1 = the
  // image IS the surface; partial amounts keep every coloring mode visible
  // under it). Anchored in WORLD space (p is the recentered position, so add
  // the offset back) — the pattern sticks to the surface while the camera
  // pans. Deliberately AFTER surfaceAlbedo, not inside it: that fn is shared
  // with the capture fs, which stays texture-free.
  albedo = mix(albedo, triplanarImage(p + G.offset.xyz, nrm), clamp(G.triU.x, 0.0, 1.0));
`
    : ""
}
...
${srSplit}  // P3 env reflections: the GGX env term samples the sky along the reflection
  ...
  if (${srEnvGuard}G.env.x > 0.0 && specAmt > 0.0) {
  ...
  col = col * G.lprm.w;
${cutShade}${filmMod}${neonEmit}  // P3 fog: 0 = the legacy 0.6·t/tFar fade EXACTLY; > 0 = exponential fog
...
export const GLOBALS_WORDS = 48; // BASE struct vec4 count (…offsetLo=45, tile=46, tilepx=47).
// Also the post shader's PG array size — envx rows must stay OUT of it, or the
// post shader text (and every non-envx march struct) changes for everyone.
// ENVX (backgrounds P5) — the three tail rows, present only in envx variants.
// The globals BUFFER is always allocated at the ALLOC ceiling and the tail
// words always written; non-envx structs are smaller than the buffer, which
// WebGPU permits (binding size ≥ struct size).
export const ENVX_WORD = 48; // vec4 index of `starsU` (48), `bandU` (49), `zenU` (50)
// IMGTEX (#631) — two more tail rows AFTER the ENVX tail (append, never
// insert). Present only in envMap/surfTex variants — which therefore also
// carry the ENVX rows as dormant padding, so these words keep fixed offsets
// against the one buffer layout. Same always-written / read-only-by-its-
// variants contract as the ENVX tail.
export const EMAP_WORD = 51; // vec4 index of `emapU` (51) and `triU` (52)
// AURORA (ENVX P6) — three more tail rows AFTER the IMGTEX tail (append,
// never insert). Present only in aurora variants — which therefore also carry
// the ENVX + IMGTEX rows as dormant padding, so these words keep fixed
// offsets against the one buffer layout. Same always-written / read-only-by-
// its-variants contract as both tails above.
export const AUR_WORD = 53; // vec4 index of `aurU` (53), `aurA` (54), `aurB` (55)
// CINE GRADE (cinematic looks) — four more tail rows, read ONLY by the graded
// post-pass variant (buildPostWGSL({ grade: true })); no march struct READS
// them (clip march variants declare them as dormant padding so the clip rows
// at 60+ keep their offsets — see CLIP_WORD). At 56, AFTER the aurora rows:
// #671 parked the grade rows here
// while aurora (PR #667) was in flight so the two appends stayed
// collision-free whichever landed first — both are in now, so 53..55 are
// aurora's and 56..59 are the grade's.
export const GRADE_WORD = 56; // vec4 index of `gradeA` (57 gradeB, 58 gradeC, 59 gradeD)
// CLIP (cross-section) — two more tail rows AFTER the grade rows (append,
// never insert). Present only in clip MARCH variants — which therefore also
// carry the ENVX/IMGTEX/AURORA rows AND the grade rows (56..59, post-pass
// words a march shader never reads) as dormant padding, so these words keep
// fixed offsets against the one buffer layout. Same always-written /
// read-only-by-its-variants contract as every tail above. Allocation ledger
// (2026-08-27): clip holds rows 60..61 of its 60..62 claim (62 spare);
// row 63 is RESERVED for the in-flight 360°/equirect arm — do not take it.
export const CLIP_WORD = 60; // vec4 index of `clipU` (61 clipS)
export const GLOBALS_WORDS_ALLOC = 62; // ceiling (base 48 + envx 3 + imgtex 2 + aurora 3 + grade 4 + clip 2)
// `grade` (CINE GRADE — the cinematic-look color pass): false — the default,
// and what the renderer builds at boot — emits the pre-GRADE post shader
// byte-for-byte (core/gradegate.test.mjs pins it; the neon/#125 "prove it's
// free" standard). true appends the grade block: a data-driven display-referred
// color transform (S-curve, shadow-weighted saturation, split-tone, duotone,
// per-look vignette) parameterized entirely by the gradeA..gradeD tail words —
// named looks are parameter SETS (app/src/looks.ts), never emitter forks.
//
// PLACEMENT — post-tonemap, on display-linear [0,1], deliberately:
//  - the P3 bloom composite adds HDR brights PRE-tonemap and the filmic
//    shoulder then rolls them off; grading after preserves those halo
//    gradients exactly (a pre-tonemap grade would tint/clip unbounded HDR
//    values before the shoulder ever sees them),
//  - the luminance masks below (shadow desat, split-tone balance) need a
//    bounded, exposure-stable domain — luma thresholds are meaningless in
//    unbounded HDR,
//  - it mirrors the film pipeline: scene-referred tonemap first, creative
//    display-referred grade second (how LUTs are applied).
export function buildPostWGSL({ grade = false } = {}) {
  return `
struct PG { w : array<vec4f, ${grade ? GRADE_WORD + 4 : GLOBALS_WORDS}> };
...
${
  grade
    ? `  // ── CINE GRADE (cinematic looks) ─────────────────────────────────────────
  // Emitted ONLY on grade builds (an active look); the base post shader stays
  // byte-identical. Display-referred (post-tonemap) — see buildPostWGSL's
  // header for the placement rationale. Everything below is a pure function of
  // the pixel's own color EXCEPT the vignette, which reads gpos/gres — the
  // ABSOLUTE full-image coordinates — so tiled exports stay seam-free
  // (TILED_EXPORT §2.2.1(a), the same contract as the post.w vignette above).
  let gA = G.w[${GRADE_WORD}];     // x strength · y S-curve contrast · z saturation · w shadow desat
  let gB = G.w[${GRADE_WORD + 1}]; // rgb dark/shadow tone color (LINEAR) · w split-tone amount
  let gC = G.w[${GRADE_WORD + 2}]; // rgb bright/highlight tone color (LINEAR) · w duotone amount
  let gD = G.w[${GRADE_WORD + 3}]; // x per-look vignette · yzw reserved
  let gBase = clamp(c, vec3f(0.0), vec3f(1.0));
  // 1. Gentle S-curve (pivot 0.5): smoothstep blend — exact identity at 0/1,
  //    so blacks and whites hold while midtone contrast deepens.
  var g = mix(gBase, gBase * gBase * (3.0 - 2.0 * gBase), gA.y);
  let gLum = dot(g, vec3f(0.2126, 0.7152, 0.0722));
  // 2. Saturation, eased off in the shadows by gA.w (filmic shadow desat —
  //    band capped at 0.45 luma so midtones keep their color).
  let gSat = gA.z * (1.0 - gA.w * (1.0 - smoothstep(0.0, 0.45, gLum)));
  g = mix(vec3f(gLum), g, gSat);
  // 3. Split tone: multiplicative ×2 against a luma-blended tone color —
  //    0.5-gray tones are an exact identity, blacks stay black. The band is
  //    deliberately steep (0.25..0.65): typical scene lumas sit mid-range, and
  //    a wide band left most pixels on the neutral midpoint (no split at all).
  let gTone = mix(gB.rgb, gC.rgb, smoothstep(0.25, 0.65, gLum));
  g = mix(g, clamp(g * gTone * 2.0, vec3f(0.0), vec3f(1.0)), gB.w);
  // 4. Duotone gradient map (Noir): luma → dark→bright endpoint, driven
  //    through pow(luma, 0.6) — a raw-luma map parked every pixel mid-gradient
  //    on midtone scenes (poster mud) and a fixed smoothstep band crushed dim
  //    scenes to the dark endpoint (both probe findings); the gamma lift
  //    spreads dim scenes into the gradient without clipping bright ones.
  g = mix(g, mix(gB.rgb, gC.rgb, pow(gLum, 0.6)), gC.w);
  c = mix(gBase, g, gA.x);                               // one strength slider
  if (gD.x > 0.0) { // per-look vignette — full-image coords, tile-seam-safe
    let gvd = distance(gpos / gres, vec2f(0.5)) * 1.41421;
    c = c * (1.0 - gD.x * gA.x * smoothstep(0.30, 1.05, gvd));
  }
`
    : ""
}  var o = l2s(c);
...
@fragment fn fs(@builtin(position) pos: vec4f) -> @location(0) vec4f {
  let px = vec2i(pos.xy);
  let w = clamp(G.w[${JITTER_WORD}].z, 0.0, 1.0);
  let c = textureLoad(cur, px, 0);
  // Weight 1 must REPLACE outright, not mix: mix(p, c, 1.0) is p*0 + c, and
  // 0*NaN/Inf is NaN — one poisoned history pixel (a NaN march sample, or an
  // uninitialized/recycled read on a flaky driver — the "green frame" class,
  // see preview.js captureFrame) would otherwise survive EVERY fresh settled
  // base frame forever. The early return makes each settle a guaranteed
  // clean slate.
  if (w >= 1.0) { return c; }
  // Refinement ticks (w < 1): clamp history to the HDR pipeline's own range
  // (colors non-negative, rgba16float-bounded upstream; alpha 0..1) so a
  // non-finite prev can at worst tint this one sequence, never dominate it.
  let p = clamp(textureLoad(prev, px, 0), vec4f(0.0), vec4f(65504.0));
  return mix(p, c, w);
}
`;
}
...
function exportShapeDE(formula, obj, leaf) {
  ...
  // #627 — a fat leaf's body reads prm2 (the objAux lane); bake it as a second
  // literal. Thin leaves emit the exact legacy text (no prm2 line).
  const fatLeaf = (leaf.params?.length ?? 0) > 4;
  const prm2 = (obj.shapeParams || []).slice(4, 8);
  while (prm2.length < 4) prm2.push(0);
...
${fatLeaf ? `  vec4 prm2 = vec4(${prm2.map(glslNum).join(", ")});\n` : ""}  ${leaf.glsl}
}
`;
}
