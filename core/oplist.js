...
const BULB_Y = {
  ...
  iters: 8,
  deOption: 0,
  ops: [{ key: "bulbAxis", values: [8.0, 1.0, 0.0, 1.0, 1.0] }],
  camera: { yawDeg: 35, pitchDeg: 12, dist: 5.0, fovDeg: 42 },
};
...
const NORM_BULB = {
  ...
  iters: 8,
  deOption: 0,
  ops: [{ key: "bulbAxis", values: [8.0, 2.0, 1.0, 1.0, 1.0] }],
  camera: { yawDeg: 35, pitchDeg: 12, dist: 5.0, fovDeg: 42 },
};
...
const SINE_BULB = {
  ...
  iters: 8,
  deOption: 0,
  ops: [{ key: "bulbAxis", values: [8.0, 0.0, 2.0, 1.0, 1.0] }],
  camera: { yawDeg: 35, pitchDeg: 12, dist: 5.0, fovDeg: 42 },
};

// bulbAxis SPLIT POWERS (the trig-bulb wave, OP_PARAM_ENCODING.md PR-2): the
// asin-latitude convention with the POLAR angle powered at HALF the azimuth's
// (ThetaMul 0.5, PhiMul 1). A convention combined with an untied angle power is
// precisely the cross TRIGBULB_SPIKE.md:73-74 booked as an accepted loss — op 29
// held axis × convention at one tied power, op 37 held split powers but only
// z-polar and cos-only, and neither had a free slot to hold both. Halving the
// polar power stops the lobes closing over the poles, so the body stretches
// top-to-bottom into a bilaterally symmetric carved totem instead of the Sine
// Bulb's knobbly sphere — a different form, not a re-lit one.
// Verified on real WebGPU AND on ?renderer=webgl2 (same silhouette on both).
const TOTEM_BULB = {
  name: "Totem Bulb",
  note: "asin-latitude bulb with the polar angle at half power — lobes stretched into a carved totem",
  addC: true,
  iters: 8,
  deOption: 0,
  ops: [{ key: "bulbAxis", values: [8.0, 0.0, 2.0, 0.5, 1.0] }],
  camera: { yawDeg: 35, pitchDeg: 12, dist: 5.0, fovDeg: 42 },
};

// ⚠ ONE preset ships for the split powers, not a family — the #86 rule. The
// full cross was swept on the CPU oracle (3024 recipes: Power 2-12 × Axis ×
// Convention × ThetaMul × PhiMul over ±0.25-2) and eight survivors rendered on
// real WebGPU. ⚠ Worth knowing before re-running it: the oracle does NOT
// discriminate here — every cell scores wobble 0.244-0.272 against the shipped
// bulbs' own 0.257-0.258, and `sharpness` is undefined for this family. It
// gates soundness only; the choice is visual. What the other cells looked like:
//   • ThetaMul alone at axis 0 / Convention 0 is sphericalTwoStage's territory
//     exactly (that IS the subsumption anchor) — "Twin Bulb" already ships it.
//   • PhiMul ≤ 0.25 (e.g. 8, 0, 2, 1, 0.25) nearly kills the azimuthal
//     variation, leaving a smooth solid of revolution — a handsome turned
//     spindle, but it reads as lathe work rather than a fractal.
//   • ThetaMul ≥ 1.5 or PhiMul ≥ 2 at any convention (C/D/H in the sweep) pack
//     the surface with high-frequency knobs that fill the silhouette — the
//     "dense everywhere, no sky" failure the ruckerBulb note describes.
//   • Convention 1 (sin-polar) with a split power (A) melts into a doughy mass
//     with no readable structure.
//   • Negative PhiMul just mirrors the winding — the spike's isometry ruling
//     (TRIGBULB_SPIKE.md:39-45), so it is a recipe, not a distinct body.

// ruckerBulb Convention 3 — the SIGNED planar polar angle (atan2(-x,z) rather
// than the unsigned acos(z/r)). This is the one flavor TRIGBULB_SPIKE.md:65-72
// measured on the MB3D `Ruckerbulb` file and then had to record as an ACCEPTED
// LOSS, because the ABI had three param slots and the op wanted five; it is the
// first thing the opAux overflow lane bought back (OP_PARAM_ENCODING.md).
// The signed angle sweeps a full turn where acos folds at the poles, so the
// lobes wind into a single off-axis pinwheel focus instead of the classic
// bulb's mirror symmetry — a genuinely different body, not a re-lit Mandelbulb.
// Verified on real WebGPU AND on ?renderer=webgl2 (same silhouette on both).
const PINWHEEL_BULB = {
  name: "Pinwheel Bulb",
  note: "signed-planar-angle bulb — the Ruckerbulb convention, lobes wound into one pinwheel focus",
  addC: true,
  iters: 8,
  deOption: 0,
  ops: [{ key: "ruckerBulb", values: [8.0, 1.0, 1.0, 0.0, 3.0] }],
  camera: { yawDeg: 35, pitchDeg: 12, dist: 5.0, fovDeg: 42 },
};

// ⚠ ONE preset ships for ruckerBulb, not four — the #86 rule. All five params
// were swept on the CPU oracle (2688 recipes: Power 2-12 × AziPow 0.5-3 ×
// ZMul ±0.5-1.5 × RadialSel × Convention) and the survivors rendered on real
// WebGPU. What the other dof actually looked like, so the next person does not
// re-run the sweep:
//   • Convention 0/1/2 are bulbAxis's own conventions, reproduced EXACTLY (that
//     is the degeneracy anchor, pinned in core/ruckerbulb.test.mjs). Shipping
//     them here would duplicate Bulb (Y-axis) / Norm Bulb / Sine Bulb.
//   • AziPow alone (e.g. 8, 1.5, 1, 0, 0) renders crisply but reads as a
//     Mandelbulb with more lobes — sphericalTwoStage's territory, and its
//     "Twin Bulb" preset already covers split-angle-power character.
//   • ZMul < 1 squashes the poles into flat horizontal terraces (measured at
//     8, 1, 0.5, 0, 2); correct, and legible as a knob, but the banding reads
//     as an artifact rather than a form. Not gallery work.
//   • RadialSel = 1 is INERT at the AziPow = 1 default and only bites at an
//     untied power, where it drives the radial exponent to Power×AziPow; the
//     oracle's high scorers there (Power 2-3, AziPow 0.5) are near-solid masses
//     that fill the frame with no silhouette — the same "dense everywhere, no
//     sky" failure the brickFold note below describes.
// The op is correct, pinned in all three emitters (core/ruckerbulb.test.mjs)
// and reachable from the Build picker; the culled dof are recorded here rather
// than shipped half-good.
...
// (#633 update: modFold's OTHER caveat — the marcher stepping through cell
// walls when a body crossed its cell — is gone: modFold now reports its wall
// distance into the seam channel and the march step is clamped by it. That
// fixes CORRECTNESS at the walls; it does not touch this density problem,
// which remains the reason no modFold/brickFold preset ships. brickFold does
// not yet declare a seam term — recorded as #633 follow-up work.)
  ...
  objects: [
    {
      ...
      // #353 r7: rot was [-90,0,0], compensating for the leaf's inverted axis
      // remap; the leaf now faces upright on its own, so no rotation.
      transform: { origin: [0, 0, 0], uscale: 1.0, rot: [0, 0, 0] },
      combine: 0,
      blendK: 0,
    },
...
const HYBRID_BULB_TRIO = {
  ...
  hybrid: {
    slots: [
      ...
      {
        ops: [{ key: "bulbAxis", values: [5.0, 1.0, 0.0, 1.0, 1.0] }],
        addC: true,
      },
    ],
...
// ── Preset wave 2026-08-10 — category-balance pass ───────────────────────────
// Kleinian (3), Scenes (4) and Hybrids (5) were thin next to Boxes/Bulbs (22
// each); this wave adds a Kleinian look, a scene, a hybrid, a cage, and ships two
// covered-but-unshipped parity capabilities as presets (the kaleido mirror-off
// sector snap; the bulbAxis convention that _SinePow2 measured out to). Every
// entry passed the #86 framing bar via baked-thumb review on a real GPU.

const KLEINIAN_CORRIDOR = {
  name: "Kleinian Corridor",
  note: "Maskit limit set, bent hard — looking along the corridor wall (approximate DE)",
  ops: [],
  iters: 8,
  deOption: 2,
  addC: false,
  camera: { yawDeg: 95, pitchDeg: 16, dist: 1.05, fovDeg: 66 },
  objects: [
    {
      shapeId: 58,
      shapeParams: [2.02, 0.03, 0.3, 64],
      ops: [],
      iters: 1,
      color: [0.55, 0.66, 0.78],
      transform: { origin: [0, 0, 0], uscale: 1.0, rot: [0, 0, 0] },
      combine: 0,
      blendK: 0,
    },
...
const GILDED_CAGE = {
  name: "Gilded Cage",
  note: "box · sphere fold · scale ×2 · deep Z offset (no +c) — an ornate hollow cage",
  addC: false,
  iters: 12,
  deOption: 2,
  ops: [
    { key: "boxFold", values: [1.0] },
    { key: "sphereFold", values: [0.5, 2.3] },
    { key: "scale", values: [2.0] },
    { key: "translate", values: [0.1, -0.3, -1.07] },
  ],
  camera: { yawDeg: 48, pitchDeg: 16, dist: 20.0, fovDeg: 46 },
};

const HYBRID_MANDALAY_MENGER = {
  name: "Mandalay x Menger",
  note: "Mandalay terraces alternating with the Menger lattice each iteration (hybrid, IFS×IFS)",
  ops: [
    { key: "mandalayFold", values: [0.5, 0.1, 0.0] },
    { key: "scale", values: [2.0] },
    { key: "translate", values: [-1.0, -1.0, 0.0] },
  ...
  iters: 12,
  deOption: 2,
  camera: { yawDeg: 40, pitchDeg: 18, dist: 7.0, fovDeg: 42 },
  hybrid: {
    b: {
      ops: [
        { key: "absFold", values: [] },
...
const STONE_STACK = {
  name: "Stone Stack",
  note: "five beach stones balanced and smooth-blended (CSG spheres)",
  ops: [],
  iters: 8,
  deOption: 2,
  addC: false,
  camera: { yawDeg: 26, pitchDeg: 5, dist: 7.5, fovDeg: 42 },
  objects: [
    {
      objType: 2,
      primParam: 1.0,
      color: [0.62, 0.58, 0.54],
      transform: { origin: [0, 0, -2.0], uscale: 1.0, rot: [0, 0, 0] },
      combine: 0,
    },
    {
      objType: 2,
      ...
      transform: { origin: [0.12, 0.05, -0.5], uscale: 1.0, rot: [0, 0, 0] },
      combine: 1,
      blendK: 0.12,
    },
    {
      ...
      primParam: 0.55,
      color: [0.55, 0.52, 0.5],
      transform: { origin: [-0.08, -0.04, 0.6], uscale: 1.0, rot: [0, 0, 0] },
      combine: 1,
      blendK: 0.1,
    },
    {
      ...
      transform: { origin: [0.06, 0.08, 1.4], uscale: 1.0, rot: [0, 0, 0] },
      combine: 1,
      blendK: 0.08,
    },
    {
      ...
      primParam: 0.28,
      color: [0.5, 0.47, 0.46],
      transform: { origin: [-0.03, 0.02, 1.98], uscale: 1.0, rot: [0, 0, 0] },
      combine: 1,
      blendK: 0.06,
    },
  ],
...
const KOCH_STAR = {
  name: "Koch Star",
  note: "pure sector snap (kaleido, mirror OFF) + box fold + scale + Z drop (no +c) — snowflake towers",
  addC: false,
  iters: 12,
  deOption: 2,
  ops: [
    { key: "kaleido", values: [6.0, 0.0, 0.0] },
    { key: "absFold", values: [] },
    { key: "scale", values: [3.0] },
    { key: "translate", values: [-2.0, 0.0, 0.0] },
  ],
  camera: { yawDeg: 8, pitchDeg: 74, dist: 4.2, fovDeg: 42 },
};

// #635 Catoptron parity — the spiral-vortex showcase. Viewed down the spiral
// axis so the log-whorl reads immediately (CPU-harness render-verified at
// this exact camera). No mirrorShells preset ships with #635: that op tiles
// ALL of space radially (an interior/fly-through experience by construction),
// so a framed exterior thumbnail cannot exist — curating an interior view is
// a live-browser task, deferred.
//
// ⚠ NO PRESET SHIPS FOR #632 (wallpaperFold / spaceGroupFold) EITHER — the
// same verdict for the same STRUCTURAL reason as modFold, brickFold and
// mirrorShells before it, now with a fourth data point. A space-group fold
// tiles ALL of space with the cell, so (a) run per-iteration under a scale, it
// re-captures every escaping point into the chamber — r stays bounded while w
// grows, DE → 0, and the marcher sees the "dense everywhere, no sky, no
// silhouette" fill (CPU-render-verified this wave: cubic and hexagonal
// Menger-motif recipes at cells 1.6–2.4, iters 12–14, all 100% hit fraction —
// flat fill from every camera); and (b) at iters = 1 the formula IS the exact
// distance field to the bare crystallographic point lattice (|f(p)| =
// d(p, orbit(0)) — the fold is the orbit-representative map), which renders
// measure-zero dots. The op pair is correct, proof-gated in all three emitters
// (core/spacegroup.test.mjs) and reachable from Build; an interior/fly-through
// curation — where an endless crystal colonnade genuinely reads — is the same
// live-browser task deferred for mirrorShells, one gallery arc for both.
const WHORL_CITADEL = {
  name: "Whorl Citadel",
  note: "a Menger citadel wound into a log-spiral whorl (spiralVortex, #635)",
  addC: false,
  iters: 14,
  deOption: 2,
  ops: [
    { key: "spiralVortex", values: [0.9, 1.0] }, // Amount, Axis (Y — upright whorl)
    { key: "absFold", values: [] },
    { key: "mengerFold", values: [] },
    { key: "scale", values: [3.0] },
    { key: "translate", values: [-2.0, -2.0, 0.0] },
  ],
  camera: { yawDeg: 25, pitchDeg: 65, dist: 8.0, fovDeg: 42 },
};

// #633 Catoptron parity — the hinge-fold showcase. A Mandelbox lattice with a
// hinge cut through it: the swung half shears the box grid into a diagonal
// staircase of bent cells — the "architectural, bent-space" look the op
// exists for. CPU-harness render-verified at this exact camera (full
// silhouette, clear shear line). Note WHY this ships a preset when
// modFold/brickFold could not (the ⚠ blocks above): hingeFold is NOT a domain
// repetition — one cut, no per-iteration lattice densification — so the
// once-per-iteration trap does not apply; the seam channel (the #633
// contract) is what makes marching the tear exact.
const HINGED_BASTION = {
  name: "Hinged Bastion",
  note: "a Mandelbox bastion cut and swung on a hinge line (hingeFold, #633)",
  addC: false,
  iters: 10,
  deOption: 2,
  ops: [
    { key: "boxFold", values: [1.0] },
    { key: "sphereFold", values: [0.5, 1.0] },
    { key: "scale", values: [2.2] },
    { key: "hingeFold", values: [35.0, 0.0, 1.0, 1.1] }, // FoldAngle, CutAngle, Axis(Y), Offset
  ],
  camera: { yawDeg: 20, pitchDeg: 70, dist: 20, fovDeg: 42 },
};

// #634 Catoptron parity — the City leaf showcase (leaf 59): hashed two-tier
// skyline on a street grid, intersect-bounded into a round district (the
// Frame-Nest rule for infinite fields; a CYLINDER bound so rim buildings get
// clean vertical slices instead of the sphere's shark-fin shaves — CPU-harness
// render-verified at this exact camera).
const CITY_BLOCKS = {
  name: "City Blocks",
  note: "procedural building lattice, neighborhood-true DE (city leaf, #634)",
  ops: [],
  iters: 8,
  deOption: 2,
  addC: false,
  camera: { yawDeg: 40, pitchDeg: 24, dist: 5.4, fovDeg: 42 },
  objects: [
    {
      shapeId: 59,
      shapeParams: [0.5, 0.3, 1.3, 0.8, 0.6, 3],
      ops: [],
      iters: 1,
      color: [0.8, 0.66, 0.5],
      transform: { origin: [0, 0, 0], uscale: 1.0, rot: [0, 0, 0] },
...
// ── Catoptron feature-showcase wave (#630-#636 wrap-up) ─────────────────────
// Four presets illustrating the parity ops that landed without a showcase.
// All four are object presets (per-object colors); every camera was CPU-
// harness render-verified at these exact settings.

// mirrorShells (#635) finally gets its preset: the op tiles ALL of space
// radially, so the Frame-Nest rule applies twice — an intersect sphere ends
// the infinite onion, and a subtracted corner box cuts it open so the nested
// shells actually read from outside.
const PEARL_ONION = {
  name: "Pearl Onion",
  note: "concentric mirror shells cut open like an onion (mirrorShells, #635)",
  ops: [],
  iters: 8,
  deOption: 2,
  addC: false,
  camera: { yawDeg: 32, pitchDeg: 18, dist: 6.0, fovDeg: 42 },
  objects: [
    {
      shapeId: 2, // sphere seed — every shell is its folded image
      shapeParams: [0.2, 0, 0, 0],
      ops: [{ key: "mirrorShells", values: [0.62, 0] }],
      iters: 1,
      color: [0.85, 0.8, 0.7],
      transform: { origin: [0, 0, 0], uscale: 1.0, rot: [0, 0, 0] },
      combine: 0,
    ...
    {
      shapeId: 2, // Frame-Nest bound: the onion is infinite without it
      shapeParams: [2.0, 0, 0, 0],
      ops: [],
      iters: 1,
      color: [0.85, 0.8, 0.7],
    ...
    {
      shapeId: 1, // corner bite — exposes the interior shells
      shapeParams: [1.9, 1.9, 1.9, 0],
      ops: [],
      iters: 1,
      color: [0.85, 0.8, 0.7],
      transform: { origin: [1.2, 1.2, 1.2], uscale: 1.0, rot: [0, 0, 0] },
      combine: 2,
      blendK: 0,
    },
  ],
...
// spiralVortex (#635) x city leaf (#634): the log-spiral rotation sweeps the
// street grid into a hurricane of towers — the shear that the op's DE factor
// (|a|+sqrt(a^2+4))/2 exists to pay for, made visible.
const WHORL_CITY = {
  name: "Whorl City",
  note: "a city grid swept into a log-spiral hurricane (spiralVortex x city)",
  ops: [],
  iters: 8,
  deOption: 2,
  addC: false,
  camera: { yawDeg: 30, pitchDeg: 50, dist: 6.0, fovDeg: 42 },
  objects: [
    {
      shapeId: 59,
      shapeParams: [0.5, 0.3, 1.3, 0.8, 0.6, 3],
      ops: [{ key: "spiralVortex", values: [2.0, 1] }],
      iters: 1,
      color: [0.8, 0.66, 0.5],
      transform: { origin: [0, 0, 0], uscale: 1.0, rot: [0, 0, 0] },
      combine: 0,
    ...
    {
      shapeId: 4, // round district bound (the City Blocks rule)
      shapeParams: [2.2, 1.5, 0, 0],
      ops: [],
      iters: 1,
      color: [0.8, 0.66, 0.5],
...
// spaceGroupFold (#632): P4/mmm replicating one box frame into a tetragonal
// lattice, intersect-bounded to a ball — a Bravais cell you can orbit.
const BRAVAIS_CAGE = {
  name: "Bravais Cage",
  note: "one box frame tiled by the P4/mmm space group (spaceGroupFold, #632)",
  ops: [],
  iters: 8,
  deOption: 2,
  addC: false,
  camera: { yawDeg: 24, pitchDeg: 18, dist: 5.4, fovDeg: 42 },
  objects: [
    {
      shapeId: 27, // boxFrame — the lattice motif
      shapeParams: [0.38, 0.055, 0, 0],
      ops: [{ key: "spaceGroupFold", values: [1, 1.15, 1.55, 0] }],
      iters: 1,
...
// wallpaperFold (#632's 2D cousin): the fold tiles the x-y plane — which is
// VERTICAL in scene space, so the honest showcase is literally wallpaper: a
// brilliant-cut gem kaleidoscoped by *632 into a faceted mosaic wall.
const JEWEL_WALLPAPER = {
  name: "Jewel Wallpaper",
  note: "a cut gem kaleidoscoped into a hex mosaic wall (wallpaperFold *632)",
  ops: [],
  iters: 8,
  deOption: 2,
  addC: false,
  camera: { yawDeg: 18, pitchDeg: 8, dist: 9.0, fovDeg: 42 },
  objects: [
    {
      shapeId: 51, // gem, the Cut Gem brilliant
      shapeParams: [8, 1, 0.35, 0.9],
      ops: [{ key: "wallpaperFold", values: [0, 2.2] }],
      iters: 1,
      color: [0.55, 0.8, 0.95],
      transform: { origin: [0, 0, 0], uscale: 1.0, rot: [0, 0, 0] },
      combine: 0,
    ...
    {
      shapeId: 1, // slab bound: a wall's worth of tiling
      shapeParams: [3.4, 3.4, 0.8, 0],
      ops: [],
      iters: 1,
      color: [0.55, 0.8, 0.95],
      transform: { origin: [0, 0, 0], uscale: 1.0, rot: [0, 0, 0] },
...
const LATITUDE_BULB = {
  name: "Latitude Bulb",
  note: "power-2 sine bulb with squared latitude — the SinePow2 map (bulbAxis convention 2)",
  addC: true,
  iters: 10,
  deOption: 0,
  ops: [{ key: "bulbAxis", values: [2.0, 0.0, 2.0, 2.0, 1.0] }],
  camera: { yawDeg: 35, pitchDeg: 12, dist: 4.6, fovDeg: 42 },
};
...
export const PRESETS = [
  ...
  HYBRID_BULB_TRIO,
  HYBRID_QUAD_CITADEL,
  // 2026-08-10 category-balance wave
  KLEINIAN_CORRIDOR,
  GILDED_CAGE,
  HYBRID_MANDALAY_MENGER,
  STONE_STACK,
  KOCH_STAR,
  LATITUDE_BULB,
  WHORL_CITADEL,
  HINGED_BASTION,
  // #634 Catoptron parity wave
  CITY_BLOCKS,
  PEARL_ONION,
  WHORL_CITY,
  BRAVAIS_CAGE,
