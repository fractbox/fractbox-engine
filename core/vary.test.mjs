...
test('TOUCHY movers are jittered on a shorter leash', () => {
  ...
  // The leash is a FRACTION of the param's declared span, so derive the bound
  // from the registry instead of hard-coding it — this used to assert `<= 1.01`
  // off a [-2,2] span, and silently became a no-op check when #538 widened
  // translate to ±6 to cover the Cantor Rotations preset.
  const [p0] = OPERATORS.find((o) => o.key === 'translate').params;
  const leashed = 0.25 * (p0.max - p0.min);
  for (const v of j.ops[0].values)
    assert.ok(Math.abs(v) <= leashed * 1.01, `translate leash broken: ${v} > ${leashed}`);
  // …and the leash must actually BITE: a touchy op moves touchyScale× as far
  // as the same roll would move an untouched one. That relation is what the
  // test is named for, and it holds at any span.
  const loose = jitterParams(f, { spread: 1.0, touchyScale: 1.0, rng: () => 1 });
  assert.ok(
    Math.abs(j.ops[0].values[0]) < Math.abs(loose.ops[0].values[0]),
    'touchyScale did not shorten the nudge',
  );
});
...
test('a bounded-cyclic orbit (all probes escape, real surface) passes isSound', () => {
  // Regression (gate blindspot #4, found when the "Smooth Box" preset gate
  // went red on dev, 2026-07-12): smoothBoxFold re-clamps every iteration
  // while the ball fold + scale(2) push |p| past the bail radius between
  // clamps — every probe orbit reads "escaped" (escaped = 1.000 in EVERY
  // region), yet the DE holds a real surface (region 1.2 coverage ≈ 0.16,
  // wobble ≈ 0.30). Presence must accept rendered mass, not just bounded
  // orbits — else every preset of this orbit class is unrollable/unshippable.
  const smoothBox = {
    name: 'T',
    addC: true,
    iters: 12,
  ...
  assert.equal(isSound(smoothBox), true, 'bounded-cyclic IFS rejected by the gate');
});
