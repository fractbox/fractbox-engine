...
test("perturb flag emits the delta variant, and ONLY under the flag (PR-2)", () => {
  ...
  assert.ok(pt.includes("ptD = kr * ptD + dk *"), "rational-radial delta cases present");
...
test("perturb PR-3: the 7 orbit-signal sites carry the delta loop", () => {
  ...
  assert.equal((pt.match(/ptRi = ptRi \+ 1u/g) || []).length, 8, "8 pt loop sites");
  ...
  assert.ok(cap.includes("binding(6) var<storage, read> ptRecs"), "ptRecs at binding 6");
