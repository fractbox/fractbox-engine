...
// The largest singular value of the unit shear [[1,0],[b,1]] — the exact
// operator norm of a rank-1 shear of magnitude b. spiralVortex is the one op
// that CHARGES it (b = Amount, constant, so the charge is affordable); twist
// (b = |rate|·ρ) and cylinderFold's mid band (b = 2|z|/ρ) have the same
// Jacobian shape with an unbounded b, which is why #643 tagged them deApprox
// instead — deaudit.test.mjs's L_SHEAR gates their derived ceilings.
const shearNorm = (b) => (Math.abs(b) + Math.sqrt(b * b + 4)) / 2;

// #632 — the shared 2-D wallpaper reflection kernel (*632 hex / *442 square /
// *333 triangle), used by BOTH wallpaperFold and the hexagonal spaceGroupFold
// arms so the two ops cannot drift on the CPU leg (the testable one; their
// WGSL/GLSL twins are textual copies, pinned by core/spacegroup.test.mjs
// cross-op agreement on the z = 0 plane). Every step is an element of the
// wallpaper group — a mirror fold or a pure lattice translation — so the
// converged output is the unique chamber representative (derivation in
// operators.js). K = 3 passes: measured convergence bound is 2 over 300k
// samples out to r = 1000·cell (the closed-form prefold makes the bound
// distance-free), plus one pass of margin.
const SQRT3 = 1.7320508075688772;
function wallpaper2D(s, tile, a) {
  let x = s.x,
    y = s.y;
  if (tile === 1) {
    x = Math.abs(x - a * Math.round(x / a));
    y = Math.abs(y - a * Math.round(y / a));
    if (y > x) {
      const t = x;
      x = y;
      y = t;
    ...
    for (let k = 0; k < 3; k++) {
      x = Math.abs(x - a * Math.round(x / a));
      if (tile === 2) {
        y = y - h * Math.round(y / h);
        let d = (SQRT3 / 2) * y - 0.5 * x;
        if (d < 0) {
          x += d;
          y -= SQRT3 * d;
        }
        d = 0.5 * x + (SQRT3 / 2) * y - 0.5 * a;
        if (d > 0) {
          x -= d;
          y -= SQRT3 * d;
        }
      } else {
        y = Math.abs(y - h * Math.round(y / h));
        let d = (SQRT3 / 2) * x - 0.5 * y;
        if (d < 0) {
          x -= SQRT3 * d;
          y += d;
        }
...
export function applyOp(key, v, s) {
  ...
  switch (key) {
    ...
    case "cylinderFold": {
      ...
      const k =
        r2 < minR2 ? fixedR2 / minR2 : r2 < fixedR2 ? fixedR2 / r2 : 1.0;
      // #643 — the mid band is NOT conformal (the z lane rides k(ρ_xy)); its
      // true norm is k·L(2|z|/ρ). w carries the best-effort k only and the op
      // is deApprox-tagged — see operators.js id 27 for why charging the exact
      // field renders fog.
      s.x *= k;
      s.y *= k;
      s.z *= k;
    ...
    case "varyScale": {
      ...
      // #643 — the mid band stretches RADIALLY by k·(1−2γ): conformal only at
      // γ = 1, so w charges k·max(1, |1−2γ|). Derivation: operators.js id 32.
      const vsL =
        rp >= minR2 && rp < fixedR2 ? Math.max(1, Math.abs(1 - 2 * v[2])) : 1.0;
      s.x *= k;
      s.y *= k;
      s.z *= k;
      s.w *= k * vsL;
    ...
    case "twist": {
      // #643 — a z-shear of magnitude |rate|·ρ in disguise, true norm
      // L(|rate|·ρ). w stays untouched (the MB3D best-effort rule) and the op
      // is deApprox-tagged: operators.js id 22 has the derivation and the
      // measurement that ruled the exact charge out.
    ...
    case "modFold": {
      ...
      // #633 seam term — distance to the nearest cell wall (post-wrap
      // coordinate gives it directly: c/2 − |t'|; see operators.js id 17
      // derivation), pulled back by the running |w| at THIS op. Mirrors the
      // WGSL/GLSL exactly.
      let mfs = 1e30;
      if (v[0] > 0) mfs = Math.min(mfs, 0.5 * v[0] - Math.abs(s.x));
      if (v[1] > 0) mfs = Math.min(mfs, 0.5 * v[1] - Math.abs(s.y));
      if (v[2] > 0) mfs = Math.min(mfs, 0.5 * v[2] - Math.abs(s.z));
      if (mfs < 1e30)
        s.seam = Math.min(
          s.seam ?? Infinity,
          Math.max(mfs, 0) / Math.max(Math.abs(s.w), 1e-9),
        );
      break;
    }
    ...
    case "mirrorShells": {
      // #635 — radial tent fold past Offset; 1-Lipschitz (see operators.js
      // derivation), so no w change. Mirrors the WGSL/GLSL exactly.
      if (v[0] > 0) {
        const msr = Math.hypot(s.x, s.y, s.z);
        const msm = msr - v[1];
        if (msm > 0 && msr > 1e-12) {
          const msf = Math.abs(msm - v[0] * Math.round(msm / v[0]));
          const msk = (v[1] + msf) / msr;
          s.x *= msk;
          s.y *= msk;
          s.z *= msk;
        }
    ...
    case "spiralVortex": {
      // #635 — rotate the plane ⊥ axis by a·ln(r); the differential is a shear,
      // so w *= (|a| + sqrt(a²+4))/2 — the exact uniform operator norm (see
      // operators.js for the derivation its own gate corrected).
      const sva = Math.round(v[1] ?? 1);
      let svu = s.x,
        svv = s.z;
      if (sva === 0) {
        svu = s.y;
      ...
      const svr2 = svu * svu + svv * svv;
      if (svr2 > 1e-24) {
        const svt = v[0] * 0.5 * Math.log(svr2);
        const svc = Math.cos(svt),
          svs = Math.sin(svt);
        const svx = svu * svc - svv * svs;
        const svy = svu * svs + svv * svc;
        if (sva === 0) {
          s.y = svx;
          s.z = svy;
        } else if (sva === 2) {
    ...
    case "wallpaperFold": {
      // #632 — *632/*442/*333 wallpaper reflection tile in x,y (z free).
      // 1-Lipschitz chamber fold, w untouched (see operators.js derivation).
      const wt0 = Math.round(v[0] ?? 0);
      const wt = wt0 >= 0 && wt0 <= 2 ? wt0 : 0;
      const wa = v[1] ?? 0;
      if (wa > 0) wallpaper2D(s, wt, wa);
      break;
    }
    case "spaceGroupFold": {
      // #632 — Pmmm / P4/mmm / P6/mmm / P-6m2 / Pm-3m reflection fold into
      // one crystallographic chamber. Param collapse rule and the full
      // derivation live in operators.js; proofs in core/spacegroup.test.mjs.
      const sg0 = Math.round(v[0] ?? 4);
      const sg = sg0 >= 0 && sg0 <= 4 ? sg0 : 0;
      const sa = v[1] ?? 0;
      if (sa > 0) {
        let sc = v[2] ?? 0;
        if (sc <= 0) sc = sa;
        if (sg === 2 || sg === 3) {
          wallpaper2D(s, sg === 3 ? 2 : 0, sa);
          s.z = Math.abs(s.z - sc * Math.round(s.z / sc));
        } else if (sg === 1) {
          s.x = Math.abs(s.x - sa * Math.round(s.x / sa));
          s.y = Math.abs(s.y - sa * Math.round(s.y / sa));
          if (s.y > s.x) {
            t = s.x;
            s.x = s.y;
            s.y = t;
          ...
          s.x = Math.abs(s.x - sa * Math.round(s.x / sa));
          s.y = Math.abs(s.y - sa * Math.round(s.y / sa));
          s.z = Math.abs(s.z - sa * Math.round(s.z / sa));
          if (s.x < s.y) {
            t = s.x;
            s.x = s.y;
            s.y = t;
          ...
          if (sb <= 0) sb = sa;
          s.x = Math.abs(s.x - sa * Math.round(s.x / sa));
          s.y = Math.abs(s.y - sb * Math.round(s.y / sb));
          s.z = Math.abs(s.z - sc * Math.round(s.z / sc));
        }
      }
      break;
    ...
    case "hingeFold": {
      // #633 — swing the half-space past the cut plane around the hinge line.
      // Per-cell isometry (w untouched); the tear distance |hgs| feeds the
      // seam channel. Mirrors the WGSL/GLSL exactly — see operators.js id 67
      // for the derivation and the seam-clamped DE contract.
      const hga = Math.round(v[2] ?? 1);
      let hgu = s.x,
        hgv = s.z;
      if (hga === 0) {
        hgu = s.y;
      ...
      const hcp = rad(v[1] ?? 0);
      const hmx = -Math.sin(hcp),
        hmy = Math.cos(hcp);
      const hgo = v[3] ?? 0;
      const hgs = hgu * hmx + hgv * hmy - hgo;
      if (hgs > 0) {
        const hox = hmx * hgo,
          hoy = hmy * hgo;
        const hfa = rad(v[0] ?? 0);
        const hfc = Math.cos(hfa),
          hfs = Math.sin(hfa);
        const hdu = hgu - hox,
          hdv = hgv - hoy;
        const hru = hox + hdu * hfc - hdv * hfs;
        const hrv = hoy + hdu * hfs + hdv * hfc;
        if (hga === 0) {
          s.y = hru;
          s.z = hrv;
        } else if (hga === 2) {
      ...
      s.seam = Math.min(
        s.seam ?? Infinity,
        Math.abs(hgs) / Math.max(Math.abs(s.w), 1e-9),
      );
      break;
    }
    default:
...
//
// makeOrbit(formula, opts) → orbit(x,y,z) → { r, aw, escaped, nan, seam }
...
export function makeOrbit(formula, opts = {}) {
  ...
  const orbit = function orbit(px, py, pz) {
    ...
    return {
      ...
      escaped,
      nan: false,
      // #633 — running min of the seam-op tear distances (pulled back by w at
      // each op; see the seam-clamped DE contract, operators.js id 67).
      // undefined when the orbit ran no seam op — callers gate on hasSeamOps.
      seam: s.seam,
    };
  };
  orbit.escape = escape;
