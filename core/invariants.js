...
export function validateOperators() {
  ...
  for (const o of OPERATORS) {
    ...
    if (!OP_CATEGORIES.includes(o.category))
      failures.push(`operator "${o.key}" has no picker category (want one of ${OP_CATEGORIES.join('/')})`);
    ...
    if (typeof o.blurb !== 'string' || !o.blurb.trim())
      failures.push(`operator "${o.key}" has no blurb (one-sentence beginner description)`);
  }
...
export function validateFormula(formula) {
  ...
  const bodyMatch = glsl.match(/void iterateJIT_[A-Za-z0-9_]+\([^)]*\)\s*\{([\s\S]*)\}\s*$/);
  const bodyCode = (bodyMatch ? bodyMatch[1] : '').replace(/\/\/[^\n]*/g, '');
  if (/\bi\b/.test(bodyCode))
    failures.push(`formula "${name}": desktop export leaks a bare 'i' iteration index — an op needs a desktopGlsl override (SCALE_VARY.md §6.4)`);
