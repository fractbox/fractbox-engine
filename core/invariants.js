...
export function validateOperators() {
  ...
  for (const o of OPERATORS) {
    ...
    // seam is the discontinuous-op tag (#633 seam-clamped DE contract,
    // operators.js id 67) — boolean when present.
    if ("seam" in o && typeof o.seam !== "boolean")
      failures.push(`operator "${o.key}" has a non-boolean seam tag`);
  }
...
export function validateFormula(formula) {
  ...
  for (const [oi, ob] of (formula.objects ?? []).entries()) {
    ...
    if ("shapeParams" in ob) {
      ...
      if (
        !Array.isArray(sp) ||
        sp.length > MAX_LEAF_PARAMS ||
        sp.some((v) => typeof v !== "number" || !Number.isFinite(v))
      )
        failures.push(
          `${at}: shapeParams must be ≤${MAX_LEAF_PARAMS} finite numbers`,
        );
    }
