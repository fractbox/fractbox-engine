...
export function makeDE(formula, iters) {
  ...
  const logFinalize = formula?.hybrid
    ? hybridDeFamily(formula) === "ifs"
      ...
      : true
    : eff === 0;
  // #633 — seam-aware wrapper: when the formula contains a seam op the DE
  // closure ALSO publishes de.seam (the running-min tear distance the orbit
  // reported, pulled back to formula space) after every call, and flags
  // itself de.seamAware so the marcher clamps its STEP by it — never the DE
  // value itself (the seam-clamped DE contract, operators.js id 67). Seam-free
  // formulas return the exact pre-#633 closures, untouched and unwrapped.
  const seamAware = hasSeamOps(formula);
  if (!formula?.hybrid && eff === 3) {
    const de = function de(px, py, pz) {
      const o0 = run(px, py, pz);
      const R = o0.r;
      const eps = 1e-4 * Math.max(1, Math.hypot(px, py, pz));
      const o1 = run(px + eps, py, pz),
        o2 = run(px, py + eps, pz),
        o3 = run(px, py, pz + eps);
      if (seamAware)
        // min across the 4 probes (mirrors the WGSL numeric branch: the
        // private accumulator spans all probe orbits) — conservative, and
        // numeric formulas are "best-effort" seam tier anyway.
        de.seam = Math.min(
          o0.seam ?? Infinity,
          o1.seam ?? Infinity,
          o2.seam ?? Infinity,
    ...
    de.seamAware = seamAware;
    return de;
  }
  if (seamAware) {
    const de = function de(px, py, pz) {
      const o = run(px, py, pz);
      de.seam = o.seam ?? Infinity;
      const { r, aw } = o;
      return logFinalize
        ? (0.5 * Math.log(Math.max(r, 1e-9)) * r) / aw
        : r / aw;
    };
    de.seamAware = true;
    return de;
...
const LEAF_FNS = {
  ...
  // #353 round 5 — cs was (1.5·s, √3·s), whose two sublattices union to a
  // RHOMBIC lattice (shortest-vector shell of multiplicity 4), not a hex one;
  // the flat-top hexagon of apothem r = s/2 tiles on (2√3·r, 2·r) = (√3·s, s).
  // See leaves.js for the full derivation and the measured smear numbers.
  11: (x, y, z, p) => {
  ...
  // #353 round 7 — exact annular-sector-slab treads, min-union over the three
  // nearest sectors (see leaves.js for the full rationale: flat rectangular
  // profile with a small edge rounding, no seam blend, no riser notch).
  // Mirrors the WGSL/GLSL exactly.
  18: (x, y, z, p) => {
    const l = Math.hypot(x, z);
    const v = (Math.atan2(z, x) / 6.2831853) * p[0];
    const rr = Math.min(0.3 * p[1], 0.2 * p[2]);
    const r1 = 1 - rr;
    const r0 = 1 - p[2] + rr;
    let d = 1e9;
    for (let dk = -1; dk <= 1; dk++) {
      const k = Math.floor(v) + dk;
      const m = Math.round(y / p[3] - k / p[0]);
      const dy = Math.abs(y - (k / p[0] + m) * p[3]) - (p[1] - rr);
      const dv = Math.abs(v - (k + 0.5));
      if (dv <= 0.5) {
        const dr = Math.max(l - r1, r0 - l);
        d = Math.min(
          d,
          Math.hypot(Math.max(dr, 0), Math.max(dy, 0)) +
            Math.min(Math.max(dr, dy), 0),
        );
      } else {
        const ang = ((dv - 0.5) / p[0]) * 6.2831853;
        const u2 = l * Math.cos(ang);
        const dr = Math.max(u2 - r1, r0 - u2);
        const face =
          Math.hypot(Math.max(dr, 0), Math.max(dy, 0)) +
          Math.min(Math.max(dr, dy), 0);
        d = Math.min(d, Math.hypot(Math.max(face, 0), l * Math.sin(ang)));
      }
    }
    return d - rr;
  },
  ...
  25: (x, y, z, p) => {
    ...
    let back = box3(x, y, z + p[2] - hw, p[0], p[1], hw);
    const side = box3(x + p[0] - hw, y, z, hw, p[1], p[2]);
    // #627 — Door (p[4], the objAux lane's proving rider): carve a doorway in
    // the back wall — half-width p[4], from the floor to half the box height.
    // 0/undefined (every pre-#627 Room) short-circuits to the legacy math.
    if ((p[4] ?? 0) > 0.0049) {
      const cut = Math.max(Math.abs(x) - p[4], y - 0.5 * p[1]);
      back = Math.max(back, -cut);
    }
  ...
  41: (x, y, z, p) => {
    ...
    // #353 — the terrain solid used to extend to y = -inf (an endless column
    // under the plateau). Floored 10 units below the baseline; a tweakable
    // Height needs a 5th param slot (the same vec4 budget wall as Room's
    // doors), tracked separately.
    return Math.max(Math.max(de, dM - skirt), -10.0 - y);
  },
  42: (x, y, z, p) => {
  ...
  59: (x, y, z, p) => {
    // city (#634): procedural building lattice — 3×3 neighborhood scan of
    // two-tier exact boxes + a thin ground slab, seeded with the cell-pitch
    // cap. Neighborhood contract: footprints stay ≥ street/2 inside their
    // cell UNCONDITIONALLY, so out-of-window buildings are ≥ c + s/2 > c
    // away horizontally whatever their height — see leaves.js for the full
    // derivation, city.test.mjs for the brute-force proof. Math.imul keeps
    // the height hash bit-identical to the u32 math on the GPU tiers.
    const c = Math.min(2, Math.max(0.1, p[0]));
    const f = Math.min(0.9, Math.max(0.05, p[1]));
    const hmax = Math.min(3, Math.max(0.05, p[2]));
    const vary = Math.min(1, Math.max(0, p[3]));
    const det = Math.min(1, Math.max(0, p[4] ?? 0));
    const seed = Math.min(1023, Math.max(0, Math.round(p[5] ?? 0)));
    const fp = 0.5 * c * (1 - f);
    const gt = 0.05 * c;
    const dg = Math.abs(y + gt) - gt;
    let db = c;
    const bx = Math.floor(x / c),
      bz = Math.floor(z / c);
    for (let oi = -1; oi <= 1; oi++)
      for (let ok = -1; ok <= 1; ok++) {
        ...
        const lx = x - (ix + 0.5) * c,
          lz = z - (iz + 0.5) * c;
        const d1 = box3(lx, y - 0.5 * h1, lz, fp, 0.5 * h1, fp);
        const d2 = box3(lx, y - 0.5 * bh, lz, fp2, 0.5 * bh, fp2);
        db = Math.min(db, Math.min(d1, d2));
      }
    return Math.min(dg, db);
  },
};
...
function makeSceneDE(formula) {
  ...
  // #633 — scene seam channel: a seam op inside an object's chain reports its
  // tear distance in the object's LOCAL space; the combine walk rescales it by
  // uscale (distances scale linearly under the rigid+scale placement, exactly
  // like dk) and folds the min across objects. Gated so seam-free scenes keep
  // the pre-#633 closures untouched.
  const seamAware = hasSeamOps(formula);
  ...
  const built = formula.objects.map((raw) => {
    ...
    const child = (px, py, pz) => {
      ...
      if (seamAware) child.seam = s.seam; // local units; de() rescales
  ...
  const de = function de(px, py, pz) {
    ...
    for (const ob of built) {
      ...
      if (seamAware && ob.child.seam !== undefined)
        sm = Math.min(sm, ob.child.seam * ob.uscale);
...
// ── CLIP JAGGED noise (the CPU mirror of shader.js clipJagNoise) ────────────
// 2-octave smoothstep value noise, world-space and static, output in [-1, 1].
// Same construction as the WGSL/GLSL copies (sin-hash, smoothstep trilinear,
// octaves at f and 2.03f weighted 1 + 0.5, /1.5) — the tiers approximate the
// same field, and the worst-case Lipschitz ≈ 7 per unit frequency is what
// frameparams' CLIP_JAG_LIP divisor covers. Exported for the unit test that
// pins the range + an empirical gradient bound.
const clipJagFract = (x) => x - Math.floor(x);
const clipJagHash = (x, y, z) =>
  clipJagFract(Math.sin(x * 127.1 + y * 311.7 + z * 74.7) * 43758.5453);
const clipJagLerp = (a, b, u) => a + (b - a) * u;
function clipJagVal(x, y, z) {
  ...
  const a = clipJagLerp(
    clipJagLerp(
      clipJagLerp(clipJagHash(ix, iy, iz), clipJagHash(ix + 1, iy, iz), ux),
      clipJagLerp(
        clipJagHash(ix, iy + 1, iz),
        clipJagHash(ix + 1, iy + 1, iz),
        ux,
      ),
      uy,
    ),
    clipJagLerp(
      clipJagLerp(
        clipJagHash(ix, iy, iz + 1),
        clipJagHash(ix + 1, iy, iz + 1),
        ux,
      ),
      clipJagLerp(
        clipJagHash(ix, iy + 1, iz + 1),
        clipJagHash(ix + 1, iy + 1, iz + 1),
        ux,
      ),
      uy,
    ),
...
export function clipJagNoise(x, y, z) {
  return (
    (clipJagVal(x, y, z) +
      0.5 * clipJagVal(x * 2.03 + 13.5, y * 2.03 + 7.2, z * 2.03 + 3.1)) /
    1.5
  );
}
...
function traceGrid(formula, opts) {
  const {
    ...
    // half-extent, tan(planetFov/4); 0 = off. The fourth hand-kept copy of the
    // map (WGSL camRight.w, GL uPlanetK, preview.js pixelRay are the others),
    // and the ONE a node test can execute: WGSL/GLSL are compiled nowhere in
    // CI, so this tier is where the projection's algebra is actually checked
    // (core/tinyplanet.test.mjs) — the same lever the tile-window mirror below
    // exists for. Mutually exclusive with orthoH: the callers enforce it
    // (core/preview.js setPlanet/setOrtho), and if both arrive orthoH wins
    // here, deterministically, rather than compounding into nonsense.
    planetK = 0,
    // CLIP (cross-section) — the CPU mirror of the WGSL/GLSL clipped march:
    // null = off (byte-identical results); else { nx, ny, nz, w } — the
    // world-space plane's unit normal + offset along it (frameparams'
    // clipN/clipW derivation; shadeGrid derives it from coloring.light). The
    // march distance becomes max(de·deScale, n·p − w) — the CSG intersection
    // lower bound, exact for the plane — in the loop AND the bisection
    // refinement, and a hit where the plane term binds is a CUT-FACE cell:
    // its normal is the plane normal (an interior DE gradient is noise), so
    // the ASCII shade flattens across the face while hx/hy/hz keep sampling
    // the coloring AT the cut point — the interior structure. This tier has
    // no recenter, so the plane applies to the marched coordinates directly.
    // JAGGED lanes (all optional, 0/absent = the exact flat plane): jamp =
    // noise amplitude (world units), jfreq = noise frequency, jinv = the
    // 1/(1 + L·amp·freq) Lipschitz correction — frameparams' clipJag words,
    // mirrored by the same substitution the GPU tiers make.
    clip = null,
    // 360° EQUIRECT — full 360×180 lat-long ray generation; false = off. The
    // fourth-mirror rule again (WGSL camUp.w, GL constants, preview.js
    // pixelRay are the others), and the ONE a node test can execute — WGSL/
    // GLSL are compiled nowhere in CI, so core/equirect.test.mjs checks the
    // projection's algebra HERE (a sphere at a known direction must land at
    // the closed-form pixel, and a sphere BEHIND the camera must split across
    // the left and right edges — the wraparound). Precedence when several
    // projections somehow arrive: orthoH > planetK > equirect,
    // deterministically (the preview setters make the combinations
    // unreachable).
    equirect = false,
  ...
  // CLIP plane distance at a point — flat, or the JAGGED noised term scaled
  // by the Lipschitz divisor (mirrors shader.js clipPlaneAt; jamp 0/absent =
  // the exact flat plane, arithmetic untouched). One helper so the march,
  // the bisection AND the cut classification use the identical expression.
  const clipPlaneAt = clip
    ? (x, y, z) => {
        const base = clip.nx * x + clip.ny * y + clip.nz * z - clip.w;
        if (!(clip.jamp > 0)) return base;
        return (
          (base -
            clip.jamp *
              clipJagNoise(x * clip.jfreq, y * clip.jfreq, z * clip.jfreq)) *
          (clip.jinv || 1)
        );
      }
    : null;
  ...
  // #633 — seam-aware march: when the formula contains a seam op, clamp the
  // STEP (never the DE value: hit test / refinement / normals stay unclamped —
  // the seam-clamped DE contract, operators.js id 67) by the tear distance the
  // DE call just published, floored at half the eps·t hit shell so a ray
  // CROSSES a membrane in bounded steps instead of stalling on it. Mirrors
  // shader.js / shader_gl.js exactly.
  const seamAware = de.seamAware === true;
  ...
  const hx = plane("hx"),
    hy = plane("hy"),
    hz = plane("hz");
  // CLIP — 1 where the nearest subray landed on the cut face (0 elsewhere);
  // all zeros when clip is off, so consumers that ignore it are unchanged.
  const cutA = plane("cutf");
  const nSub = ss * ss;
    ...
    for (let c = 0; c < cols; c++) {
      ...
      for (let sy = 0; sy < ss; sy++) {
        ...
        for (let sx = 0; sx < ss; sx++) {
          ...
          // the ORIGIN and holds the direction (#441). TINY PLANET keeps the
          // single origin and bends the DIRECTION through the inverse
          // stereographic map (see core/shader.js rayGen for the derivation):
          //   q = u·u + v·v ;  dir = (2u·right + 2v·up + (1−q)·fwd) / (1+q)
          let dx, dy, dz;
          let ox = eye[0],
            oy = eye[1],
            oz = eye[2];
          if (orthoH > 0) {
            ...
            const a = 2 * pu * pInv,
              c = 2 * pv * pInv,
              f = (1 - pq) * pInv;
            dx = a * right[0] + c * up[0] + f * fwd[0];
            dy = a * right[1] + c * up[1] + f * fwd[1];
            dz = a * right[2] + c * up[2] + f * fwd[2];
          } else if (equirect) {
            // 360° EQUIRECT (see core/shader.js rayGenEquirect): the plane
            // coordinate becomes (longitude, latitude) — lon spans ±π across
            // the full frame (wx spans ±aspect, so the scale is π/aspect,
            // exactly the WGSL camUp.w word for an untiled render), lat spans
            // ±π/2. dir = cos(lat)·(sin(lon)·right + cos(lon)·fwd)
            //            + sin(lat)·up — single origin, direction only.
            const lon = wx * (Math.PI / aspect),
              lat = wy * (Math.PI / 2);
            const cl = Math.cos(lat),
              sl = Math.sin(lat);
            const a = cl * Math.sin(lon),
              f = cl * Math.cos(lon);
            dx = a * right[0] + sl * up[0] + f * fwd[0];
            dy = a * right[1] + sl * up[1] + f * fwd[1];
            dz = a * right[2] + sl * up[2] + f * fwd[2];
          } else {
          ...
          for (; st < maxSteps; st++) {
            const sx = ox + dx * t,
              sy = oy + dy * t,
              sz = oz + dz * t;
            // CLIP — the CSG max with the exact plane distance (mirrors
            // shader.js clipDE / shader_gl.js clipDEGL); clip null leaves the
            // arithmetic untouched.
            let d = de(sx, sy, sz) * deScale;
            if (clip) d = Math.max(d, clipPlaneAt(sx, sy, sz));
            lastD = d;
            if (d < eps * t) {
              hit = true;
              break;
            ...
            tPrev = t;
            // #633 BAND-GATED seam clamp (mirrors shader.js / shader_gl.js):
            // brake to the tear only while the bound is actionable (seam in
            // [d/4, d)); a degenerate bound (< d/4) steps plain — the old d/4
            // floor there bought no crossing guarantee while quartering the
            // march (the v1.4 "not sharpening" field regression).
            t += seamAware && de.seam >= 0.25 * d ? Math.min(d, de.seam) : d;
            if (t > tFar) break;
          }
          ...
          if (hit && t > tPrev) {
            ...
            for (let r = 0; r < 8; r++) {
              ...
              const mx = ox + dx * mid,
                my = oy + dy * mid,
                mz = oz + dz * mid;
              // CLIP — the refinement probes the CLIPPED distance (mirrors
              // the GPU tiers): against the unclipped DE the bracket would
              // walk back through the cut face (interior DE ≈ 0 reads as
              // "hit" everywhere behind the plane).
              let dm = de(mx, my, mz) * deScale;
              if (clip) dm = Math.max(dm, clipPlaneAt(mx, my, mz));
              if (dm < eps * mid) {
                hi = mid;
              } else {
                lo = mid;
          ...
          const px = ox + dx * t,
            py = oy + dy * t,
            pz = oz + dz * t;
          // CLIP — cut-face classification (mirrors the GPU tiers): the plane
          // term binding the max at the hit means this cell is the
          // CROSS-SECTION, and the normal becomes the plane normal, flipped
          // to face the ray (an interior DE gradient is noise — on the ASCII
          // tier it reads as static). The flat plane normal is also what
          // flattens the lit intensity across the face, the tier's own
          // version of the MRI lightbox shade.
          let cut = false;
          if (clip) {
            const pl = clipPlaneAt(px, py, pz);
            cut = pl >= de(px, py, pz) * deScale;
          }
          ...
          if (cut) {
            const cs =
              dx * clip.nx + dy * clip.ny + dz * clip.nz > 0 ? -1 : 1;
            nX = clip.nx * cs;
            nY = clip.ny * cs;
            nZ = clip.nz * cs;
          }
          ...
          if (t < bestT) {
            ...
            hy[i] = py;
            hz[i] = pz;
            cutA[i] = cut ? 1 : 0; // CLIP — nearest subray's cut-face flag
          }
        }
      }
      cov[i] = hits / nSub;
  ...
  return {
    ...
    hy,
    hz,
    cut: cutA, // CLIP — 1 = the cell landed on the cut face (all 0 when off)
  };
}
...
function shadeRGB(
  ...
  // NEON — this tier has no HDR target or bloom pipeline, so the GPU emissive
  // term (shader.js neonEmit) degrades to an LDR albedo brightening clamped at
  // white: same albedo·gain·sig² add, no halo. A DELIBERATE divergence, the
  // bloom precedent (COLORING.md divergence notes). gain 0 = exact legacy math.
  const neonGain = (L.neon ?? 0) * 8;
  ...
  // THIN FILM — the LDR mirror of the GPU modulation (shader.js filmMod):
  // phase ∝ cosθ at the inverse-λ ratios, grazing-leaning weight, factor
  // mix(1, 2·filmCol, w) per channel, clamped at white (this tier is LDR —
  // the neon-clamp precedent). Gated so the amount-0 path is byte-identical.
  // filmAmt arrives from the caller WITH the flat-only scene guard applied
  // (the neonSig shape) — V1 matches the GPU latch.
  let filmF = null;
  if (filmAmt > 0) {
    const fc = Math.min(Math.max(-(nx * dx + ny * dy + nz * dz), 0), 1);
    const fw = filmAmt * (0.3 + 0.7 * (1 - fc) * (1 - fc));
    const ph = 12.0 * fc;
    filmF = [1.0, 1.2218, 1.4444].map(
      (r) => 1 - fw + fw * (1 + Math.cos(r * ph)),
    );
  }
  return [0, 1, 2].map((i) => {
    let v = Math.max(
    ...
    // THIN FILM modulation (see above) — before the NEON add, mirroring the
    // GPU splice order (interference tints reflected light, not emission).
    if (filmF) v = Math.min(v * filmF[i], 1);
    // NEON add (see above) — gated so the gain-0 path is byte-identical.
    if (neonGain > 0)
      v = Math.min(v + alb[i] * neonGain * neonSig * neonSig, 1);
    return Math.round(255 * Math.pow(v, 1 / 2.2));
  });
}
...
export function shadeGrid(formula, opts) {
  ...
  // CLIP — the CPU mirror of frameparams' clipN/clipW derivation (axis+flip →
  // unit normal; flip negates the normal AND the constant so the plane holds
  // its axis position and only the kept half swaps; keep n·p <= w). Derived
  // here because this tier reads coloring directly (the thinFilm precedent);
  // clipOn absent/false ⇒ null ⇒ traceGrid's arithmetic is untouched. An
  // explicit opts.clip wins (tests drive the plane directly).
  const CL = coloring.light || {};
  let clipDerived = null;
  if (CL.clipOn === true) {
    const cAx = Math.max(0, Math.min(2, Math.round(CL.clipAxis ?? 0)));
    const cSgn = CL.clipFlip ? -1 : 1;
    const cOff = Math.max(
      -CLIP_OFFSET_LIMIT,
      Math.min(CLIP_OFFSET_LIMIT, CL.clipOffset ?? 0),
    );
    const cN = [0, 0, 0];
    cN[cAx] = cSgn;
    // JAGGED lanes — the frameparams derivation mirrored (slider 0..1 →
    // world amplitude × CLIP_JAG_WORLD, fixed CLIP_JAG_FREQ, and the
    // 1/(1 + LIP·amp·freq) divisor). jamp 0 keeps the exact flat plane.
    const cJag = Math.max(0, Math.min(1, CL.clipJag ?? 0));
    const cJAmp = CLIP_JAG_WORLD * cJag;
    clipDerived = {
      nx: cN[0],
      ny: cN[1],
      nz: cN[2],
      ...
      jamp: cJAmp,
      jfreq: CLIP_JAG_FREQ,
      jinv: 1 / (1 + CLIP_JAG_LIP * cJAmp * CLIP_JAG_FREQ),
    };
  }
  const g = traceGrid(formula, {
    ...opts,
    cols,
    rows,
    lightDir,
    clip: opts.clip ?? clipDerived,
  });
  const edge = edges ? detectEdges(g, opts) : null;
  const struct = structure
    ? detectContours(
  ...
  const painter =
    mode === 6 && !scene
      ...
      : null;
  // THIN FILM — flat formulas only (the GPU latch's scene guard, the neon/S6
  // precedent); the amount itself rides the light object.
  const filmAmt = scene ? 0 : (coloring.light?.thinFilm ?? 0);
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      const i = r * cols + c;
      if (g.cov[i] <= 0) continue;
      ...
      // NEON — the emission-weight signal, recorded post auto-levels and
      // pre-iridescence/phase (mirrors the GPU tiers); Painter records 1
      // (its palette blend carries the variation); flat formulas only (the
      // GPU latch's scene guard, iridescence S6 precedent).
      const neonSig = scene ? 0 : painter ? 1 : mixT;
      if (iridFn)
