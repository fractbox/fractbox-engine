...
`,
  ...
  },
  {
    ...
    category: "power",
    blurb:
      "Mandelbulb power with a choice of which axis the bulbs grow along, covering several bulb styles in one move. Escape-time mode.",
    params: [
      {
        name: "Power",
        default: 8.0,
    ...
    // White/Nylander default; 1 = y, the IQ convention; 2 = x) and a TRIG
    ...
    // the "sine bulb"). All three share the radial map r→rⁿ, so the analytic
    // dr (w update) is identical — the convention only re-phases the angle
    // (TRIGBULB_SPIKE.md). Escape-time like Mandelbulb Power: set DEoption 0 +
    // AddC. Axis 0 + Convention 0 reproduces Mandelbulb Power exactly.
    wgsl: `
        let bp = op.p0;
        let m = i32(op.p1 + 0.5);
    ...
    // convention (0 cos · 1 sin-polar · 2 asin-latitude) — escape-time.
    {
        float bp = ${v[0]};
        int m = int(${v[1]} + 0.5);
        ...
        if (br > 1e-9) {
            ...
            float bth = (conv == 2 ? asin(bu) : acos(bu)) * bp;
