...
`
    ...
    neon
      ? `uniform float uNeon; // NEON — emissive-glow gain (mirrors WGSL p3ctl.w; neon variants only)
`
      : ""
  }${
    aurora
      ? `uniform float uAurora, uNebula, uAurDrift; // AURORA (ENVX P6) — amounts + reserved drift
uniform vec3 uAurA, uAurB; // AURORA — curtain floor / tip-accent colors (sRGB)
`
      : ""
  }${
    grade
      ? `uniform vec4 uGradeA, uGradeB, uGradeC, uGradeD; // CINE GRADE (mirrors WGSL gradeA..D; grade variants only)
`
      : ""
  }${
    thinFilm
      ? `uniform float uFilm; // THIN FILM — interference amount (mirrors WGSL morphX.w; thinFilm variants only)
`
      : ""
  }${
    planet
      ? `uniform float uPlanetK; // TINY PLANET — tan(planetFov/4) (mirrors WGSL camRight.w; planet variants only)
`
      : ""
  }${
    clip
      ? `uniform vec4 uClip; // CLIP — plane unit normal (xyz) + offset (w) (mirrors WGSL clipU; clip variants only)
uniform float uClipS; // CLIP — cut-face brightness gain (mirrors WGSL clipS.x; reserved, 1 today)
`
      : ""
  }${
    clip && clipJag
      ? `uniform vec3 uClipJ; // CLIP JAGGED — amp, freq, invD (mirrors WGSL clipS.yzw; jag variants only)
`
      : ""
...
export function bulkLayout({
  ...
  if (scene) {
    ...
    // #627 — the leaf-param overflow lane (sp4..sp7), the GL mirror of the
    // WGSL objAux storage buffer. Added ONLY when the scene's leaf set has a
    // >4-param leaf, so a thin scene's Bulk block (and emitted GLSL) stays
    // byte-identical. renderer_gl passes the predicate at link time; fatness
    // is a function of shapeId, which is already in the structural signature.
    if (leafAux) add("uObjPrimP2", "vec4", 4, MAX_OBJECTS);
...
// NEON — the emissive-glow uniform, present only when the look uses it (the
// commonUniformsGL neon splice). Kept OUT of UNIFORM_TABLE_FLAT for the same
// reason as the ENVX table: the drift test's default surface (and the golden
// fixtures) stay untouched; the bake path appends this exactly when the
// emitted shader declares it.
export const UNIFORM_TABLE_NEON = [{ type: "float", name: "uNeon" }];

// AURORA (ENVX P6) — the fbm-sky uniforms, present only when the look uses it
// (the commonUniformsGL aurora splice). Kept OUT of UNIFORM_TABLE_FLAT for the
// same reason as the ENVX/NEON tables: the drift test's default surface (and
// the golden fixtures) stay untouched; the bake path appends this exactly when
// the emitted shader declares it.
export const UNIFORM_TABLE_AURORA = [
  { type: "float", name: "uAurora" },
  { type: "float", name: "uNebula" },
  { type: "float", name: "uAurDrift" },
  { type: "vec3", name: "uAurA" },
  { type: "vec3", name: "uAurB" },
];

// THIN FILM — the interference-amount uniform, present only when the look uses
// it (the commonUniformsGL thinFilm splice). Kept OUT of UNIFORM_TABLE_FLAT
// for the same reason as the ENVX/NEON tables: the drift test's default
// surface (and the golden fixtures) stay untouched; the bake path appends
// this exactly when the emitted shader declares it.
export const UNIFORM_TABLE_FILM = [{ type: "float", name: "uFilm" }];

// TINY PLANET — the stereographic plane radius, present only when the emitted
// shader is the planet variant (the commonUniformsGL planet splice). Same
// reason it is kept OUT of UNIFORM_TABLE_FLAT as ENVX/NEON. A standalone
// bundle never sees it: exportStandalone bakes PERSPECTIVE (planet: false),
// the #441 rule for a shareable artifact.
export const UNIFORM_TABLE_PLANET = [{ type: "float", name: "uPlanetK" }];

// CLIP (cross-section) — the plane words, present only when the emitted shader
// is the clip variant (the commonUniformsGL clip splice). Same reason it is
// kept OUT of UNIFORM_TABLE_FLAT as ENVX/NEON. A standalone bundle never sees
// it: exportStandalone bakes UNCLIPPED (clip: false), the planet/#441 rule
// for a shareable artifact.
export const UNIFORM_TABLE_CLIP = [
  { type: "vec4", name: "uClip" },
  { type: "float", name: "uClipS" },
];

// CLIP JAGGED — the noised-cut words (amp, freq, invD), present only in the
// jag sub-variant (the commonUniformsGL clipJag splice). Same out-of-FLAT
// reasoning as UNIFORM_TABLE_CLIP above.
export const UNIFORM_TABLE_CLIPJAG = [{ type: "vec3", name: "uClipJ" }];
...
export function bakeUniformBlock(
  emit,
  {
    envx = false,
    neon = false,
  ...
  return [
    ...UNIFORM_TABLE_FLAT,
    ...(envx ? UNIFORM_TABLE_ENVX : []),
    ...(neon ? UNIFORM_TABLE_NEON : []),
    ...(aurora ? UNIFORM_TABLE_AURORA : []),
    ...(thinFilm ? UNIFORM_TABLE_FILM : []),
    ...(planet ? UNIFORM_TABLE_PLANET : []),
    ...(clip ? UNIFORM_TABLE_CLIP : []),
  ]
    .map(({ type, name, array }) => {
...
`
  ...
  aurora
    ? `// AURORA (ENVX P6) — colored fbm sky (curtains + nebula), the GLSL mirror of
// shader.js auroraSky (same constants — core/aurora.test.mjs pins the parity).
// Background composite only (see mainGL) — never inside envColor, so the
// IBL/reflection env taps stay fbm-free on this tier too.
float hashAu(vec3 p) {
  return fract(sin(dot(p, vec3(157.31, 113.97, 271.89))) * 43758.5453);
}
float vnoiseAu(vec3 p) {
  vec3 i = floor(p);
  vec3 f = fract(p);
  vec3 u = f * f * (3.0 - 2.0 * f);
  return mix(
    mix(mix(hashAu(i), hashAu(i + vec3(1.0, 0.0, 0.0)), u.x),
        mix(hashAu(i + vec3(0.0, 1.0, 0.0)), hashAu(i + vec3(1.0, 1.0, 0.0)), u.x), u.y),
    mix(mix(hashAu(i + vec3(0.0, 0.0, 1.0)), hashAu(i + vec3(1.0, 0.0, 1.0)), u.x),
        mix(hashAu(i + vec3(0.0, 1.0, 1.0)), hashAu(i + vec3(1.0, 1.0, 1.0)), u.x), u.y),
    u.z);
}
float fbmAu(vec3 p) {
  float s = vnoiseAu(p) * 0.5;
  s += vnoiseAu(p * 2.03 + vec3(19.1)) * 0.25;
  s += vnoiseAu(p * 4.07 + vec3(7.7)) * 0.125;
  return s * 1.1428571; // normalize the octave sum to ~0..1
}
vec3 auroraSky(vec3 rd) {
  vec3 c = vec3(0.0);
  vec3 drift = vec3(uAurDrift);
  vec2 ring = rd.xy / max(length(rd.xy), 1e-4);
  float n = fbmAu(vec3(ring.x * 4.0, ring.y * 4.0, rd.z * 0.6) + drift);
  float curt = smoothstep(0.32, 0.72, n);
  float st = fbmAu(vec3(ring.x * 13.0, ring.y * 13.0, rd.z * 1.1) + drift + vec3(5.0));
  float ripple = 0.25 + 0.75 * smoothstep(0.3, 0.7, st);
  float reach = 0.35 + 0.75 * n;
  float v = max((rd.z + 0.3) / reach, 0.0);
  float prof = min(smoothstep(0.0, 0.1, v) * exp(-1.7 * v) * 1.55, 1.0);
  vec3 aCol = mix(uAurA, uAurB, clamp(rd.z * 2.2, 0.0, 1.0));
  c = s2l(aCol) * (curt * ripple * prof * 0.8 * uAurora);
  float nb = fbmAu(rd * 2.3 + vec3(37.7) + drift);
  float cl = smoothstep(0.3, 0.8, nb);
  vec3 nCol = mix(uAurA, uAurB, clamp(nb * 2.2 - 1.1, 0.0, 1.0));
  c = c + s2l(nCol) * (cl * cl * 0.45 * uNebula);
...
// NEON (mirrors shader.js): a function of the codegen flag — false emits the
// pre-NEON text byte-for-byte; true records the signal (post auto-levels,
// pre-iridescence/phase; Painter records 1.0) into nSig for the emission term
// mainGL splices after the lighting stack.
const albedoFlatGL = (neon = false) => `  vec3 albedo;
${neon ? "  float nSig = 0.0; // NEON — the emission-weight signal record\n" : ""}  if (uColorMode == 6) {
    albedo = orbitPainterGL(p); // COLORING R S7 — per-iteration palette blend (direct color)
${neon ? "    nSig = 1.0; // NEON — Painter's palette blend carries the variation\n" : ""}  } else {
    float mixT;
...
// #633 — does this op list contain a seam-declaring op (operators.js `seam`
// flag)? The GL mirror of shader.js usesSeam, keyed on raw op lists (this
// tier codegens per formula, so the op-list IS the variant key). Gates the
// g_seam global, the per-mapDE reset and the clamped march step — a seam-free
// formula's GLSL is byte-identical to the pre-#633 text.
const opsUseSeam = (list) => (list || []).some((op) => byKey(op.key)?.seam);

// The raymarch + shade main(), shared verbatim except the albedo block above.
// `neon` splices the emission add (mirrors shader.js neonEmit; false = the
// pre-NEON text byte-for-byte). Single-pass LDR tier: the same pre-tonemap
// math, but tone3 compresses instead of blooming — no halo (bloom = WebGPU-
// only, the RENDER_QUALITY "backend-structural absence" precedent).
// `grade` (CINE GRADE) splices the cineGrade() display-referred color pass at
// BOTH encode points (miss + hit) — mirrors buildPostWGSL's gated grade block
// exactly (same math, same post-tonemap placement); false = pre-GRADE text
// byte-for-byte. This tier never renders tiles (tiled export is WebGPU-only),
// so the vignette's gl_FragCoord/uRes address is always full-image — the
// seam-safety question the WGSL twin answers with gpos/gres doesn't arise.
// `thinFilm` (THIN FILM) splices the angle-driven interference modulation into
// the hit path (before the NEON emission and fog); false = pre-THIN-FILM text
// byte-for-byte.
// `planet` (TINY PLANET) splices the whole ray-generation region — see the
// rayGenGL const inside; false = pre-TINY-PLANET text byte-for-byte.
// `clip` (CLIP cross-section) splices the plane term into the march + the
// bisection refinement, the cut-face classification after calcNormal, the
// AO/shadow cut guards and the flat MRI cut shade — mirrors shader.js's
// clipDE/clipDEMid/cutShade splices exactly (same math, same placement);
// false = pre-CLIP text byte-for-byte (core/clipplane.test.mjs hashes it).
// `clipJag` (CLIP JAGGED) erodes the plane with static world-space value
// noise — the shader.js clipPlaneAt substitution mirrored (amp/freq/invD as
// the uClipJ uniform); false = the FLAT-clip text byte-for-byte.
// `equirect` (360° EQUIRECT) splices the same region with the lat-long map —
// the second arm on the planet's seam; false = byte-identical text. This tier
// never renders tiles (tiled export is WebGPU-only), so unlike the WGSL twin
// it needs NO runtime scale word: the full-image longitude span is π and the
// local aspect IS the full-image aspect, so lon = ndc.x·π exactly.
function mainGL(
  albedoBlock,
  envx = false,
  seam = false,
  ...
  // TINY PLANET splices the whole ray-gen region, so the off arm below is the
  // pre-tiny-planet text byte for byte (core/tinyplanet.test.mjs hashes it).
  // The map is documented once, in core/shader.js's rayGen; this is its GLSL
  // mirror and must not drift from it — the test pins the two texts' math
  // tokens against each other, the ENVX "star math is pinned identically"
  // pattern. This tier is NEVER compiled in CI (#206), so verify by hand with
  // ?renderer=webgl2 before merge.
  const rayGenGL = equirect
    ? `  // 360 EQUIRECT — lat-long ray generation (mirrors core/shader.js's
  // rayGenEquirect; core/equirect.test.mjs pins the two texts' math tokens
  // against each other). No tiling on this tier, so the longitude scale is
  // exactly pi over the local aspect: lon = (ndc.x * aspect) * (pi/aspect)
  // = ndc.x * pi, and the frame spans 360 x 180 edge to edge.
  float eqLon = ndc.x * 3.141592653589793;
  float eqLat = ndc.y * 1.5707963267948966;
  float eqCl = cos(eqLat);
  vec3 rd = normalize((eqCl * sin(eqLon)) * uCamRight
    + (sin(eqLat)) * uCamUp
    + (eqCl * cos(eqLon)) * uCamFwd);
  // Deep zoom §3.1 — uCamPos is the RESIDUAL ro_rel, not the absolute eye.
  // Equirect remaps only the DIRECTION (single origin), so the origin and
  // every reconstruction downstream are exactly what perspective does.
  vec3 ro = uCamPos;`
    : planet
      ? `  // TINY PLANET — inverse stereographic ray generation. uPlanetK =
  // tan(planetFov/4) = the plane radius at the frame's vertical half-extent.
  float pu = (ndc.x * aspect) * uPlanetK;
  float pv = ndc.y * uPlanetK;
  float pq = (pu * pu) + (pv * pv);
  float pInv = 1.0 / (1.0 + pq);
  vec3 rd = normalize(((2.0 * pu) * pInv) * uCamRight
    + ((2.0 * pv) * pInv) * uCamUp
    + ((1.0 - pq) * pInv) * uCamFwd);
  // Deep zoom §3.1 — uCamPos is the RESIDUAL ro_rel, not the absolute eye.
  // Tiny planet remaps only the DIRECTION, so the origin (and therefore every
  // reconstruction downstream) is exactly what perspective does.
  vec3 ro = uCamPos;`
  ...
  // CLIP — the clipped march distance (mirrors shader.js clipDE/clipDEMid):
  // the CSG max of the scaled DE and the exact world-space plane distance
  // (p is the residual, so uOffset is added back). clip:false emits the
  // pre-CLIP expression byte for byte in the loop AND the bisection;
  // clipJag:false emits the FLAT-clip term byte for byte. One shared helper
  // (shader.js clipPlaneAt mirrored) so march/bisection/classification use
  // the identical expression.
  const clipPlaneGL = (pt) =>
    clipJag
      ? `(dot(uClip.xyz, ${pt} + uOffset) - uClip.w - uClipJ.x * clipJagNoise((${pt} + uOffset) * uClipJ.y)) * uClipJ.z`
      : `dot(uClip.xyz, ${pt} + uOffset) - uClip.w`;
  const clipDEGL = (pt) =>
    clip
      ? `max(mapDE(${pt}) * uDeScale, ${clipPlaneGL(pt)})`
      : `mapDE(${pt}) * uDeScale`;
  return `${
    clip && clipJag
      ? `// CLIP JAGGED — 2-octave smoothstep value noise for the eroded cut (jag
// variants only; mirrors shader.js clipJagNoise — same construction, same
// worst-case Lipschitz ≈ 7·freq, which frameparams' invD divisor covers).
float clipJagHash(vec3 q) {
  return fract(sin(dot(q, vec3(127.1, 311.7, 74.7))) * 43758.5453);
}
float clipJagVal(vec3 q) {
  vec3 i = floor(q);
  vec3 f = fract(q);
  vec3 u = f * f * (3.0 - 2.0 * f);
  float a = mix(
    mix(mix(clipJagHash(i), clipJagHash(i + vec3(1.0, 0.0, 0.0)), u.x),
        mix(clipJagHash(i + vec3(0.0, 1.0, 0.0)), clipJagHash(i + vec3(1.0, 1.0, 0.0)), u.x), u.y),
    mix(mix(clipJagHash(i + vec3(0.0, 0.0, 1.0)), clipJagHash(i + vec3(1.0, 0.0, 1.0)), u.x),
        mix(clipJagHash(i + vec3(0.0, 1.0, 1.0)), clipJagHash(i + vec3(1.0, 1.0, 1.0)), u.x), u.y),
    u.z);
  return a * 2.0 - 1.0;
}
float clipJagNoise(vec3 q) {
  return (clipJagVal(q) + 0.5 * clipJagVal(q * 2.03 + vec3(13.5, 7.2, 3.1))) / 1.5;
}
...
`
    ...
    grade
      ? `// CINE GRADE — the cinematic-look color pass (grade variants only). Runs on
// display-linear [0,1] AFTER tone3, BEFORE l2s — the WGSL post pass's exact
// placement — so both GPU tiers grade the same values.
vec3 cineGrade(vec3 c) {
  vec3 base = clamp(c, 0.0, 1.0);
  vec3 g = mix(base, base * base * (3.0 - 2.0 * base), uGradeA.y); // gentle S
  float lum = dot(g, vec3(0.2126, 0.7152, 0.0722));
  float sat = uGradeA.z * (1.0 - uGradeA.w * (1.0 - smoothstep(0.0, 0.45, lum)));
  g = mix(vec3(lum), g, sat);                       // shadow-weighted saturation
  vec3 tone = mix(uGradeB.rgb, uGradeC.rgb, smoothstep(0.25, 0.65, lum));
  g = mix(g, clamp(g * tone * 2.0, 0.0, 1.0), uGradeB.w); // split tone (0.5-gray = identity)
  g = mix(g, mix(uGradeB.rgb, uGradeC.rgb, pow(lum, 0.6)), uGradeC.w); // duotone map (Noir)
  vec3 outc = mix(base, g, uGradeA.x);              // one strength slider
  if (uGradeD.x > 0.0) {                            // per-look vignette
    float vd = distance(gl_FragCoord.xy / uRes, vec2(0.5)) * 1.41421;
    outc *= 1.0 - uGradeD.x * uGradeA.x * smoothstep(0.30, 1.05, vd);
  }
  return outc;
}
`
...
${rayGenGL}
  ...
  for (int i = 0; i < 512; i++) {
    ...
    steps = i;
    float d = ${clipDEGL("ro + rd * t")};
    lastD = d;
    if (d < uEps * t) { hit = true; exhausted = false; break; }
    tPrev = t;
    t += ${
      seam
        ? `(g_seam >= 0.25 * d) ? min(d, g_seam) : d; // #633 — BAND-GATED seam
    // clamp (mirrors shader.js): brake to the tear distance mapDE just
    // published only while that bound is ACTIONABLE (g_seam in [d/4, d) —
    // braking lands the ray short of the tear). Below d/4 the running-min
    // bound is degenerate (dense fold-preimages) and the old d/4 floor bought
    // no crossing guarantee while quartering the march — the v1.4 "not
    // sharpening" field regression. Never the DE value: hit test / refinement
    // / normals stay unclamped — the seam-clamp contract, operators.js id 67.`
        : `d;`
    }
  ...
  if (hit && t > tPrev) {
    ...
    for (int r = 0; r < 8; r++) {
      float mid = 0.5 * (lo + hi);
      if (${clipDEGL("ro + rd * mid")} < uEps * mid) { hi = mid; } else { lo = mid; }
    }
    t = hi;
  }
...
`
    ...
    aurora
      ? `  // AURORA (ENVX P6): background composite only — full amount at any Sky
  // (envColor never carries it), and bgOut is also the fog target, so fogged
  // hits fade toward the same aurora sky. Mirrors shader.js.
  bgOut += auroraSky(rd);
`
      : ""
  ...
  if (!hit) {
    ...
    fragColor = vec4(l2s(${
      grade
        ? "cineGrade(tone3(skyOut * exp2(uExposure)))"
        : "tone3(skyOut * exp2(uExposure))"
    }), 1.0); return;
  }
...
${
    clip
      ? `  // CLIP — cut-face classification (mirrors shader.js): the plane term
  // binding the max at the hit means this pixel is the CROSS-SECTION; the
  // normal becomes the plane normal, flipped to face the ray.
  vec3 nrm0 = calcNormal(p, t);
  float clipPl = ${clipPlaneGL("p")};
  bool cutFace = hit && clipPl >= mapDE(p) * uDeScale;
  vec3 nrm = cutFace ? normalize(uClip.xyz) * (dot(rd, uClip.xyz) > 0.0 ? -1.0 : 1.0) : nrm0;`
  ...
  float amb = uAmbient;
  float occ = uAoStr > 0.0${clip ? " && !cutFace" : ""} ? calcAO(p, nrm, t) : 1.0;               // P1 real AO
  float sh = uShadowOn > 0.5${clip ? " && !cutFace" : ""} ? 0.15 + 0.85 * softShadow(p, nrm, lightDir, t, uShadowK) : 1.0; // P1 (15% floor)
...
${
  clip
    ? `  // CLIP — the MRI cut-face shade (mirrors shader.js cutShade): a flat,
  // shadow/AO-free lightbox read of the albedo AT the cut point; the
  // plane-normal diffuse term is constant across the face, so the spatial
  // variation is all coloring signal — the interior structure. Fog below
  // still applies (depth cue). uClipS is the reserved brightness gain.
  if (cutFace) {
    col = albedo * ((0.42 + 0.58 * max(dot(nrm, lightDir), 0.0)) * uClipS) * uIntensity;
  }
`
    : ""
}${
  thinFilm
    ? `  // THIN FILM — angle-driven interference tint (mirrors shader.js filmMod):
  // phase ∝ cosθ sampled at inverse-λ ratios, grazing-leaning weight, a
  // modulation with mean 1 over phase (bounded 2×, never an HDR add) —
  // before the NEON emission and before fog.
  float filmCos = clamp(dot(nrm, -rd), 0.0, 1.0);
  float filmPh = 12.0 * filmCos;
  vec3 filmCol = 0.5 + 0.5 * cos(vec3(1.0, 1.2218, 1.4444) * filmPh);
  float filmW = uFilm * (0.3 + 0.7 * pow(1.0 - filmCos, 2.0));
  col *= mix(vec3(1.0), filmCol * 2.0, filmW);
`
    : ""
}${
    neon
      ? `  // NEON — emissive glow (mirrors shader.js neonEmit): self-luminous, after
  // the intensity multiply, before fog. LDR tier: tone3 compresses the overs
  // toward white instead of blooming.
  col += albedo * (uNeon * nSig * nSig);
`
      : ""
  ...
  col = l2s(${
    grade
      ? "cineGrade(tone3(max(col, vec3(0.0)) * exp2(uExposure)))"
      : "tone3(max(col, vec3(0.0)) * exp2(uExposure))"
  });
...
// `neon` (NEON emissive glow, mirrors shader.js buildWGSL's option): false —
// the default, and the only value renderer_gl passes until the look derives
// neonGain > 0 — emits the pre-NEON text byte-for-byte. `aurora` (ENVX P6)
// rides the same contract: false emits the pre-AURORA text byte-for-byte.
// `grade` (CINE GRADE, mirrors buildPostWGSL's option): false — the default —
// emits the pre-GRADE text byte-for-byte; true splices cineGrade() into the
// shared fragment tail (mainGL). Engine path only: the standalone bake (emit)
// never grades — an exported shader is the ungraded scene, the bloom/DOF
// structural-absence precedent.
// `thinFilm` (THIN FILM interference material, mirrors shader.js buildWGSL's
// option, same contract): false — the default until the look derives the
// slider > 0 — emits the pre-THIN-FILM text byte-for-byte. Unlike grade it IS
// bakeable (the exported shader keeps the sheen as a baked const).
export function buildFragGL(
  ops,
  extraSlots,
  emit,
  {
  ...
  // The buildWGSL guard, mirrored: only one projection can answer "how does a
  // pixel become a ray", and the preview setters never key both.
  if (planet && equirect)
    throw new Error(
      "buildFragGL: planet and equirect are exclusive projections",
    );
  ...
  // #633 — seam channel gate: any seam op in ANY slot arms the channel for
  // the whole program (one shared march).
  const seamUsed =
    opsUseSeam(ops) ||
    (hybrid && extraSlots.some((slot) => opsUseSeam(slot.ops)));
  const seamReset = seamUsed
    ? "\n  g_seam = 1e30; // #633 — fresh seam accumulator per DE evaluation"
    : "";
...
float mapDE(vec3 p_rel) {${seamReset}
  vec3 p0 = p_rel + uOffset;
...
float escapeIter(vec3 p_rel) {
  ...
  const uniformSection = emit
    ? // The standalone bake never grades (see the header) and never carries
      // the projection either — a bundle is always PERSPECTIVE, the #441 rule
      // for a shareable artifact, so `planet` is deliberately absent here.
      bakeUniformBlock(emit, { envx, neon, aurora, thinFilm })
    : `${commonUniformsGL(variantUniforms, envx, neon, aurora, grade, thinFilm, planet, clip, clipJag)}
...
${
  seamUsed
    ? `float g_seam; // #633 seam channel — running-min tear distance the seam-op
// bodies write (the g_wq convention); mapDE resets it per call, the march
// step clamps by it. See the seam-clamped DE contract, operators.js id 67.
`
    : ""
}${core}

${shadeLibGL(envx, aurora)}

${painterLibGL(hybrid)}

${mainGL(albedoFlatGL(neon), envx, seamUsed, neon, aurora, !emit && grade, thinFilm, planet, clip, clipJag, equirect)}`;
}
...
export function buildSceneFragGL(
  objects,
  {
    envx = false,
    aurora = false,
  ...
  // Same exclusivity guard as buildFragGL — one projection at a time.
  if (planet && equirect)
    throw new Error(
      "buildSceneFragGL: planet and equirect are exclusive projections",
    );
  const layout = sceneParamLayout(objects);
  const norm = objects.map(normalizeSceneObject);
  // #633 — scene seam gate: a seam op inside any object's ACTIVE chain arms
  // the channel. Object chains report seams in LOCAL units; the placement
  // walk rescales by uscale (exactly like dk) and folds the min.
  const seamUsed = norm.some((n) => opsUseSeam(n.ops));
  ...
  let funcs = "";
  // #627 — fat leaf set ⇒ every leaf fn gains prm2 (uObjPrimP2 lane); thin
  // sets emit the exact legacy text (thin bodies never read prm2).
  const fatLeaf = usedIds.some(
    (id) => ((LEAVES.find((l) => l.id === id) || {}).params?.length ?? 0) > 4,
  );
  const sig2 = fatLeaf ? ", vec4 prm2" : "";
  for (const l of LEAVES) {
    if (!usedIds.includes(l.id)) continue;
    funcs += `
float leaf_${l.key}(vec3 p, vec4 prm${sig2}) {
  ${l.glsl}
}`;
  }
  ...
  for (let k = 0; k < norm.length; k++) {
    ...
    const leafCall = leaf
      ? `leaf_${leaf.key}(pos, uObjPrimP[${k}]${fatLeaf ? `, uObjPrimP2[${k}]` : ""})`
      : null;
...
${iterMin}    if (dot(pos, pos) > uBailout) break;
  ...
  for (let k = 0; k < norm.length; k++) {
    ...
    vec3 pk = qrotGL(qinv, p0 - uObjOrigin[${k}]) / uObjUscale[${k}];${
      seamUsed
        ? `
    float sm0_${k} = g_seam; g_seam = 1e30; // #633 — child reports in local units
    float dk = objDE_${k}(pk) * uObjUscale[${k}];
    g_seam = min(sm0_${k}, g_seam * uObjUscale[${k}]);`
        : `
    float dk = objDE_${k}(pk) * uObjUscale[${k}];`
    }`;
    mapBody += `  {
...
${commonUniformsGL(variantUniforms, envx, false, aurora, grade, false, planet, clip, clipJag)}
...
${
  // #627 — leafAux MUST ride here too: this in-shader block is the DECLARATION
  // the fat-leaf call sites (leaf_*(…, uObjPrimP2[k])) compile against.
  // Omitting it emitted uObjPrimP2 uses with no declaration, so EVERY
  // fat-leaf scene (city/heightfield leaves) failed to COMPILE on the GL
  // tier — and the glHealth gate then demoted the whole tier to ASCII (found
  // 2026-08-31 chasing the v1.4 "not sharpening" reports: a background
  // thumbnail bake of "Whorl City" was killing the live GL tier at boot).
  // renderer_gl's bulkLayout call (its leafAux predicate) only shapes the
  // UBO packing — the two must agree, and both now derive from the leaf set.
  bulkBlockGL(bulkLayout({ scene: true, leafAux: fatLeaf }))
}

float g_wq; // desktop 4D scratch the shared op bodies write to; unused here
${
  seamUsed
    ? `float g_seam; // #633 seam channel — see operators.js id 67 (reset per mapDE call)
`
    : ""
}
...
${mainGL(ALBEDO_SCENE_GL, envx, seamUsed, false, aurora, grade, false, planet, clip, clipJag, equirect)}`;
}
