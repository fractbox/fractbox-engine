...
export function sceneParamLayout(objects) {
  ...
  // and a sanitize-legal scene (8 objects × 24 ops × 3 params = 576) can exceed
...
function commonUniformsGL(variantUniforms) {
...
// uP[192] alone is 192 vec4s and the scene per-object arrays add ~88 more, so
...
export function bakeUniformBlock(emit) {
  return UNIFORM_TABLE_FLAT.map(({ type, name, array }) => {
...
// builders splice it verbatim after their mapDE core.
const SHADE_LIB_GL = `// e scales with hit distance t (deep zoom §3.4/§5) — a fixed epsilon straddles
...
export function buildFragGL(ops, extraSlots, emit) {
...
float escapeIter(vec3 p_rel) {
  ...
  const uniformSection = emit
    ? bakeUniformBlock(emit)
    : `${commonUniformsGL(variantUniforms)}
