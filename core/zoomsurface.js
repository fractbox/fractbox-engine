...
export function surfaceHitDist(de, eye, fwd, near, far) {
  ...
  for (let i = 0; i < 192; i++) {
    ...
    // Conservative step (deScale-ish). The zero-step-stall floor must be
    // SCALE-RELATIVE: an absolute 1e-6 floor catapulted the first step past
    // `far` for any camera below ~×10¹⁰, silently killing every deep probe
    // (re-pin, wheel anchor, burial recovery) — near ∝ dist, so near·1e-3
    // keeps the guard proportional at every depth (pt tier reaches 1e-30).
    t += Math.max(d * 0.7, near * 1e-3);
