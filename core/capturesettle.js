...
export function frameFeaturesFor(
  ...
  coloring,
  {
    ...
    perturb = false,
    blendModeB = null,
    // IMGTEX (#631) — the user-image texture flags. Renderer-local state (an
    // image is loaded) the caller must supply: preview.js passes its live
    // renderer's presence ANDed with the amount sliders so the prewarm
    // prediction matches writeGlobals' latch; headless capture passes nothing
    // (default false — capture shaders are texture-free).
    envMap = false,
    surfTex = false,
    // TINY PLANET — the stereographic ray-gen latch (planetK > 0). Live-only
    // state, so it arrives explicitly like morphF/df64: headless capture
    // passes nothing and captures perspective, which is what a splat/volume
    // export means (fsCapture marches its own view rays regardless).
    planet = false,
    // 360° EQUIRECT — the lat-long ray-gen latch (equirectS > 0). Same
    // contract as planet in every clause above.
    equirect = false,
  } = {},
) {
  ...
  return {
    ...
    envMap: !!envMap, // IMGTEX #631
    surfTex: !!surfTex,
    // #630 self-reflection — predict the SETTLED frame's variant (prewarm
    // holds frames on this): the look wants mirrors and the precision tiers
    // allow them (df64/perturb are an eligibility domain — renderer.js keys
    // the same exclusion, buildWGSL throws on the combination). Mirrors the
    // deriveFrameParams latch incl. its reflectivity > 0 guard; cheap frames
    // fall back to the plain variant via shadeLight's reflBounces override.
    sreflect:
      !df64 &&
      !perturb &&
      Math.round(coloring?.light?.reflBounces ?? 0) > 0 &&
      (coloring?.light?.reflectivity ?? 0.8) > 0,
    // NEON emissive glow — mirrors the renderer's activeFeat latch (flat
    // formulas only, neonGain > 0 ⇔ the slider is up; deriveFrameParams owns
    // the mapping, this predicts only the boolean).
    neon: !scene && (coloring?.light?.neon ?? 0) > 0,
    // AURORA (ENVX P6) — mirrors the renderer's activeFeat latch (either
    // amount > 0 flips it; deriveFrameParams owns the mapping, this predicts
    // only the boolean). A background layer, so scenes carry it too.
    aurora:
      (coloring?.light?.aurora ?? 0) > 0 || (coloring?.light?.nebula ?? 0) > 0,
    // THIN FILM interference material — mirrors the renderer's activeFeat
    // latch (flat formulas only, slider > 0 ⇔ on; deriveFrameParams owns the
    // mapping, this predicts only the boolean).
    thinFilm: !scene && (coloring?.light?.thinFilm ?? 0) > 0,
    // TINY PLANET — no scene/flat guard: ray generation runs before anything
    // knows which it is, so every kind gets the same latch.
    planet: !!planet,
    // CLIP cross-section — mirrors the renderer's activeFeat latch (the
    // explicit clipOn boolean; deriveFrameParams owns the plane derivation,
    // this predicts only the latch). March geometry, so no scene guard —
    // the planet/aurora contract. clipJag mirrors the amount-keyed noised-cut
    // sub-variant (clip on AND Jagged > 0 — deriveFrameParams' clipJag latch).
    clip: coloring?.light?.clipOn === true,
    clipJag:
      coloring?.light?.clipOn === true && (coloring?.light?.clipJag ?? 0) > 0,
    // 360° EQUIRECT — same no-guard rule (and mutually exclusive with planet
    // at the source: preview.js's setters never latch both).
    equirect: !!equirect,
...
export function settleFrame(renderer, f, coloring, q, res, cam, live = {}) {
  ...
  const base = {
    ...
    // TINY PLANET — tan(planetFov/4); 0 = off, the byte-identical path. Rides
    // the options bag exactly like orthoH, and is BOTH the shader's runtime
    // word and the codegen latch (renderer.js writeGlobals derives
    // activeFeat.planet from it — one number, one source of truth).
    planetK: live.planetK ?? 0,
    // 360° EQUIRECT — π·H/W of the FULL image; 0 = off, the byte-identical
    // path. Same double duty as planetK (runtime word + codegen latch), same
    // ride on the options bag.
    equirectS: live.equirectS ?? 0,
