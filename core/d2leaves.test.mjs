...
test("hexGrid (#353): the exact corner-aware SDF no longer underestimates near a vertex", () => {
  // Same params as above (cellSize .5, zThick .3, wall .04, cellR .9). Near a
  // hex corner, the pre-fix `max(q.x·0.866+q.y·0.5, q.y) − r` shortcut is only
  // exact along each FACE — it undershoots the true (vertex-aware) distance,
  // the reported "walls never quite close at a corner" bug. Pinned against an
  // inline copy of the old formula (same dual-offset fold, only the final
  // hd step differs) so a regression back to the shortcut fails loudly.
  const p = [0.5, 0.3, 0.04, 0.9];
  ...
  const oldHd = (x, y) => {
    ...
    return Math.max(qx * 0.8660254 + qy * 0.5, qy) - p[0] * 0.5 * p[3];
  };
  const x = -0.22914918996092,
    y = 0.180538068769719; // near a cell corner (found by search, see #353)
  const newDe = de(x, y, 0);
  const oldWallDist = Math.abs(oldHd(x, y)) - p[2]; // what the pre-fix formula reported
  assert.ok(close(newDe, -0.008232690798637754, 1e-9), "exact corner-aware distance");
...
test("heartSurf (#353): the lobe/point asymmetry now rides world Y, not world Z", () => {
  // Pre-fix (q = p directly) the sextic's asymmetric z³ terms rode world Z,
  // which the app's fixed default camera (yaw 35°/pitch 22°) views nearly
  // edge-on — the shape read as a featureless blob instead of a heart. Fixed
  // by remapping q = (x, z, −y), so the asymmetry now rides world Y. At a
  // fixed (x=1, z=0), z³-driven terms must now differ in SIGN between
  // y=+0.5 and y=−0.5 (they were identical pre-fix, since both mapped to the
  // same q.z=0 plane regardless of y's sign).
  const de = leafDE(28, [1, 1.6]);
  const above = de(1, 0.5, 0);
  const below = de(1, -0.5, 0);
  assert.ok(close(above, 0.12480743338154411, 1e-9));
  assert.ok(close(below, -0.18981382118013107, 1e-9));
  assert.ok(above > 0 && below < 0, "opposite sides of the surface — real asymmetry, not noise");
});
...
test("Taubin wave: on-surface zeros and inside signs match the cited implicits", () => {
  ...
  // heart (formula z = lobe/point axis): world (0,−1,0) and (0,−0.3,0) are the
  // pre-#353 (0,0,1)/(0,0,0.3) root/inside checks carried through the leaf's
  // world→formula remap q=(x,z,−y) (leaves.js #353 — faces the fixed default
  // camera instead of grazing it edge-on). A(0,0,1)=0 → f=0 is still the root.
  assert.ok(Math.abs(leafDE(28, [1, 1.6])(0, -1, 0)) < eps, "heart root");
  assert.ok(leafDE(28, [1, 1.6])(0, -0.3, 0) < 0, "heart inside");
...
test("helixStairs (#353): the tread height no longer JUMPS in value at a sector seam", () => {
  ...
  // 2e-6 rad step — this asserts they now agree to float precision.
  assert.ok(close(d1, d2, 1e-9), `seam jump: ${d1} vs ${d2}`);
});
