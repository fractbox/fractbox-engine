...
test('§2.8: Surprise never emits a ≥3-slot hybrid', () => {
  // The seed pool excludes hybrids and capGeneratedSlots is the belt-and-braces
  // degrade — across many draws NOT ONE may carry ≥3 hybrid slots.
  for (let i = 0; i < 120; i++) {
    const f = randomFormula();
    assert.ok(hybridSlots(f).slots.length < 3, `draw ${i} generated a ≥3-slot hybrid`);
  }
});
