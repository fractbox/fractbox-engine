...
const UNIFORM_SOURCES = {
  ...
  uExposure: "exposure",
  // ENVX (backgrounds P5) — baked only when the look uses it (the emitted
  // shader then declares the UNIFORM_TABLE_ENVX surface; see standaloneRaw).
  uStars: "stars",
  ...
  uZenC: "zenith",
  uZenOn: "zenithOn",
  // NEON — baked only when the look uses it (the emitted shader then declares
  // the UNIFORM_TABLE_NEON surface; see standaloneRaw). The value is the
  // derived HDR gain, exactly what the live GL tier uploads as uNeon.
  uNeon: "neonGain",
  // AURORA (ENVX P6) — baked only when the look uses it (the emitted shader
  // then declares the UNIFORM_TABLE_AURORA surface; see standaloneRaw). The
  // colors are the hue-derived floor/tip pair, exactly what the live GL tier
  // uploads.
  uAurora: "auroraAmt",
  uNebula: "nebulaAmt",
  uAurDrift: "auroraDrift",
  uAurA: "auroraColA",
  uAurB: "auroraColB",
  // THIN FILM — baked only when the look uses it (the emitted shader then
  // declares the UNIFORM_TABLE_FILM surface; see standaloneRaw). The value is
  // the raw slider amount, exactly what the live GL tier uploads as uFilm.
  uFilm: "filmAmt",
  uP: "uP",
};
...
function standaloneRaw(formula, state) {
  ...
  const vals = bakeValues(state || {}, ops);
  // ENVX (backgrounds P5): the export mirrors the live GL tier's codegen gate —
  // a starred look bakes the star surface as consts, everything else emits the
  // pre-ENVX text (golden-pinned) with not one extra token. NEON rides the
  // same contract (neonGain > 0 = the look uses it).
  // TINY PLANET is never baked: `planet` is left at its default false, so the
  // bundle emits the PERSPECTIVE ray-gen and declares no uPlanetK at all. Same
  // call #441 made for ortho — a shareable shader artifact should carry the
  // LOOK, not the projection the studio happened to be inspecting through.
  // (Because the flag is codegen, "off" here is not a zeroed uniform but the
  // pre-tiny-planet text, which is what the golden fixtures pin.)
  // CLIP is never baked either: `clip` stays default false (the jagged
  // sub-variant with it), so a bundle emits the UNCLIPPED march and declares
  // no uClip — the same "carry the LOOK, not the inspection state" call, and
  // a cross-section is exactly an inspection state (the share LINK carries
  // it; a baked shader artifact does not). Deferred with the
  // clipped-splat-capture note in renderer.js.
  // 360° EQUIRECT is never baked either — same #441 rule, same mechanism
  // (`equirect` left at its default false, so the bundle emits the
  // perspective ray-gen with not one equirect token).
  return buildFragGL(ops, null, bakeStrategy(vals), {
    envx: !!vals.envx,
    neon: (vals.neonGain ?? 0) > 0,
    aurora: !!vals.aurora, // AURORA (ENVX P6) — same contract as envx
    // THIN FILM rides the same contract (filmAmt > 0 = the look uses it).
    thinFilm: (vals.filmAmt ?? 0) > 0,
  });
}
