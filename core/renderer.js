...
export async function createRenderer(canvas, opts = {}) {
  ...
  const marchBind = (pl, perturb = false, aux = false) =>
  ...
  function buildMarchVariant(feat) {
    ...
    const v = { pl, bg: marchBind(pl, !!feat.perturb, usesOpAux(o.ops)) };
    marchVariants.set(key, v);
    lruEvict(key);
    return v;
  }
      ...
      .then(
        (pl) => {
          if (pl && !marchVariants.has(key)) {
            marchVariants.set(key, {
              pl,
              bg: marchBind(pl, !!feat.perturb, usesOpAux(o.ops)),
            });
            lruEvict(key);
          }
        },
  ...
  function buildGeneral(feat) {
    ...
    const v = { pl, bg: marchBind(pl, false, usesOpAux(o.ops)) };
      ...
      .then(
        (pl) => {
          if (pl && !marchVariants.has(gkey)) {
            marchVariants.set(gkey, {
              pl,
              bg: marchBind(pl, false, usesOpAux(o.ops)),
            });
            lruEvict(gkey);
          }
        },
