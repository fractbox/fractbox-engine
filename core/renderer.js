...
import { bandRect } from "./renderpolicy.js";
...
// Derived from shader.js's GLOBALS_WORDS (was a hand-typed 560 — #239): the
// WGSL struct is the single source of truth for the layout, so the byte size
// follows it rather than drifting behind a second constant.
const GLOBALS_BYTES = GLOBALS_WORDS * 16; // GLOBALS_WORDS * vec4 (16 B each)
...
export async function createRenderer(canvas, opts = {}) {
  ...
  const marchBind = (pl, perturb = false) =>
  ...
  function buildMarchVariant(feat) {
    ...
    const m = device.createShaderModule({ code: buildWGSL(wgslOf(feat)) });
    const pl = timedPipelineSync("variant:" + key, m);
    const v = { pl, bg: marchBind(pl, !!feat.perturb) };
    marchVariants.set(key, v);
    lruEvict(key);
    return v;
  }
      ...
      .then(
        (pl) => {
          if (pl && !marchVariants.has(key)) {
            marchVariants.set(key, { pl, bg: marchBind(pl, !!feat.perturb) });
            lruEvict(key);
          }
        },
  ...
  function buildGeneral(feat) {
    ...
    const m = device.createShaderModule({
      code: buildWGSL(generalFeat(feat)),
    });
    const pl = timedPipelineSync("general:" + gkey, m);
    const v = { pl, bg: marchBind(pl) };
      ...
      .then(
        (pl) => {
          if (pl && !marchVariants.has(gkey)) {
            marchVariants.set(gkey, { pl, bg: marchBind(pl) });
            lruEvict(gkey);
          }
        },
        () => {},
  ...
  const activeMarch = () => {
    ...
    return gkey ? buildGeneral(activeFeat) : buildMarchVariant(activeFeat);
  };
  ...
  async function createSplatCapture(feat, res) {
    ...
    try {
      const mod = device.createShaderModule({
        code: buildWGSL({ ...wgslOf(feat), capture: true }),
      });
