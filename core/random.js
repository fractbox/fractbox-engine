...
// stay recipe/preset territory for now (N-slot spec §2.8: Surprise never
// generates a hybrid in v1).
const seedablePresets = () =>
  PRESETS.filter((p) => !p.objects?.length && !p.hybrid);

// §2.8 guard — Surprise never GENERATES a ≥3-slot hybrid in v1. The seed pool
// (seedablePresets) already excludes every hybrid and jitterParams preserves slot
// count, so no path here can emit one; this is belt-and-braces for a future seed/
// recipe that grew a hybrid. A ≥3-slot roll degrades to its FLAT slot-A formula
// (the old-build degrade, §2.2) rather than surprising the user with a phase
// schedule they didn't ask for.
function capGeneratedSlots(f) {
  if (f?.hybrid && hybridSlots(f).slots.length >= 3) {
    const { hybrid: _drop, ...rest } = f;
    return rest;
  }
  return f;
}
...
function seededFormula() {
  ...
  const candidate = (attempt) => {
    ...
    if (typeof f.iters === "number")
      f.iters = Math.max(
        3,
        Math.min(24, f.iters + Math.floor(Math.random() * 5) - 2),
      );
    if (f.camera) {
...
export function randomFormula() {
  ...
  return capGeneratedSlots(
    Math.random() < 0.5 ? seededFormula() : recipeFormula(),
  );
}
...
function recipeFormula() {
  ...
  if (family === "bulb") {
    ...
    return {
      name: "Random",
      note: "randomized bulb — tweak it, Save it, or Reset",
      addC: true,
      iters: Math.floor(rnd(7, 11)),
      deOption: 0,
      ops,
      camera: {
        yawDeg: r2(0, 360),
    ...
    const r =
      family === "menger"
        ? {
            ...
            iters: Math.floor(rnd(4, 7)),
            dist: 9.0,
            note: "randomized sponge",
          }
        : {
            ops: [
            ...
            iters: Math.floor(rnd(11, 16)),
            dist: 8.0,
            note: "randomized tetra",
          };
    return {
      name: "Random",
      note: `${r.note} — tweak it, Save it, or Reset`,
      addC: false,
      iters: r.iters,
      deOption: 2,
      ops: r.ops,
  ...
  for (let k = 0; k < nRot; k++)
    ops.push({ key: pick(rots), values: [r2(-90, 90)] });
  ...
  if (chance(0.5)) {
    if (chance(0.5))
      ops.push({ key: "kaleido", values: [pick([3, 4, 5, 6, 8]), r2(0, 30)] });
    else
      ops.push({
        key: "translate",
        values: [r2(-0.4, 0.4), r2(-0.4, 0.4), r2(-0.4, 0.4)],
      });
  }
  return {
    name: "Random",
    note: "randomized box — tweak it, Save it, or Reset",
    addC: true,
    iters: Math.floor(rnd(9, 14)),
    deOption: 2,
    ops,
    camera: {
      yawDeg: r2(0, 360),
