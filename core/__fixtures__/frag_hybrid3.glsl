...
void iterStep2(inout vec3 pos, inout float w, int i) {
  g_wq = 1.0;

    // sphere fold (uniform scale by k → tracked onto w)
    {
        float minR2 = uP[2] * uP[2];
        float fixedR2 = uP[3] * uP[3];
...
void hybStep(inout vec3 pos, inout float w, inout vec3 c, int i) {
  int period = uHyb[0] + uHyb[1] + uHyb[2];
  ...
  acc += uHyb[2]; if (ph < acc) { iterStep2(pos, w, i); if (uAddGate[2] == 1) pos += c; return; }
}
