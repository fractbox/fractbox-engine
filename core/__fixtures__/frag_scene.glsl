...
uniform int uMaxSteps, uColorMode;
...
// CSG scene per-object arrays + op params + palette stops (cap 8
// objects). They ride a std140 uniform BUFFER, not the default uniform block —
// on a GLES-3.0-minimum device the per-object arrays + uP[192] overflowed the
...
float objOrbit_0(vec3 p0, int mode) {
  vec3 pos = p0; float w = 1.0;
  vec3 c = (uObjJulia[0] > 0.5) ? uObjJc[0] : p0;
  int it = uObjIters[0];
...
float objDE_0(vec3 p0) {
  ...
  float r = length(pos);
  if (uObjDeOption[0] < 1.0) return 0.5 * log(max(r, 1e-9)) * r / max(abs(w), 1e-9);
...
float mapDE(vec3 p_rel) {
  ...
  float d = 1.0e9;
  {
    vec4 q = uObjQuat[0]; vec4 qinv = vec4(-q.x, -q.y, -q.z, q.w);
    vec3 pk = qrotGL(qinv, p0 - uObjOrigin[0]) / uObjUscale[0];
    float dk = objDE_0(pk) * uObjUscale[0];
    d = min(d, dk);
  }
  return d;
}
...
vec3 sceneTintGL(vec3 p_rel) {
  vec3 p0 = p_rel + uOffset;
  float d = 1.0e9; int win = 0;
  {
    vec4 q = uObjQuat[0]; vec4 qinv = vec4(-q.x, -q.y, -q.z, q.w);
    vec3 pk = qrotGL(qinv, p0 - uObjOrigin[0]) / uObjUscale[0];
    float dk = objDE_0(pk) * uObjUscale[0];
    if (dk < d) win = 0; d = min(d, dk);
  }
  return uObjColor[win];
}
...
float sceneOrbitGL(vec3 p_rel, int mode) {
  ...
  if (win == 0) {
    vec4 q = uObjQuat[0]; vec4 qi = vec4(-q.x, -q.y, -q.z, q.w);
    vec3 pk = qrotGL(qi, p0 - uObjOrigin[0]) / uObjUscale[0];
    return objOrbit_0(pk, mode);
  }
  return 0.5;
}
...
void main() {
  ...
  if (uColorMode == 5) {
    ...
    if (uPalCount > 1.5) albedo = albedoStopsGL(mixT);
    else if (uPalOn > 0.5)
