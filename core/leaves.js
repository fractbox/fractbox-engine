...
export const LEAVES = [
  ...
  },
  {
    ...
    wgsl: `let cs = vec2f(prm.x * 1.5, prm.x * 1.7320508);
  ...
  return max(abs(hd) - prm.z, abs(p.z) - prm.y);`,
    glsl: `vec2 cs = vec2(prm.x * 1.5, prm.x * 1.7320508);
  ...
  },
  {
    ...
    // Angular sector quantize → each sector is one slab step at the height of
    // the spiral there (derivable construction; the sector unroll is approx).
    // #353 two fixes:
    //  - Width used to BE the outer radius, so raising it grew the silhouette
    //    from the middle outward. It's now the tread's span inward from a
    //    FIXED outer rim (radius 1) — the outer edge stays put, matching the
    //    reporter's MB3D reference (a stairwell of fixed OD with a bigger/
    //    smaller central void), and the always-open void replaces the old
    //    thin center pole.
    //  - The single-candidate tread height had a genuine VALUE discontinuity
    //    at each sector seam (adjacent sectors round p.y/Pitch independently,
    //    so their nearest tread can differ by more than one Lipschitz-safe
    //    step there) — the raymarcher's classic overshoot-at-a-jump artifact,
    //    seen as noise right at the risers. Blending this sector's tread with
    //    the next one's over the outer 40% of the sector (am/the riser notch
    //    already hides that zone) removes the jump; the inner 60% stays a
    //    flat, undistorted tread.
    wgsl: `let theta = atan2(p.z, p.x) / 6.2831853;
  let v = theta * prm.x;
  let k0 = floor(v);
  let f = v - k0;
  let wgt = smoothstep(0.6, 1.0, f);
  let turnA = round(p.y / prm.w - k0 / prm.x);
  let slabA = abs(p.y - (k0 / prm.x + turnA) * prm.w) - prm.y;
  let k1 = k0 + 1.0;
  let turnB = round(p.y / prm.w - k1 / prm.x);
  let slabB = abs(p.y - (k1 / prm.x + turnB) * prm.w) - prm.y;
  let slab = mix(slabA, slabB, wgt);
  let l = length(p.xz);
  let band = max(l - 1.0, (1.0 - prm.z) - l);
  let am = (0.5 - min(f, 1.0 - f)) / prm.x * 6.2831853 * max(l, 0.05);
  return max(max(slab, band), am - 3.0 / prm.x);`,
    glsl: `float theta = atan(p.z, p.x) / 6.2831853;
  float v = theta * prm.x;
  float k0 = floor(v);
  float f = v - k0;
  float wgt = smoothstep(0.6, 1.0, f);
  float turnA = floor(p.y / prm.w - k0 / prm.x + 0.5);
  float slabA = abs(p.y - (k0 / prm.x + turnA) * prm.w) - prm.y;
  float k1 = k0 + 1.0;
  float turnB = floor(p.y / prm.w - k1 / prm.x + 0.5);
  float slabB = abs(p.y - (k1 / prm.x + turnB) * prm.w) - prm.y;
  float slab = mix(slabA, slabB, wgt);
  float l = length(p.xz);
  float band = max(l - 1.0, (1.0 - prm.z) - l);
  float am = (0.5 - min(f, 1.0 - f)) / prm.x * 6.2831853 * max(l, 0.05);
  return max(max(slab, band), am - 3.0 / prm.x);`,
  },
  {
    id: 19,
  ...
  // sphere-bounded so the algebraic far-field can't leak.
  {
    ...
    // #353 — the published axes (x=left/right, y=thin/depth, z=lobe/point) don't
    // match this engine's fixed default camera (yaw 35°/pitch 22°): mapped 1:1,
    // the camera grazes the shape edge-on and it reads as a featureless blob
    // instead of a heart (confirmed against the reporter's own MathWorld
    // reference — same formula, same expected look, just the wrong axes facing
    // the lens). Re-mapped so the formula's thin axis rides world Z (closest to
    // this camera's forward vector) and its lobe/point axis rides -world Y —
    // screenshot-verified square-on under the untouched default camera at
    // several orbit angles; world X (left/right) is unchanged.
