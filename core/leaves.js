...
export const LEAVES = [
  ...
  },
  {
    ...
    params: [
      ...
      // #627 — the first overflow-lane param (rides objAux/uObjPrimP2/prm2.x,
      // this leaf is the lane's proving rider, the ruckerBulb role): door
      // HALF-width, world units. 0 (the default every existing Room carries)
      // short-circuits to the exact legacy 4-param math.
      { name: "Door", def: 0, min: 0, max: 2, step: 0.01 },
    ],
  ...
  var db2 = db;
  if (prm2.x > 0.0049) {
    let cut = max(abs(p.x) - prm2.x, p.y - 0.5 * prm.y);
    db2 = max(db, -cut);
  }
  return min(df, min(db2, ds));`,
  ...
  float db2 = db;
  if (prm2.x > 0.0049) {
    float cut = max(abs(p.x) - prm2.x, p.y - 0.5 * prm.y);
    db2 = max(db, -cut);
  }
  return min(df, min(db2, ds));`,
  },
  ...
  },
  {
    ...
    unbounded: true,
    params: [
      { name: "Cell", def: 0.5, min: 0.1, max: 2, step: 0.005 },
      { name: "Street", def: 0.25, min: 0.05, max: 0.9, step: 0.005 },
      { name: "Height", def: 0.8, min: 0.05, max: 3, step: 0.005 },
      { name: "Vary", def: 0.6, min: 0, max: 1, step: 0.005 },
      // #627 overflow lane (sp4/sp5 → prm2.xy), like Room's Door.
      { name: "Detail", def: 0.5, min: 0, max: 1, step: 0.005 },
      { name: "Seed", def: 1, min: 0, max: 1023, step: 1 },
    ],
    // Procedural building lattice (#634, Catoptron3D parity — IDEA-LEVEL only:
    // that repo has no license, so this construction is derived independently;
    // no code, constants, or comments were taken from it). A square street
    // grid of pitch c in the xz-plane; each cell (i,k) holds one building —
    // two stacked exact boxes on a shared centre: a base slab up to a hashed
    // split height and a set-back tower to the full hashed height (Detail 0
    // collapses the setback, leaving a plain box). A thin ground slab
    // (y ∈ [−0.1c, 0]) makes the streets read as streets.
    //
    // INVARIANT (the #634 neighborhood contract — the randomCells-style bound
    // the spec review required stated up front): both tiers have horizontal
    // half-extent ≤ fp = 0.5·c·(1−f), with the street fraction f CLAMPED to
    // ≥ 0.05 before use, so every footprint sits ≥ s/2 = 0.5·f·c inside its
    // own cell for every parameter setting, independent of height. A building
    // whose cell lies OUTSIDE the 3×3 window around p's cell is therefore
    // ≥ 1.5c − fp = c + s/2 > c away HORIZONTALLY (offset ≥ 2 in x or z), and
    // 3D distance ≥ horizontal distance regardless of the neighbor's hashed
    // height — so seeding the scan with d = c is a true bound: the taller
    // tower two blocks over can never be nearer than the cap. Inside the
    // window every building is an exact box union; the ground slab is global
    // closed-form and needs no cap. Hence the DE never overestimates —
    // city.test.mjs proves it by brute force against an independent
    // clamp-to-box reference, plus the cell-border tall-neighbor case that a
    // cell-local DE gets wrong. EXACT (no deApprox); cost ≈ randomCells
    // (9 cells × 2 boxes vs 27 cells × 1 primitive).
    //
    // Heights are a pure integer hash of (i, k, seed) — the repo's Wang-style
    // mix from randomCells, kept bit-identical across WGSL/GLSL/CPU
    // (Math.imul parity; float sin-hashes would diverge between tiers).
    wgsl: `let c = clamp(prm.x, 0.1, 2.0);
  let f = clamp(prm.y, 0.05, 0.9);
  let hmax = clamp(prm.z, 0.05, 3.0);
  let vary = clamp(prm.w, 0.0, 1.0);
  let det = clamp(prm2.x, 0.0, 1.0);
  let seed = u32(clamp(prm2.y, 0.0, 1023.0));
  let fp = 0.5 * c * (1.0 - f);
  let gt = 0.05 * c;
  let dg = abs(p.y + gt) - gt;
  var db = c;
  let base = vec2i(floor(p.xz / c));
  for (var oi: i32 = -1; oi <= 1; oi = oi + 1) {
  for (var ok: i32 = -1; ok <= 1; ok = ok + 1) {
    let id = base + vec2i(oi, ok);
    var h = (bitcast<u32>(id.x) * 73856093u) ^ (bitcast<u32>(id.y) * 83492791u) ^ (seed * 2654435761u);
    h = h ^ (h >> 13u); h = h * 1274126177u; h = h ^ (h >> 16u);
    let bh = hmax * max(1.0 - vary * f32(h & 1023u) / 1023.0, 0.02);
    let h1 = bh * (0.45 + 0.25 * f32((h >> 10u) & 255u) / 255.0);
    let fp2 = fp * (1.0 - det * (0.15 + 0.45 * f32((h >> 18u) & 255u) / 255.0));
    let l = p.xz - (vec2f(id) + vec2f(0.5, 0.5)) * c;
    let q1 = abs(vec3f(l.x, p.y - 0.5 * h1, l.y)) - vec3f(fp, 0.5 * h1, fp);
    let d1 = length(max(q1, vec3f(0.0))) + min(max(q1.x, max(q1.y, q1.z)), 0.0);
    let q2 = abs(vec3f(l.x, p.y - 0.5 * bh, l.y)) - vec3f(fp2, 0.5 * bh, fp2);
    let d2 = length(max(q2, vec3f(0.0))) + min(max(q2.x, max(q2.y, q2.z)), 0.0);
    db = min(db, min(d1, d2));
  }}
  return min(dg, db);`,
    glsl: `float c = clamp(prm.x, 0.1, 2.0);
  float f = clamp(prm.y, 0.05, 0.9);
  float hmax = clamp(prm.z, 0.05, 3.0);
  float vary = clamp(prm.w, 0.0, 1.0);
  float det = clamp(prm2.x, 0.0, 1.0);
  uint seed = uint(clamp(prm2.y, 0.0, 1023.0));
  float fp = 0.5 * c * (1.0 - f);
  float gt = 0.05 * c;
  float dg = abs(p.y + gt) - gt;
  float db = c;
  ivec2 base = ivec2(floor(p.xz / c));
  for (int oi = -1; oi <= 1; oi++) {
  for (int ok = -1; ok <= 1; ok++) {
    ivec2 id = base + ivec2(oi, ok);
    uint h = uint(id.x) * 73856093u ^ uint(id.y) * 83492791u ^ (seed * 2654435761u);
    h = h ^ (h >> 13u); h = h * 1274126177u; h = h ^ (h >> 16u);
    float bh = hmax * max(1.0 - vary * float(h & 1023u) / 1023.0, 0.02);
    float h1 = bh * (0.45 + 0.25 * float((h >> 10u) & 255u) / 255.0);
    float fp2 = fp * (1.0 - det * (0.15 + 0.45 * float((h >> 18u) & 255u) / 255.0));
    vec2 l = p.xz - (vec2(id) + vec2(0.5)) * c;
    vec3 q1 = abs(vec3(l.x, p.y - 0.5 * h1, l.y)) - vec3(fp, 0.5 * h1, fp);
    float d1 = length(max(q1, vec3(0.0))) + min(max(q1.x, max(q1.y, q1.z)), 0.0);
    vec3 q2 = abs(vec3(l.x, p.y - 0.5 * bh, l.y)) - vec3(fp2, 0.5 * bh, fp2);
    float d2 = length(max(q2, vec3(0.0))) + min(max(q2.x, max(q2.y, q2.z)), 0.0);
    db = min(db, min(d1, d2));
  }}
  return min(dg, db);`,
  },
...
export function validateLeaves() {
  ...
  for (const l of LEAVES) {
    if (!l.params || l.params.length < 1 || l.params.length > MAX_LEAF_PARAMS)
      failures.push(`${l.key}: leaves take 1-${MAX_LEAF_PARAMS} params`);
    // #627 — params 5+ ride the objAux overflow lane; a fat leaf's WGSL/GLSL
    // must read prm2 (and thin leaves must NOT), or the lane predicate and the
    // emitted signatures desync silently.
    const fat = l.params && l.params.length > MAX_LEAF_PARAMS_INLINE;
    const readsPrm2 =
      /\bprm2\b/.test(String(l.wgsl)) || /\bprm2\b/.test(String(l.glsl));
    if (fat && !readsPrm2)
      failures.push(
        `${l.key}: declares ${l.params.length} params but never reads prm2`,
      );
    if (!fat && readsPrm2)
      failures.push(
        `${l.key}: reads prm2 but declares ≤${MAX_LEAF_PARAMS_INLINE} params`,
      );
