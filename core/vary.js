...
export const TOUCHY = new Set(["translate", "modFold", "tentFold", "absOffsetFold", "zFold"]);
 ...
 * full span (0.15 = a firm remix, 0.5 = a wild new take). Hybrid slot-B ops
 * ride along; scene objects are left to the caller (their params live in
 * transforms, not op values). Pass `rng` for reproducible tests.
 */
export function jitterParams(formula, opts = {}) {
  const { spread = 0.15, touchyScale = 0.3, rng = Math.random } = opts;
  const f = clone(formula);
  for (const op of f.ops) jitterOpValues(op, spread, touchyScale, rng);
  if (f.hybrid?.b?.ops) for (const op of f.hybrid.b.ops) jitterOpValues(op, spread, touchyScale, rng);
  return f;
}
...
export function isSound(formula, opts = {}) {
  ...
  if (deFamily(formula) === "ifs" && Math.abs(scaleProduct(formula)) < minExpansion) return false;
  ...
  for (const region of regions) {
    ...
    const present = typeof m.escaped === "number" ? m.escaped < 1 : true;
    if (!present) continue;
    if (m.wobble <= maxWobble) return true;
    const escapeTime = m.family === "escape" || m.family === "numeric";
    if (escapeTime && typeof m.escaped === "number" && m.escaped <= escapedCap) return true;
  }
