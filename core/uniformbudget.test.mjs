...
const WORST_CASE = {
  ...
  "numeric-DE variant": buildFragGL(
    numericOp
      ? [{ key: numericOp.key, values: numericOp.params.map(() => 0) }]
      : [],
  ),
  // ENVX (backgrounds P5) adds 4 float + 2 vec3 + 1 float scalars to the
  // default block — the heaviest flat and scene shapes must still fit with it.
  "flat (all ops) + envx": buildFragGL(allOps, undefined, undefined, {
    envx: true,
  }),
  "scene (all leaves, 8 objects) + envx": buildSceneFragGL(sceneMax, {
    envx: true,
  }),
  // AURORA (ENVX P6) adds 3 float + 2 vec3 scalars — the heaviest flat and
  // scene shapes must still fit with BOTH sky extensions on at once.
  "flat (all ops) + envx + aurora": buildFragGL(allOps, undefined, undefined, {
    envx: true,
    aurora: true,
  }),
  "scene (all leaves, 8 objects) + envx + aurora": buildSceneFragGL(sceneMax, {
    envx: true,
    aurora: true,
  }),
  // NEON + THIN FILM add one float each, CINE GRADE adds four vec4s, AURORA
  // adds 3 floats + 2 vec3s, to the default block — the heaviest flat shape
  // must still fit with every look-driven scalar on at once.
  "flat (all ops) + envx + neon + aurora + grade + thinFilm": buildFragGL(
    allOps,
    undefined,
    undefined,
    { envx: true, neon: true, aurora: true, grade: true, thinFilm: true },
  ),
};
