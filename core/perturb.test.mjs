...
for (const fix of FIX) {
  test(`ladder — ${fix.name} tracks the exact map to 1e-32 (gate ${GATE.toExponential(1)})`, () => {
    ...
    for (const N of [6, 12, 20, 26, 32]) {
      ...
      if (compared >= 12) {
        trackedRungs++;
        assert.ok(max <= GATE, `rung 1e-${N}: relErr ${max.toExponential(2)} ≤ ${GATE.toExponential(1)}`);
      }
    }
...
test("adversarial margins — reference within 0.3·pixel of a fold plane", () => {
  ...
  for (const N of [8, 20, 30]) {
    ...
    assert.ok(max <= GATE, `1e-${N}: adversarial relErr ${max.toExponential(2)}`);
  }
});
...
test("τ boundary — samples across the switchover stay finite and sane", () => {
  ...
  assert.ok(switchedSeen > 5 && trackedSeen > 5, `both regimes exercised (${switchedSeen}/${trackedSeen})`);
});
...
for (const op of OP_FIXTURES) {
  test(`per-op pin — ${op.key}`, () => {
    ...
    const fix = {
      ...
      iters: 8,
      deOption: 2,
      ops: [op, { key: "boxFold", values: [1.0] }, { key: "sphereFold", values: [0.5, 1.0] }],
    };
...
test("reference escape forces the f32 tail (no NaN, plausible state)", () => {
  ...
  const fix = { addC: true, iters: 20, deOption: 2, ops: [{ key: "scale", values: [2.0] }] };
  const T = [0.9, 0.7, 0.8];
...
test("camera basis exposes an absorption-free residual (D11)", async () => {
  ...
  const cam = makeCamera({ target: [9.7, 2.1, 1.4], dist: 1, yawDeg: 35, pitchDeg: 22 });
  ...
  const rt = [b.eye[0] - cam.target[0], b.eye[1] - cam.target[1], b.eye[2] - cam.target[2]];
  const rtErr = Math.hypot(rt[0] - b.roRel[0], rt[1] - b.roRel[1], rt[2] - b.roRel[2]);
  assert.ok(rtErr / mag > 0.5, `eye−target is garbage here (err ${(rtErr / mag).toExponential(1)}× the residual)`);
});
...
test("wgslPt registry membership === ptSupported membership", async () => {
  ...
  const reg = OPERATORS.filter((o) => o.wgslPt).map((o) => o.key).sort();
  const sup = OPERATORS.filter((o) => ptSupported(o.key)).map((o) => o.key).sort();
...
test("ptEligible: the inversion family is finally deep-eligible", async () => {
  ...
  assert.ok(ptEligible(kd, ptSupported), "Kleinian Drop (radialInvert) eligible");
  assert.ok(ptEligible(TOURBILLON, ptSupported), "Tourbillon eligible");
  assert.ok(
    !ptEligible({ addC: true, iters: 8, ops: [{ key: "kaleido", values: [6] }] }, ptSupported),
    "kaleido not in v1",
  );
  assert.ok(!ptEligible({ ...kd, hybrid: { b: { ops: [] } } }, ptSupported), "hybrid excluded");
  assert.ok(
...
test("preview.js has no raw MIN_DIST clamps (M1 grep-gate)", async () => {
  ...
  assert.ok(src.includes("cam.zoom(factor, ptMinDist())"), "cam.zoom callers pass the floor");
});
