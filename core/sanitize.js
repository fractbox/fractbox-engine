...
import { MAX_LEAF_PARAMS } from "./limits.js";
import {
...
import { CLIP_OFFSET_LIMIT } from "./frameparams.js";
...
const LIGHT_SCALARS = {
  ...
  aperture: [0, 1, 0],
  exposure: [-1, 1, 0],
  // ENVX (backgrounds P5) — stars/band macros + their shape scalars. Bounds are
  // the app's slider domains, like every entry above.
  stars: [0, 1, 0],
  starDensity: [0, 1, 0.5],
  starSeed: [0, 1, 0],
  band: [0, 1, 0],
  bandTilt: [0, 1, 0.6],
  // IMGTEX (#631) — user-image env-map / surface-texture sliders. Settings
  // only ("settings yes, pixels no"): the image itself never rides a share
  // link or PNG chunk, so these are inert until an image is loaded in-session.
  envMapAmount: [0, 1, 1],
  envMapBright: [0, 3, 1],
  envMapRot: [0, 1, 0],
  surfTexAmount: [0, 1, 1],
  surfTexScale: [0.02, 3, 1],
  // #630 self-reflection — bounce count + the three 0..1 controls. The wire
  // writes the count as a plain u8, so the clamp here (not the encoder) is
  // what keeps a hostile payload from requesting a 255-bounce march;
  // deriveFrameParams rounds, so a fractional count can't slip through either.
  reflBounces: [0, 6, 0],
  reflectivity: [0, 1, 0.8],
  reflFresnel: [0, 1, 0.5],
  reflTint: [0, 1, 0],
  // NEON — emissive glow amount. Slider domain 0..1; frameparams owns the
  // HDR gain mapping, so the wire can never request more than the UI can.
  neon: [0, 1, 0],
  // AURORA (ENVX P6) — curtain/nebula amounts + the one hue control. Slider
  // domains; frameparams owns the hue→color derivation, so the wire can never
  // request colors outside the hue slice.
  aurora: [0, 1, 0],
  auroraHue: [0, 1, 0.36],
  nebula: [0, 1, 0],
  // THIN FILM — interference amount. Slider domain 0..1; the shaders shape
  // the angle weight, so the wire can never request more than the UI can.
  thinFilm: [0, 1, 0],
  // CINE GRADE — the named-look id + the one strength slider. The id domain is
  // deliberately wider than the shipped roster (0..15): app/src/looks.ts maps
  // an unknown id to None, so a link from a FUTURE client with more looks
  // degrades to ungraded here rather than aliasing to a wrong look. The
  // derived numeric block (light.grade) is deliberately NOT whitelisted — it
  // is app-derived per push and must never ride an import.
  gradeLook: [0, 15, 0],
  gradeStrength: [0, 1, 1],
  // CLIP (cross-section) — axis index + plane offset. The booleans (clipOn,
  // clipFlip) ride LIGHT_FLAGS below; deriveFrameParams rounds the axis (the
  // reflBounces shape). The offset domain is frameparams' CLIP_OFFSET_LIMIT
  // (world units — WIDER than the World slider's −2..2, because formula
  // world size varies enormously: Mandelbox's body alone reaches ±7, and the
  // MRI sweep drives measured world-sized offsets through this fence). Inert
  // past the measurable world (±48), so a hostile wire value still can't
  // request anything a big offset wouldn't.
  clipAxis: [0, 2, 0],
  clipOffset: [-CLIP_OFFSET_LIMIT, CLIP_OFFSET_LIMIT, 0],
  // CLIP JAGGED — the World-pane "Jagged" slider (0 = exact flat plane;
  // frameparams owns the world-amplitude/Lipschitz derivation, so the wire
  // can never request more erosion than the UI can).
  clipJag: [0, 1, 0],
};
const LIGHT_COLORS = {
  keyColor: [1, 1, 1],
  fillColor: [1, 1, 1],
  backColor: [1, 1, 1],
  // ENVX — the optional zenith color; ABSENT stays absent (shape-preserving),
  // which is the "legacy 0.35·bg zenith" signal.
  zenith: [0, 0, 0.1],
};
const LIGHT_FLAGS = ["sunGlow", "lightIndicator", "clipOn", "clipFlip"];
...
function sanitizeLeafObject(o, oi, caps) {
  ...
  // #627 — width DERIVED, never hand-typed: 4 slots minimum (the struct-Obj
  // inline word, so every existing ≤4-param object round-trips byte-identical),
  // growing to the leaf's declared count (≤ MAX_LEAF_PARAMS) when a fat leaf's
  // overflow params ride the objAux lane. Unknown (future) leaves preserve up
  // to whatever the payload carried, capped at the same ceiling.
  const spWidth = Math.max(
    4,
    Math.min(
      MAX_LEAF_PARAMS,
      leaf?.params?.length ??
        (shapeId > MAX_LEAF_ID && Array.isArray(sp) ? sp.length : 0),
    ),
  );
  const shapeParams = Array.from({ length: spWidth }, (_, j) => {
    const p = leaf?.params?.[j];
    // Unknown (future-D2) leaf: engine-bound clamps on all slots; known
