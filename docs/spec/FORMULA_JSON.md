...
The registry is `core/operators.js`; it holds **68** operators as of this
...
No operator declares more than **6** parameters. (The first three ride the GPU
op struct directly; any beyond that ride a parallel overflow lane. That split is
an engine implementation detail — it changes nothing about this format, where
`values` has simply always been as long as the operator declares.)
...
registry (`core/leaves.js`, currently **59** leaves, ids 1 … 59; `0` means "no
leaf — classic IFS distance"). `shapeParams` is a positional array of up to 8
numbers, clamped per the leaf's declared ranges. Slots 0-3 ride the object's
inline descriptor word; slots 4-7 (#627) ride the objAux overflow lane and
exist only when the leaf declares them — a ≤4-param object's array stays
length 4, and encoders/decoders older than #627 simply see the first 4 (the
share wire carries the overflow in the separate SHAPES2 section).
...
operator parameters across **all** slots MUST NOT exceed **384**. Exceeding it
is a **hard error** — one of only two caps in this format that refuses rather
than truncates.

The budget is the flat op cap (§3) times the per-operator parameter maximum
(§3.2) — 64 × 6 — so a single flat op-list can never exceed it. Only a multi-slot
hybrid can.
...
| A hybrid whose slots pack more than 384 parameters           | `over-cap`      |
...
equivalence is asserted in `core/validate.test.mjs` over the whole 109-preset
...
```json
{
  ...
  "hybrid": {
    "slots": [
      ...
      {
        "ops": [{ "key": "bulbAxis", "values": [5, 1, 0, 1, 1] }],
        "addC": true
      }
    ],
    "schedule": { "counts": [2, 1, 3] }
